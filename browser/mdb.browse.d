...
ifeq ($(TARGET_ASM),yes)
mdb.browse.mh mdb.browse.mih : mdb.browse.s
else
mdb.browse.mh mdb.browse.mih : mdb.browse.c
endif

ifeq ($(TARGET_ASM),yes)
mdb.browse.module_dep : mdb.browse.s
else
 ifeq ($(findstring il,$(GRADE)),il)
mdb.browse.module_dep : mdb.browse.il
  ifeq ($(findstring java,$(GRADE)),java)
mdb.browse.module_dep : mercury/mdb_/browse.java
  else
...
ifeq ($(TARGET_ASM),yes)
mdb.browse.s_date : browse.m
	$(MCG) $(ALL_GRADEFLAGS) $(ALL_MCGFLAGS) --target-code-only mdb.browse $(ERR_REDIRECT)
mdb.browse.pic_s_date : browse.m
	$(MCG) $(ALL_GRADEFLAGS) $(ALL_MCGFLAGS) --target-code-only --pic \
		--cflags "$(GCCFLAGS_FOR_PIC)" mdb.browse $(ERR_REDIRECT)
endif # TARGET_ASM

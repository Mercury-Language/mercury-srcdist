...
ifeq ($(TARGET_ASM),yes)
mdb.name_mangle.mh mdb.name_mangle.mih : mdb.name_mangle.s
else
mdb.name_mangle.mh mdb.name_mangle.mih : mdb.name_mangle.c
endif

ifeq ($(TARGET_ASM),yes)
mdb.name_mangle.module_dep : mdb.name_mangle.s
else
 ifeq ($(findstring il,$(GRADE)),il)
mdb.name_mangle.module_dep : mdb.name_mangle.il
  ifeq ($(findstring java,$(GRADE)),java)
mdb.name_mangle.module_dep : mercury/mdb_/name_mangle.java
  else
...
ifeq ($(TARGET_ASM),yes)
mdb.name_mangle.s_date : name_mangle.m
	$(MCG) $(ALL_GRADEFLAGS) $(ALL_MCGFLAGS) --target-code-only mdb.name_mangle $(ERR_REDIRECT)
mdb.name_mangle.pic_s_date : name_mangle.m
	$(MCG) $(ALL_GRADEFLAGS) $(ALL_MCGFLAGS) --target-code-only --pic \
		--cflags "$(GCCFLAGS_FOR_PIC)" mdb.name_mangle $(ERR_REDIRECT)
endif # TARGET_ASM

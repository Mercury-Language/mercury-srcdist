...
ifeq ($(TARGET_ASM),yes)
mdb.collect_lib.mh mdb.collect_lib.mih : mdb.collect_lib.s
else
mdb.collect_lib.mh mdb.collect_lib.mih : mdb.collect_lib.c
endif

ifeq ($(TARGET_ASM),yes)
mdb.collect_lib.module_dep : mdb.collect_lib.s
else
 ifeq ($(findstring il,$(GRADE)),il)
mdb.collect_lib.module_dep : mdb.collect_lib.il
  ifeq ($(findstring java,$(GRADE)),java)
mdb.collect_lib.module_dep : mercury/mdb_/collect_lib.java
  else
...
ifeq ($(TARGET_ASM),yes)
mdb.collect_lib.s_date : collect_lib.m
	$(MCG) $(ALL_GRADEFLAGS) $(ALL_MCGFLAGS) --target-code-only mdb.collect_lib $(ERR_REDIRECT)
mdb.collect_lib.pic_s_date : collect_lib.m
	$(MCG) $(ALL_GRADEFLAGS) $(ALL_MCGFLAGS) --target-code-only --pic \
		--cflags "$(GCCFLAGS_FOR_PIC)" mdb.collect_lib $(ERR_REDIRECT)
endif # TARGET_ASM

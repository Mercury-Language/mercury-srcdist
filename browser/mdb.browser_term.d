...
ifeq ($(TARGET_ASM),yes)
mdb.browser_term.mh mdb.browser_term.mih : mdb.browser_term.s
else
mdb.browser_term.mh mdb.browser_term.mih : mdb.browser_term.c
endif

ifeq ($(TARGET_ASM),yes)
mdb.browser_term.module_dep : mdb.browser_term.s
else
 ifeq ($(findstring il,$(GRADE)),il)
mdb.browser_term.module_dep : mdb.browser_term.il
  ifeq ($(findstring java,$(GRADE)),java)
mdb.browser_term.module_dep : mercury/mdb_/browser_term.java
  else
...
ifeq ($(TARGET_ASM),yes)
mdb.browser_term.s_date : browser_term.m
	$(MCG) $(ALL_GRADEFLAGS) $(ALL_MCGFLAGS) --target-code-only mdb.browser_term $(ERR_REDIRECT)
mdb.browser_term.pic_s_date : browser_term.m
	$(MCG) $(ALL_GRADEFLAGS) $(ALL_MCGFLAGS) --target-code-only --pic \
		--cflags "$(GCCFLAGS_FOR_PIC)" mdb.browser_term $(ERR_REDIRECT)
endif # TARGET_ASM

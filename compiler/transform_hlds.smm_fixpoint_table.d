...
ifeq ($(TARGET_ASM),yes)
transform_hlds.smm_fixpoint_table.mh transform_hlds.smm_fixpoint_table.mih : transform_hlds.smm_fixpoint_table.s
else
transform_hlds.smm_fixpoint_table.mh transform_hlds.smm_fixpoint_table.mih : transform_hlds.smm_fixpoint_table.c
endif

ifeq ($(TARGET_ASM),yes)
transform_hlds.smm_fixpoint_table.module_dep : transform_hlds.smm_fixpoint_table.s
else
 ifeq ($(findstring il,$(GRADE)),il)
transform_hlds.smm_fixpoint_table.module_dep : transform_hlds.smm_fixpoint_table.il
  ifeq ($(findstring java,$(GRADE)),java)
transform_hlds.smm_fixpoint_table.module_dep : mercury/transform_hlds_/smm_fixpoint_table.java
  else
...
ifeq ($(TARGET_ASM),yes)
transform_hlds.smm_fixpoint_table.s_date : smm_fixpoint_table.m
	$(MCG) $(ALL_GRADEFLAGS) $(ALL_MCGFLAGS) --target-code-only transform_hlds.smm_fixpoint_table $(ERR_REDIRECT)
transform_hlds.smm_fixpoint_table.pic_s_date : smm_fixpoint_table.m
	$(MCG) $(ALL_GRADEFLAGS) $(ALL_MCGFLAGS) --target-code-only --pic \
		--cflags "$(GCCFLAGS_FOR_PIC)" transform_hlds.smm_fixpoint_table $(ERR_REDIRECT)
endif # TARGET_ASM

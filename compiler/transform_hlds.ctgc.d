...
ifeq ($(TARGET_ASM),yes)
transform_hlds.ctgc.mh transform_hlds.ctgc.mih : transform_hlds.ctgc.s
else
transform_hlds.ctgc.mh transform_hlds.ctgc.mih : transform_hlds.ctgc.c
endif

ifeq ($(TARGET_ASM),yes)
transform_hlds.ctgc.module_dep : transform_hlds.ctgc.s
else
 ifeq ($(findstring il,$(GRADE)),il)
transform_hlds.ctgc.module_dep : transform_hlds.ctgc.il
  ifeq ($(findstring java,$(GRADE)),java)
transform_hlds.ctgc.module_dep : mercury/transform_hlds_/ctgc.java
  else
...
ifeq ($(TARGET_ASM),yes)
transform_hlds.ctgc.s_date : ctgc.m
	$(MCG) $(ALL_GRADEFLAGS) $(ALL_MCGFLAGS) --target-code-only transform_hlds.ctgc $(ERR_REDIRECT)
transform_hlds.ctgc.pic_s_date : ctgc.m
	$(MCG) $(ALL_GRADEFLAGS) $(ALL_MCGFLAGS) --target-code-only --pic \
		--cflags "$(GCCFLAGS_FOR_PIC)" transform_hlds.ctgc $(ERR_REDIRECT)
endif # TARGET_ASM

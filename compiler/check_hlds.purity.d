# vim: ts=8 sw=8 noexpandtab ft=make

ifeq (x,y)



check_hlds.purity.optdate check_hlds.purity.trans_opt_date purity.err check_hlds.purity.c_date check_hlds.purity.java_date : purity.m \
	check_hlds.int0 \
	assoc_list.int \
	builtin.int \
	hlds.int \
	io.int \
	list.int \
	map.int \
	maybe.int \
	mdbcomp.int \
	pair.int \
	parse_tree.int \
	private_builtin.int \
	require.int \
	string.int \
	term.int \
	varset.int \
	check_hlds.check_promise.int \
	check_hlds.resolve_unify_functor.int \
	hlds.from_ground_term_util.int \
	hlds.goal_util.int \
	hlds.hlds_clauses.int \
	hlds.hlds_error_util.int \
	hlds.hlds_goal.int \
	hlds.hlds_module.int \
	hlds.hlds_pred.int \
	hlds.hlds_rtti.int \
	hlds.instmap.int \
	hlds.passes_aux.int \
	hlds.pred_table.int \
	hlds.quantification.int \
	hlds.vartypes.int \
	mdbcomp.builtin_modules.int \
	mdbcomp.prim_data.int \
	mdbcomp.sym_name.int \
	parse_tree.builtin_lib_types.int \
	parse_tree.error_util.int \
	parse_tree.parse_tree_out_term.int \
	parse_tree.prog_data.int \
	parse_tree.prog_data_foreign.int \
	parse_tree.prog_mode.int \
	parse_tree.prog_out.int \
	parse_tree.prog_type.int \
	parse_tree.set_of_var.int \
	analysis.int2 \
	array.int2 \
	bag.int2 \
	bimap.int2 \
	bitmap.int2 \
	bool.int2 \
	char.int2 \
	construct.int2 \
	cord.int2 \
	deconstruct.int2 \
	digraph.int2 \
	enum.int2 \
	getopt_io.int2 \
	integer.int2 \
	libs.int2 \
	mode_robdd.int2 \
	multi_map.int2 \
	ops.int2 \
	pretty_printer.int2 \
	queue.int2 \
	random.int2 \
	recompilation.int2 \
	robdd.int2 \
	rtti_implementation.int2 \
	set.int2 \
	set_ordlist.int2 \
	set_tree234.int2 \
	sparse_bitset.int2 \
	stack.int2 \
	stream.int2 \
	time.int2 \
	transform_hlds.int2 \
	tree234.int2 \
	type_desc.int2 \
	unit.int2 \
	univ.int2 \
	check_hlds.delay_info.int2 \
	check_hlds.mode_constraint_robdd.int2 \
	check_hlds.mode_errors.int2 \
	check_hlds.mode_info.int2 \
	check_hlds.proc_requests.int2 \
	hlds.const_struct.int2 \
	hlds.hlds_args.int2 \
	hlds.hlds_data.int2 \
	hlds.hlds_dependency_graph.int2 \
	hlds.hlds_llds.int2 \
	hlds.inst_graph.int2 \
	hlds.special_pred.int2 \
	hlds.status.int2 \
	libs.compiler_util.int2 \
	libs.dependency_graph.int2 \
	libs.globals.int2 \
	libs.lp_rational.int2 \
	libs.op_mode.int2 \
	libs.options.int2 \
	libs.polyhedron.int2 \
	libs.rat.int2 \
	libs.timestamp.int2 \
	libs.trace_params.int2 \
	mdbcomp.feedback.int2 \
	mdbcomp.goal_path.int2 \
	mdbcomp.program_representation.int2 \
	mdbcomp.rtti_access.int2 \
	mdbcomp.trace_counts.int2 \
	mode_robdd.tfeirn.int2 \
	parse_tree.file_kind.int2 \
	parse_tree.maybe_error.int2 \
	parse_tree.module_qual.int2 \
	parse_tree.parse_tree_out_info.int2 \
	parse_tree.prog_data_event.int2 \
	parse_tree.prog_data_pragma.int2 \
	parse_tree.prog_data_used_modules.int2 \
	parse_tree.prog_foreign.int2 \
	parse_tree.prog_item.int2 \
	parse_tree.prog_rename.int2 \
	transform_hlds.term_constr_data.int2 \
	transform_hlds.term_constr_errors.int2 \
	transform_hlds.term_constr_main_types.int2 \
	transform_hlds.term_errors.int2 \
	transform_hlds.term_norm.int2 \
	transform_hlds.term_util.int2 \
	mdbcomp.feedback.automatic_parallelism.int2


endif # ifeq(x,y)

# RULE date_file_deps
# source group names:
#   (unnamed)
#   parent deps
#   long deps
#   short deps
check_hlds.purity.optdate check_hlds.purity.trans_opt_date purity.err check_hlds.purity.c_date check_hlds.purity.java_date : \
		purity.m \
	\
		check_hlds.int0 \
	\
		assoc_list.int \
		builtin.int \
		hlds.int \
		io.int \
		list.int \
		map.int \
		maybe.int \
		mdbcomp.int \
		pair.int \
		parse_tree.int \
		private_builtin.int \
		require.int \
		string.int \
		term.int \
		varset.int \
		check_hlds.check_promise.int \
		check_hlds.resolve_unify_functor.int \
		hlds.from_ground_term_util.int \
		hlds.goal_util.int \
		hlds.hlds_clauses.int \
		hlds.hlds_error_util.int \
		hlds.hlds_goal.int \
		hlds.hlds_module.int \
		hlds.hlds_pred.int \
		hlds.hlds_rtti.int \
		hlds.instmap.int \
		hlds.passes_aux.int \
		hlds.pred_table.int \
		hlds.quantification.int \
		hlds.vartypes.int \
		mdbcomp.builtin_modules.int \
		mdbcomp.prim_data.int \
		mdbcomp.sym_name.int \
		parse_tree.builtin_lib_types.int \
		parse_tree.error_util.int \
		parse_tree.parse_tree_out_term.int \
		parse_tree.prog_data.int \
		parse_tree.prog_data_foreign.int \
		parse_tree.prog_mode.int \
		parse_tree.prog_out.int \
		parse_tree.prog_type.int \
		parse_tree.set_of_var.int \
	\
		analysis.int2 \
		array.int2 \
		bag.int2 \
		bimap.int2 \
		bitmap.int2 \
		bool.int2 \
		char.int2 \
		construct.int2 \
		cord.int2 \
		deconstruct.int2 \
		digraph.int2 \
		enum.int2 \
		getopt_io.int2 \
		integer.int2 \
		libs.int2 \
		mode_robdd.int2 \
		multi_map.int2 \
		ops.int2 \
		pretty_printer.int2 \
		queue.int2 \
		random.int2 \
		recompilation.int2 \
		robdd.int2 \
		rtti_implementation.int2 \
		set.int2 \
		set_ordlist.int2 \
		set_tree234.int2 \
		sparse_bitset.int2 \
		stack.int2 \
		stream.int2 \
		time.int2 \
		transform_hlds.int2 \
		tree234.int2 \
		type_desc.int2 \
		unit.int2 \
		univ.int2 \
		check_hlds.delay_info.int2 \
		check_hlds.mode_constraint_robdd.int2 \
		check_hlds.mode_errors.int2 \
		check_hlds.mode_info.int2 \
		check_hlds.proc_requests.int2 \
		hlds.const_struct.int2 \
		hlds.hlds_args.int2 \
		hlds.hlds_data.int2 \
		hlds.hlds_dependency_graph.int2 \
		hlds.hlds_llds.int2 \
		hlds.inst_graph.int2 \
		hlds.special_pred.int2 \
		hlds.status.int2 \
		libs.compiler_util.int2 \
		libs.dependency_graph.int2 \
		libs.globals.int2 \
		libs.lp_rational.int2 \
		libs.op_mode.int2 \
		libs.options.int2 \
		libs.polyhedron.int2 \
		libs.rat.int2 \
		libs.timestamp.int2 \
		libs.trace_params.int2 \
		mdbcomp.feedback.int2 \
		mdbcomp.goal_path.int2 \
		mdbcomp.program_representation.int2 \
		mdbcomp.rtti_access.int2 \
		mdbcomp.trace_counts.int2 \
		mode_robdd.tfeirn.int2 \
		parse_tree.file_kind.int2 \
		parse_tree.maybe_error.int2 \
		parse_tree.module_qual.int2 \
		parse_tree.parse_tree_out_info.int2 \
		parse_tree.prog_data_event.int2 \
		parse_tree.prog_data_pragma.int2 \
		parse_tree.prog_data_used_modules.int2 \
		parse_tree.prog_foreign.int2 \
		parse_tree.prog_item.int2 \
		parse_tree.prog_rename.int2 \
		transform_hlds.term_constr_data.int2 \
		transform_hlds.term_constr_errors.int2 \
		transform_hlds.term_constr_main_types.int2 \
		transform_hlds.term_errors.int2 \
		transform_hlds.term_norm.int2 \
		transform_hlds.term_util.int2 \
		mdbcomp.feedback.automatic_parallelism.int2

ifeq (x,y)



check_hlds.purity.pic_o check_hlds.purity.$O : \
	analysis.mih \
	array.mih \
	assoc_list.mih \
	bag.mih \
	bimap.mih \
	bitmap.mih \
	bool.mih \
	builtin.mih \
	char.mih \
	check_hlds.mih \
	construct.mih \
	cord.mih \
	deconstruct.mih \
	digraph.mih \
	enum.mih \
	getopt_io.mih \
	hlds.mih \
	integer.mih \
	io.mih \
	libs.mih \
	list.mih \
	map.mih \
	maybe.mih \
	mdbcomp.mih \
	mode_robdd.mih \
	multi_map.mih \
	ops.mih \
	pair.mih \
	parse_tree.mih \
	pretty_printer.mih \
	private_builtin.mih \
	queue.mih \
	random.mih \
	recompilation.mih \
	require.mih \
	robdd.mih \
	rtti_implementation.mih \
	set.mih \
	set_ordlist.mih \
	set_tree234.mih \
	sparse_bitset.mih \
	stack.mih \
	stream.mih \
	string.mih \
	term.mih \
	time.mih \
	transform_hlds.mih \
	tree234.mih \
	type_desc.mih \
	unit.mih \
	univ.mih \
	varset.mih \
	check_hlds.check_promise.mih \
	check_hlds.delay_info.mih \
	check_hlds.mode_constraint_robdd.mih \
	check_hlds.mode_errors.mih \
	check_hlds.mode_info.mih \
	check_hlds.proc_requests.mih \
	check_hlds.resolve_unify_functor.mih \
	hlds.const_struct.mih \
	hlds.from_ground_term_util.mih \
	hlds.goal_util.mih \
	hlds.hlds_args.mih \
	hlds.hlds_clauses.mih \
	hlds.hlds_data.mih \
	hlds.hlds_dependency_graph.mih \
	hlds.hlds_error_util.mih \
	hlds.hlds_goal.mih \
	hlds.hlds_llds.mih \
	hlds.hlds_module.mih \
	hlds.hlds_pred.mih \
	hlds.hlds_rtti.mih \
	hlds.inst_graph.mih \
	hlds.instmap.mih \
	hlds.passes_aux.mih \
	hlds.pred_table.mih \
	hlds.quantification.mih \
	hlds.special_pred.mih \
	hlds.status.mih \
	hlds.vartypes.mih \
	libs.compiler_util.mih \
	libs.dependency_graph.mih \
	libs.globals.mih \
	libs.lp_rational.mih \
	libs.op_mode.mih \
	libs.options.mih \
	libs.polyhedron.mih \
	libs.rat.mih \
	libs.timestamp.mih \
	libs.trace_params.mih \
	mdbcomp.builtin_modules.mih \
	mdbcomp.feedback.mih \
	mdbcomp.goal_path.mih \
	mdbcomp.prim_data.mih \
	mdbcomp.program_representation.mih \
	mdbcomp.rtti_access.mih \
	mdbcomp.sym_name.mih \
	mdbcomp.trace_counts.mih \
	mode_robdd.tfeirn.mih \
	parse_tree.builtin_lib_types.mih \
	parse_tree.error_util.mih \
	parse_tree.file_kind.mih \
	parse_tree.maybe_error.mih \
	parse_tree.module_qual.mih \
	parse_tree.parse_tree_out_info.mih \
	parse_tree.parse_tree_out_term.mih \
	parse_tree.prog_data.mih \
	parse_tree.prog_data_event.mih \
	parse_tree.prog_data_foreign.mih \
	parse_tree.prog_data_pragma.mih \
	parse_tree.prog_data_used_modules.mih \
	parse_tree.prog_foreign.mih \
	parse_tree.prog_item.mih \
	parse_tree.prog_mode.mih \
	parse_tree.prog_out.mih \
	parse_tree.prog_rename.mih \
	parse_tree.prog_type.mih \
	parse_tree.set_of_var.mih \
	transform_hlds.term_constr_data.mih \
	transform_hlds.term_constr_errors.mih \
	transform_hlds.term_constr_main_types.mih \
	transform_hlds.term_errors.mih \
	transform_hlds.term_norm.mih \
	transform_hlds.term_util.mih \
	mdbcomp.feedback.automatic_parallelism.mih


endif # ifeq(x,y)

# RULE objs_on_mihs
check_hlds.purity.pic_o check_hlds.purity.$O : \
		analysis.mih \
		array.mih \
		assoc_list.mih \
		bag.mih \
		bimap.mih \
		bitmap.mih \
		bool.mih \
		builtin.mih \
		char.mih \
		check_hlds.mih \
		construct.mih \
		cord.mih \
		deconstruct.mih \
		digraph.mih \
		enum.mih \
		getopt_io.mih \
		hlds.mih \
		integer.mih \
		io.mih \
		libs.mih \
		list.mih \
		map.mih \
		maybe.mih \
		mdbcomp.mih \
		mode_robdd.mih \
		multi_map.mih \
		ops.mih \
		pair.mih \
		parse_tree.mih \
		pretty_printer.mih \
		private_builtin.mih \
		queue.mih \
		random.mih \
		recompilation.mih \
		require.mih \
		robdd.mih \
		rtti_implementation.mih \
		set.mih \
		set_ordlist.mih \
		set_tree234.mih \
		sparse_bitset.mih \
		stack.mih \
		stream.mih \
		string.mih \
		term.mih \
		time.mih \
		transform_hlds.mih \
		tree234.mih \
		type_desc.mih \
		unit.mih \
		univ.mih \
		varset.mih \
		check_hlds.check_promise.mih \
		check_hlds.delay_info.mih \
		check_hlds.mode_constraint_robdd.mih \
		check_hlds.mode_errors.mih \
		check_hlds.mode_info.mih \
		check_hlds.proc_requests.mih \
		check_hlds.resolve_unify_functor.mih \
		hlds.const_struct.mih \
		hlds.from_ground_term_util.mih \
		hlds.goal_util.mih \
		hlds.hlds_args.mih \
		hlds.hlds_clauses.mih \
		hlds.hlds_data.mih \
		hlds.hlds_dependency_graph.mih \
		hlds.hlds_error_util.mih \
		hlds.hlds_goal.mih \
		hlds.hlds_llds.mih \
		hlds.hlds_module.mih \
		hlds.hlds_pred.mih \
		hlds.hlds_rtti.mih \
		hlds.inst_graph.mih \
		hlds.instmap.mih \
		hlds.passes_aux.mih \
		hlds.pred_table.mih \
		hlds.quantification.mih \
		hlds.special_pred.mih \
		hlds.status.mih \
		hlds.vartypes.mih \
		libs.compiler_util.mih \
		libs.dependency_graph.mih \
		libs.globals.mih \
		libs.lp_rational.mih \
		libs.op_mode.mih \
		libs.options.mih \
		libs.polyhedron.mih \
		libs.rat.mih \
		libs.timestamp.mih \
		libs.trace_params.mih \
		mdbcomp.builtin_modules.mih \
		mdbcomp.feedback.mih \
		mdbcomp.goal_path.mih \
		mdbcomp.prim_data.mih \
		mdbcomp.program_representation.mih \
		mdbcomp.rtti_access.mih \
		mdbcomp.sym_name.mih \
		mdbcomp.trace_counts.mih \
		mode_robdd.tfeirn.mih \
		parse_tree.builtin_lib_types.mih \
		parse_tree.error_util.mih \
		parse_tree.file_kind.mih \
		parse_tree.maybe_error.mih \
		parse_tree.module_qual.mih \
		parse_tree.parse_tree_out_info.mih \
		parse_tree.parse_tree_out_term.mih \
		parse_tree.prog_data.mih \
		parse_tree.prog_data_event.mih \
		parse_tree.prog_data_foreign.mih \
		parse_tree.prog_data_pragma.mih \
		parse_tree.prog_data_used_modules.mih \
		parse_tree.prog_foreign.mih \
		parse_tree.prog_item.mih \
		parse_tree.prog_mode.mih \
		parse_tree.prog_out.mih \
		parse_tree.prog_rename.mih \
		parse_tree.prog_type.mih \
		parse_tree.set_of_var.mih \
		transform_hlds.term_constr_data.mih \
		transform_hlds.term_constr_errors.mih \
		transform_hlds.term_constr_main_types.mih \
		transform_hlds.term_errors.mih \
		transform_hlds.term_norm.mih \
		transform_hlds.term_util.mih \
		mdbcomp.feedback.automatic_parallelism.mih

ifeq (x,y)



check_hlds.purity.mh check_hlds.purity.mih : check_hlds.purity.c


endif # ifeq(x,y)

# RULE mh_and_mih_on_c
check_hlds.purity.mh check_hlds.purity.mih : check_hlds.purity.c

ifeq (x,y)



ifeq ($(findstring java,$(GRADE)),java)
check_hlds.purity.module_dep : jmercury/check_hlds__purity.java
else
check_hlds.purity.module_dep : check_hlds.purity.c
endif


endif # ifeq(x,y)

ifeq ($(findstring java,$(GRADE)),java)

# RULE module_dep_on_java
check_hlds.purity.module_dep : jmercury/check_hlds__purity.java

else

# RULE module_dep_on_c
check_hlds.purity.module_dep : check_hlds.purity.c

endif # conditional fragment

ifeq (x,y)



check_hlds.purity.date check_hlds.purity.date0 \
	check_hlds.date : purity.m \
	check_hlds.int0 \
	assoc_list.int3 \
	builtin.int3 \
	hlds.int3 \
	io.int3 \
	list.int3 \
	map.int3 \
	maybe.int3 \
	mdbcomp.int3 \
	pair.int3 \
	parse_tree.int3 \
	private_builtin.int3 \
	require.int3 \
	string.int3 \
	term.int3 \
	varset.int3 \
	check_hlds.check_promise.int3 \
	check_hlds.resolve_unify_functor.int3 \
	hlds.from_ground_term_util.int3 \
	hlds.goal_util.int3 \
	hlds.hlds_clauses.int3 \
	hlds.hlds_error_util.int3 \
	hlds.hlds_goal.int3 \
	hlds.hlds_module.int3 \
	hlds.hlds_pred.int3 \
	hlds.hlds_rtti.int3 \
	hlds.instmap.int3 \
	hlds.passes_aux.int3 \
	hlds.pred_table.int3 \
	hlds.quantification.int3 \
	hlds.vartypes.int3 \
	mdbcomp.builtin_modules.int3 \
	mdbcomp.prim_data.int3 \
	mdbcomp.sym_name.int3 \
	parse_tree.builtin_lib_types.int3 \
	parse_tree.error_util.int3 \
	parse_tree.parse_tree_out_term.int3 \
	parse_tree.prog_data.int3 \
	parse_tree.prog_data_foreign.int3 \
	parse_tree.prog_mode.int3 \
	parse_tree.prog_out.int3 \
	parse_tree.prog_type.int3 \
	parse_tree.set_of_var.int3 \
	analysis.int3 \
	array.int3 \
	bag.int3 \
	bimap.int3 \
	bitmap.int3 \
	bool.int3 \
	char.int3 \
	construct.int3 \
	cord.int3 \
	deconstruct.int3 \
	digraph.int3 \
	enum.int3 \
	getopt_io.int3 \
	integer.int3 \
	libs.int3 \
	mode_robdd.int3 \
	multi_map.int3 \
	ops.int3 \
	pretty_printer.int3 \
	queue.int3 \
	random.int3 \
	recompilation.int3 \
	robdd.int3 \
	rtti_implementation.int3 \
	set.int3 \
	set_ordlist.int3 \
	set_tree234.int3 \
	sparse_bitset.int3 \
	stack.int3 \
	stream.int3 \
	time.int3 \
	transform_hlds.int3 \
	tree234.int3 \
	type_desc.int3 \
	unit.int3 \
	univ.int3 \
	check_hlds.delay_info.int3 \
	check_hlds.mode_constraint_robdd.int3 \
	check_hlds.mode_errors.int3 \
	check_hlds.mode_info.int3 \
	check_hlds.proc_requests.int3 \
	hlds.const_struct.int3 \
	hlds.hlds_args.int3 \
	hlds.hlds_data.int3 \
	hlds.hlds_dependency_graph.int3 \
	hlds.hlds_llds.int3 \
	hlds.inst_graph.int3 \
	hlds.special_pred.int3 \
	hlds.status.int3 \
	libs.compiler_util.int3 \
	libs.dependency_graph.int3 \
	libs.globals.int3 \
	libs.lp_rational.int3 \
	libs.op_mode.int3 \
	libs.options.int3 \
	libs.polyhedron.int3 \
	libs.rat.int3 \
	libs.timestamp.int3 \
	libs.trace_params.int3 \
	mdbcomp.feedback.int3 \
	mdbcomp.goal_path.int3 \
	mdbcomp.program_representation.int3 \
	mdbcomp.rtti_access.int3 \
	mdbcomp.trace_counts.int3 \
	mode_robdd.tfeirn.int3 \
	parse_tree.file_kind.int3 \
	parse_tree.maybe_error.int3 \
	parse_tree.module_qual.int3 \
	parse_tree.parse_tree_out_info.int3 \
	parse_tree.prog_data_event.int3 \
	parse_tree.prog_data_pragma.int3 \
	parse_tree.prog_data_used_modules.int3 \
	parse_tree.prog_foreign.int3 \
	parse_tree.prog_item.int3 \
	parse_tree.prog_rename.int3 \
	transform_hlds.term_constr_data.int3 \
	transform_hlds.term_constr_errors.int3 \
	transform_hlds.term_constr_main_types.int3 \
	transform_hlds.term_errors.int3 \
	transform_hlds.term_norm.int3 \
	transform_hlds.term_util.int3 \
	mdbcomp.feedback.automatic_parallelism.int3


endif # ifeq(x,y)

# RULE self_and_parent_date_deps
# source group names:
#   (unnamed)
#   parent dep int0
#   long dep int3s
#   short dep int3s
check_hlds.purity.date check_hlds.purity.date0 check_hlds.date : \
		purity.m \
	\
		check_hlds.int0 \
	\
		assoc_list.int3 \
		builtin.int3 \
		hlds.int3 \
		io.int3 \
		list.int3 \
		map.int3 \
		maybe.int3 \
		mdbcomp.int3 \
		pair.int3 \
		parse_tree.int3 \
		private_builtin.int3 \
		require.int3 \
		string.int3 \
		term.int3 \
		varset.int3 \
		check_hlds.check_promise.int3 \
		check_hlds.resolve_unify_functor.int3 \
		hlds.from_ground_term_util.int3 \
		hlds.goal_util.int3 \
		hlds.hlds_clauses.int3 \
		hlds.hlds_error_util.int3 \
		hlds.hlds_goal.int3 \
		hlds.hlds_module.int3 \
		hlds.hlds_pred.int3 \
		hlds.hlds_rtti.int3 \
		hlds.instmap.int3 \
		hlds.passes_aux.int3 \
		hlds.pred_table.int3 \
		hlds.quantification.int3 \
		hlds.vartypes.int3 \
		mdbcomp.builtin_modules.int3 \
		mdbcomp.prim_data.int3 \
		mdbcomp.sym_name.int3 \
		parse_tree.builtin_lib_types.int3 \
		parse_tree.error_util.int3 \
		parse_tree.parse_tree_out_term.int3 \
		parse_tree.prog_data.int3 \
		parse_tree.prog_data_foreign.int3 \
		parse_tree.prog_mode.int3 \
		parse_tree.prog_out.int3 \
		parse_tree.prog_type.int3 \
		parse_tree.set_of_var.int3 \
	\
		analysis.int3 \
		array.int3 \
		bag.int3 \
		bimap.int3 \
		bitmap.int3 \
		bool.int3 \
		char.int3 \
		construct.int3 \
		cord.int3 \
		deconstruct.int3 \
		digraph.int3 \
		enum.int3 \
		getopt_io.int3 \
		integer.int3 \
		libs.int3 \
		mode_robdd.int3 \
		multi_map.int3 \
		ops.int3 \
		pretty_printer.int3 \
		queue.int3 \
		random.int3 \
		recompilation.int3 \
		robdd.int3 \
		rtti_implementation.int3 \
		set.int3 \
		set_ordlist.int3 \
		set_tree234.int3 \
		sparse_bitset.int3 \
		stack.int3 \
		stream.int3 \
		time.int3 \
		transform_hlds.int3 \
		tree234.int3 \
		type_desc.int3 \
		unit.int3 \
		univ.int3 \
		check_hlds.delay_info.int3 \
		check_hlds.mode_constraint_robdd.int3 \
		check_hlds.mode_errors.int3 \
		check_hlds.mode_info.int3 \
		check_hlds.proc_requests.int3 \
		hlds.const_struct.int3 \
		hlds.hlds_args.int3 \
		hlds.hlds_data.int3 \
		hlds.hlds_dependency_graph.int3 \
		hlds.hlds_llds.int3 \
		hlds.inst_graph.int3 \
		hlds.special_pred.int3 \
		hlds.status.int3 \
		libs.compiler_util.int3 \
		libs.dependency_graph.int3 \
		libs.globals.int3 \
		libs.lp_rational.int3 \
		libs.op_mode.int3 \
		libs.options.int3 \
		libs.polyhedron.int3 \
		libs.rat.int3 \
		libs.timestamp.int3 \
		libs.trace_params.int3 \
		mdbcomp.feedback.int3 \
		mdbcomp.goal_path.int3 \
		mdbcomp.program_representation.int3 \
		mdbcomp.rtti_access.int3 \
		mdbcomp.trace_counts.int3 \
		mode_robdd.tfeirn.int3 \
		parse_tree.file_kind.int3 \
		parse_tree.maybe_error.int3 \
		parse_tree.module_qual.int3 \
		parse_tree.parse_tree_out_info.int3 \
		parse_tree.prog_data_event.int3 \
		parse_tree.prog_data_pragma.int3 \
		parse_tree.prog_data_used_modules.int3 \
		parse_tree.prog_foreign.int3 \
		parse_tree.prog_item.int3 \
		parse_tree.prog_rename.int3 \
		transform_hlds.term_constr_data.int3 \
		transform_hlds.term_constr_errors.int3 \
		transform_hlds.term_constr_main_types.int3 \
		transform_hlds.term_errors.int3 \
		transform_hlds.term_norm.int3 \
		transform_hlds.term_util.int3 \
		mdbcomp.feedback.automatic_parallelism.int3

ifeq (x,y)



check_hlds.purity.date0 \
	check_hlds.date0 : purity.m \
	assoc_list.int3 \
	builtin.int3 \
	hlds.int3 \
	io.int3 \
	list.int3 \
	map.int3 \
	maybe.int3 \
	mdbcomp.int3 \
	pair.int3 \
	parse_tree.int3 \
	private_builtin.int3 \
	require.int3 \
	string.int3 \
	term.int3 \
	varset.int3 \
	check_hlds.check_promise.int3 \
	check_hlds.resolve_unify_functor.int3 \
	hlds.from_ground_term_util.int3 \
	hlds.goal_util.int3 \
	hlds.hlds_clauses.int3 \
	hlds.hlds_error_util.int3 \
	hlds.hlds_goal.int3 \
	hlds.hlds_module.int3 \
	hlds.hlds_pred.int3 \
	hlds.hlds_rtti.int3 \
	hlds.instmap.int3 \
	hlds.passes_aux.int3 \
	hlds.pred_table.int3 \
	hlds.quantification.int3 \
	hlds.vartypes.int3 \
	mdbcomp.builtin_modules.int3 \
	mdbcomp.prim_data.int3 \
	mdbcomp.sym_name.int3 \
	parse_tree.builtin_lib_types.int3 \
	parse_tree.error_util.int3 \
	parse_tree.parse_tree_out_term.int3 \
	parse_tree.prog_data.int3 \
	parse_tree.prog_data_foreign.int3 \
	parse_tree.prog_mode.int3 \
	parse_tree.prog_out.int3 \
	parse_tree.prog_type.int3 \
	parse_tree.set_of_var.int3 \
	analysis.int3 \
	array.int3 \
	bag.int3 \
	bimap.int3 \
	bitmap.int3 \
	bool.int3 \
	char.int3 \
	construct.int3 \
	cord.int3 \
	deconstruct.int3 \
	digraph.int3 \
	enum.int3 \
	getopt_io.int3 \
	integer.int3 \
	libs.int3 \
	mode_robdd.int3 \
	multi_map.int3 \
	ops.int3 \
	pretty_printer.int3 \
	queue.int3 \
	random.int3 \
	recompilation.int3 \
	robdd.int3 \
	rtti_implementation.int3 \
	set.int3 \
	set_ordlist.int3 \
	set_tree234.int3 \
	sparse_bitset.int3 \
	stack.int3 \
	stream.int3 \
	time.int3 \
	transform_hlds.int3 \
	tree234.int3 \
	type_desc.int3 \
	unit.int3 \
	univ.int3 \
	check_hlds.delay_info.int3 \
	check_hlds.mode_constraint_robdd.int3 \
	check_hlds.mode_errors.int3 \
	check_hlds.mode_info.int3 \
	check_hlds.proc_requests.int3 \
	hlds.const_struct.int3 \
	hlds.hlds_args.int3 \
	hlds.hlds_data.int3 \
	hlds.hlds_dependency_graph.int3 \
	hlds.hlds_llds.int3 \
	hlds.inst_graph.int3 \
	hlds.special_pred.int3 \
	hlds.status.int3 \
	libs.compiler_util.int3 \
	libs.dependency_graph.int3 \
	libs.globals.int3 \
	libs.lp_rational.int3 \
	libs.op_mode.int3 \
	libs.options.int3 \
	libs.polyhedron.int3 \
	libs.rat.int3 \
	libs.timestamp.int3 \
	libs.trace_params.int3 \
	mdbcomp.feedback.int3 \
	mdbcomp.goal_path.int3 \
	mdbcomp.program_representation.int3 \
	mdbcomp.rtti_access.int3 \
	mdbcomp.trace_counts.int3 \
	mode_robdd.tfeirn.int3 \
	parse_tree.file_kind.int3 \
	parse_tree.maybe_error.int3 \
	parse_tree.module_qual.int3 \
	parse_tree.parse_tree_out_info.int3 \
	parse_tree.prog_data_event.int3 \
	parse_tree.prog_data_pragma.int3 \
	parse_tree.prog_data_used_modules.int3 \
	parse_tree.prog_foreign.int3 \
	parse_tree.prog_item.int3 \
	parse_tree.prog_rename.int3 \
	transform_hlds.term_constr_data.int3 \
	transform_hlds.term_constr_errors.int3 \
	transform_hlds.term_constr_main_types.int3 \
	transform_hlds.term_errors.int3 \
	transform_hlds.term_norm.int3 \
	transform_hlds.term_util.int3 \
	mdbcomp.feedback.automatic_parallelism.int3


endif # ifeq(x,y)

# RULE self_and_parent_date0_deps
# source group names:
#   (unnamed)
#   long dep int3s
#   short dep int3s
check_hlds.purity.date0 check_hlds.date0 : \
		purity.m \
	\
		assoc_list.int3 \
		builtin.int3 \
		hlds.int3 \
		io.int3 \
		list.int3 \
		map.int3 \
		maybe.int3 \
		mdbcomp.int3 \
		pair.int3 \
		parse_tree.int3 \
		private_builtin.int3 \
		require.int3 \
		string.int3 \
		term.int3 \
		varset.int3 \
		check_hlds.check_promise.int3 \
		check_hlds.resolve_unify_functor.int3 \
		hlds.from_ground_term_util.int3 \
		hlds.goal_util.int3 \
		hlds.hlds_clauses.int3 \
		hlds.hlds_error_util.int3 \
		hlds.hlds_goal.int3 \
		hlds.hlds_module.int3 \
		hlds.hlds_pred.int3 \
		hlds.hlds_rtti.int3 \
		hlds.instmap.int3 \
		hlds.passes_aux.int3 \
		hlds.pred_table.int3 \
		hlds.quantification.int3 \
		hlds.vartypes.int3 \
		mdbcomp.builtin_modules.int3 \
		mdbcomp.prim_data.int3 \
		mdbcomp.sym_name.int3 \
		parse_tree.builtin_lib_types.int3 \
		parse_tree.error_util.int3 \
		parse_tree.parse_tree_out_term.int3 \
		parse_tree.prog_data.int3 \
		parse_tree.prog_data_foreign.int3 \
		parse_tree.prog_mode.int3 \
		parse_tree.prog_out.int3 \
		parse_tree.prog_type.int3 \
		parse_tree.set_of_var.int3 \
	\
		analysis.int3 \
		array.int3 \
		bag.int3 \
		bimap.int3 \
		bitmap.int3 \
		bool.int3 \
		char.int3 \
		construct.int3 \
		cord.int3 \
		deconstruct.int3 \
		digraph.int3 \
		enum.int3 \
		getopt_io.int3 \
		integer.int3 \
		libs.int3 \
		mode_robdd.int3 \
		multi_map.int3 \
		ops.int3 \
		pretty_printer.int3 \
		queue.int3 \
		random.int3 \
		recompilation.int3 \
		robdd.int3 \
		rtti_implementation.int3 \
		set.int3 \
		set_ordlist.int3 \
		set_tree234.int3 \
		sparse_bitset.int3 \
		stack.int3 \
		stream.int3 \
		time.int3 \
		transform_hlds.int3 \
		tree234.int3 \
		type_desc.int3 \
		unit.int3 \
		univ.int3 \
		check_hlds.delay_info.int3 \
		check_hlds.mode_constraint_robdd.int3 \
		check_hlds.mode_errors.int3 \
		check_hlds.mode_info.int3 \
		check_hlds.proc_requests.int3 \
		hlds.const_struct.int3 \
		hlds.hlds_args.int3 \
		hlds.hlds_data.int3 \
		hlds.hlds_dependency_graph.int3 \
		hlds.hlds_llds.int3 \
		hlds.inst_graph.int3 \
		hlds.special_pred.int3 \
		hlds.status.int3 \
		libs.compiler_util.int3 \
		libs.dependency_graph.int3 \
		libs.globals.int3 \
		libs.lp_rational.int3 \
		libs.op_mode.int3 \
		libs.options.int3 \
		libs.polyhedron.int3 \
		libs.rat.int3 \
		libs.timestamp.int3 \
		libs.trace_params.int3 \
		mdbcomp.feedback.int3 \
		mdbcomp.goal_path.int3 \
		mdbcomp.program_representation.int3 \
		mdbcomp.rtti_access.int3 \
		mdbcomp.trace_counts.int3 \
		mode_robdd.tfeirn.int3 \
		parse_tree.file_kind.int3 \
		parse_tree.maybe_error.int3 \
		parse_tree.module_qual.int3 \
		parse_tree.parse_tree_out_info.int3 \
		parse_tree.prog_data_event.int3 \
		parse_tree.prog_data_pragma.int3 \
		parse_tree.prog_data_used_modules.int3 \
		parse_tree.prog_foreign.int3 \
		parse_tree.prog_item.int3 \
		parse_tree.prog_rename.int3 \
		transform_hlds.term_constr_data.int3 \
		transform_hlds.term_constr_errors.int3 \
		transform_hlds.term_constr_main_types.int3 \
		transform_hlds.term_errors.int3 \
		transform_hlds.term_norm.int3 \
		transform_hlds.term_util.int3 \
		mdbcomp.feedback.automatic_parallelism.int3

ifeq (x,y)



check_hlds.purity.$O :  \
	io.mh \
	string.mh \
	time.mh \
	mdbcomp.rtti_access.mh



check_hlds.purity.pic_o :  \
	io.mh \
	string.mh \
	time.mh \
	mdbcomp.rtti_access.mh


endif # ifeq(x,y)

# RULE foreign_deps_for_.mh
check_hlds.purity.$O : \
		io.mh \
		string.mh \
		time.mh \
		mdbcomp.rtti_access.mh

# RULE foreign_deps_for_.mh
check_hlds.purity.pic_o : \
		io.mh \
		string.mh \
		time.mh \
		mdbcomp.rtti_access.mh

ifeq (x,y)


check_hlds.purity.int0 : check_hlds.purity.date0
	@:
check_hlds.purity.int : check_hlds.purity.date
	@:
check_hlds.purity.int2 : check_hlds.purity.date
	@:
check_hlds.purity.int3 : check_hlds.purity.date3
	@:
check_hlds.purity.opt : check_hlds.purity.optdate
	@:
check_hlds.purity.trans_opt : check_hlds.purity.trans_opt_date
	@:


endif # ifeq(x,y)

check_hlds.purity.int0 : check_hlds.purity.date0
	@:

check_hlds.purity.int : check_hlds.purity.date
	@:

check_hlds.purity.int2 : check_hlds.purity.date
	@:

check_hlds.purity.int3 : check_hlds.purity.date3
	@:

check_hlds.purity.opt : check_hlds.purity.optdate
	@:

check_hlds.purity.trans_opt : check_hlds.purity.trans_opt_date
	@:

ifeq (x,y)


check_hlds.purity.date0 : purity.m
	$(MCPI) $(ALL_GRADEFLAGS) $(ALL_MCPIFLAGS) check_hlds.purity
check_hlds.purity.date : purity.m
	$(MCI) $(ALL_GRADEFLAGS) $(ALL_MCIFLAGS) check_hlds.purity
check_hlds.purity.date3 : purity.m
	$(MCSI) $(ALL_GRADEFLAGS) $(ALL_MCSIFLAGS) check_hlds.purity
check_hlds.purity.optdate : purity.m
	$(MCOI) $(ALL_GRADEFLAGS) $(ALL_MCOIFLAGS) check_hlds.purity
check_hlds.purity.trans_opt_date : purity.m
	$(MCTOI) $(ALL_GRADEFLAGS) $(ALL_MCTOIFLAGS) check_hlds.purity
check_hlds.purity.c_date : purity.m
	$(MCG) $(ALL_GRADEFLAGS) $(ALL_MCGFLAGS) check_hlds.purity $(ERR_REDIRECT)
check_hlds.purity.java_date : purity.m
	$(MCG) $(ALL_GRADEFLAGS) $(ALL_MCGFLAGS) --java-only check_hlds.purity $(ERR_REDIRECT)


endif # ifeq(x,y)

check_hlds.purity.date0 : purity.m
	$(MCPI) $(ALL_GRADEFLAGS) $(ALL_MCPIFLAGS) check_hlds.purity

check_hlds.purity.date : purity.m
	$(MCI) $(ALL_GRADEFLAGS) $(ALL_MCIFLAGS) check_hlds.purity

check_hlds.purity.date3 : purity.m
	$(MCSI) $(ALL_GRADEFLAGS) $(ALL_MCSIFLAGS) check_hlds.purity

check_hlds.purity.optdate : purity.m
	$(MCOI) $(ALL_GRADEFLAGS) $(ALL_MCOIFLAGS) check_hlds.purity

check_hlds.purity.trans_opt_date : purity.m
	$(MCTOI) $(ALL_GRADEFLAGS) $(ALL_MCTOIFLAGS) check_hlds.purity

check_hlds.purity.c_date : purity.m
	$(MCG) $(ALL_GRADEFLAGS) $(ALL_MCGFLAGS) check_hlds.purity $(ERR_REDIRECT)

check_hlds.purity.java_date : purity.m
	$(MCG) $(ALL_GRADEFLAGS) $(ALL_MCGFLAGS) --java-only check_hlds.purity $(ERR_REDIRECT)

ifeq (x,y)


endif # ifeq(x,y)


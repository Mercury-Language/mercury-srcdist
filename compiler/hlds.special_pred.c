    ...
    if (succeeded)
    {
      ...
      if (succeeded)
      {
        ...
        if (!(succeeded))
        {
          ...
          if (succeeded)
          {
            Var_19 = hlds__status__type_status_defined_in_this_module_1_f_0(TypeStatus_8);
            succeeded = (Var_19 == (MR_Integer) 0);
            if (succeeded)
            {
              succeeded = ((MR_tag((MR_Word) TypeBody_7)) == (MR_mktag((MR_Integer) 0)));
              if (succeeded)
                Var_18 = (MR_Word) MR_body(((MR_Word) TypeBody_7), (MR_Integer) 0);
              succeeded = !(succeeded);
              if (succeeded)
              {
                Var_20 = (MR_Integer) 0;
                succeeded = hlds__special_pred__special_pred_for_type_needs_typecheck_3_p_0(ModuleInfo_5, Var_20, TypeBody_7);
                succeeded = !(succeeded);
              }
            }
          }
    ...
    if (!(succeeded))
    {
      ...
      if (succeeded)
      {
        ...
        if (succeeded)
        {
          ...
          Var_17 = hlds__status__type_status_defined_in_this_module_1_f_0(TypeStatus_11);
          succeeded = (Var_17 == (MR_Integer) 0);
          if (succeeded)
          {
            succeeded = ((MR_tag((MR_Word) TypeBody_10)) == (MR_mktag((MR_Integer) 0)));
            if (succeeded)
              Var_16 = (MR_Word) MR_body(((MR_Word) TypeBody_10), (MR_Integer) 0);
            succeeded = !(succeeded);
            if (succeeded)
            {
              Var_18 = (MR_Integer) 0;
              succeeded = hlds__special_pred__special_pred_for_type_needs_typecheck_3_p_0(ModuleInfo_3, Var_18, TypeBody_10);
              succeeded = !(succeeded);
            }
          }
        }

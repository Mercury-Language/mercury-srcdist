/*
** Automatically generated from `compiler_util.m'
** by the Mercury compiler,
** version rotd-2017-06-12
** configured for x86_64-apple-darwin13.4.0.
** Do not edit.
**
** The autoconfigured grade settings governing
** the generation of this C file were
**
** TAG_BITS=2
** UNBOXED_FLOAT=no
** PREGENERATED_DIST=yes
** HIGHLEVEL_CODE=yes
**
** END_OF_C_GRADE_INFO
*/


/* :- module libs.compiler_util. */
/* :- implementation. */

/*
INIT mercury__libs__compiler_util__init
ENDINIT
*/

#include "libs.compiler_util.mih"


#include "libs.mih"
#include "mdbcomp.mih"
#include "parse_tree.mih"
#include "recompilation.mih"
#include "libs.globals.mih"
#include "libs.op_mode.mih"
#include "libs.options.mih"
#include "libs.rat.mih"
#include "libs.timestamp.mih"
#include "libs.trace_params.mih"
#include "mdbcomp.feedback.mih"
#include "mdbcomp.goal_path.mih"
#include "mdbcomp.prim_data.mih"
#include "mdbcomp.program_representation.mih"
#include "mdbcomp.rtti_access.mih"
#include "mdbcomp.sym_name.mih"
#include "mdbcomp.trace_counts.mih"
#include "array.mih"
#include "assoc_list.mih"
#include "bimap.mih"
#include "bitmap.mih"
#include "bool.mih"
#include "builtin.mih"
#include "char.mih"
#include "construct.mih"
#include "cord.mih"
#include "deconstruct.mih"
#include "enum.mih"
#include "getopt_io.mih"
#include "integer.mih"
#include "io.mih"
#include "list.mih"
#include "map.mih"
#include "maybe.mih"
#include "ops.mih"
#include "pair.mih"
#include "pretty_printer.mih"
#include "private_builtin.mih"
#include "random.mih"
#include "rtti_implementation.mih"
#include "set.mih"
#include "set_ordlist.mih"
#include "sparse_bitset.mih"
#include "stream.mih"
#include "string.mih"
#include "term.mih"
#include "time.mih"
#include "tree234.mih"
#include "type_desc.mih"
#include "unit.mih"
#include "univ.mih"
#include "varset.mih"
#include "parse_tree.error_util.mih"
#include "parse_tree.file_kind.mih"
#include "parse_tree.maybe_error.mih"
#include "parse_tree.prog_data.mih"
#include "parse_tree.prog_data_foreign.mih"
#include "parse_tree.prog_data_pragma.mih"
#include "parse_tree.prog_item.mih"
#include "parse_tree.set_of_var.mih"
#include "mdbcomp.feedback.automatic_parallelism.mih"




static const MR_EnumFunctorDesc libs__compiler_util__libs__compiler_util__enum_functor_desc_warning_or_error_0_0;

static const MR_EnumFunctorDesc libs__compiler_util__libs__compiler_util__enum_functor_desc_warning_or_error_0_1;

static const MR_EnumFunctorDescPtr libs__compiler_util__libs__compiler_util__enum_value_ordered_warning_or_error_0[2];

static const MR_EnumFunctorDescPtr libs__compiler_util__libs__compiler_util__enum_name_ordered_warning_or_error_0[2];

static const MR_Integer libs__compiler_util__libs__compiler_util__functor_number_map_warning_or_error_0[2];

static MR_bool MR_CALL 
libs__compiler_util____Unify____warning_or_error_0_0_10001(
  MR_Box libs__compiler_util__wrapper_arg_1,
  MR_Box libs__compiler_util__wrapper_arg_2);

static void MR_CALL 
libs__compiler_util____Compare____warning_or_error_0_0_10001(
  MR_Box * libs__compiler_util__wrapper_arg_1,
  MR_Box libs__compiler_util__wrapper_arg_2,
  MR_Box libs__compiler_util__wrapper_arg_3);







#include "io.mh"
#include "string.mh"
#include "time.mh"
#include "mdbcomp.rtti_access.mh"



static const MR_EnumFunctorDesc libs__compiler_util__libs__compiler_util__enum_functor_desc_warning_or_error_0_0 = {
  (MR_String) "we_warning",
  (MR_Integer) 0
};

static const MR_EnumFunctorDesc libs__compiler_util__libs__compiler_util__enum_functor_desc_warning_or_error_0_1 = {
  (MR_String) "we_error",
  (MR_Integer) 1
};

static const MR_EnumFunctorDescPtr libs__compiler_util__libs__compiler_util__enum_value_ordered_warning_or_error_0[2] = {
  &libs__compiler_util__libs__compiler_util__enum_functor_desc_warning_or_error_0_0,
  &libs__compiler_util__libs__compiler_util__enum_functor_desc_warning_or_error_0_1
};

static const MR_EnumFunctorDescPtr libs__compiler_util__libs__compiler_util__enum_name_ordered_warning_or_error_0[2] = {
  &libs__compiler_util__libs__compiler_util__enum_functor_desc_warning_or_error_0_1,
  &libs__compiler_util__libs__compiler_util__enum_functor_desc_warning_or_error_0_0
};

static const MR_Integer libs__compiler_util__libs__compiler_util__functor_number_map_warning_or_error_0[2] = {
  (MR_Integer) 1,
  (MR_Integer) 0
};

const MR_TypeCtorInfo_Struct libs__compiler_util__libs__compiler_util__type_ctor_info_warning_or_error_0 = {
  (MR_Integer) 0,
  (MR_Integer) 17,
  (MR_Integer) -1,
  mercury__private_builtin__MR_TYPECTOR_REP_ENUM,
  ((MR_Box) (libs__compiler_util____Unify____warning_or_error_0_0_10001)),
  ((MR_Box) (libs__compiler_util____Compare____warning_or_error_0_0_10001)),
  (MR_String) "libs.compiler_util",
  (MR_String) "warning_or_error",
  {     libs__compiler_util__libs__compiler_util__enum_name_ordered_warning_or_error_0 },
  {     libs__compiler_util__libs__compiler_util__enum_value_ordered_warning_or_error_0 },
  (MR_Integer) 2,
  (MR_Integer) 4,
  libs__compiler_util__libs__compiler_util__functor_number_map_warning_or_error_0
};

static MR_bool MR_CALL 
libs__compiler_util____Unify____warning_or_error_0_0_10001(
  MR_Box libs__compiler_util__wrapper_arg_1,
  MR_Box libs__compiler_util__wrapper_arg_2)
{
  {
    MR_bool libs__compiler_util__succeeded;

    {
      libs__compiler_util__succeeded = libs__compiler_util____Unify____warning_or_error_0_0(((MR_Word) libs__compiler_util__wrapper_arg_1), ((MR_Word) libs__compiler_util__wrapper_arg_2));
    }
    return libs__compiler_util__succeeded;
  }
}

static void MR_CALL 
libs__compiler_util____Compare____warning_or_error_0_0_10001(
  MR_Box * libs__compiler_util__wrapper_arg_1,
  MR_Box libs__compiler_util__wrapper_arg_2,
  MR_Box libs__compiler_util__wrapper_arg_3)
{
  {
    MR_Word libs__compiler_util__conv0_HeadVar__1_1;

    {
      libs__compiler_util____Compare____warning_or_error_0_0(&libs__compiler_util__conv0_HeadVar__1_1, ((MR_Word) libs__compiler_util__wrapper_arg_2), ((MR_Word) libs__compiler_util__wrapper_arg_3));
    }
    *libs__compiler_util__wrapper_arg_1 = ((MR_Box) (libs__compiler_util__conv0_HeadVar__1_1));
  }
}

void MR_CALL 
libs__compiler_util____Compare____warning_or_error_0_0(
  MR_Word * libs__compiler_util__HeadVar__1_1,
  MR_Word libs__compiler_util__HeadVar__2_2,
  MR_Word libs__compiler_util__HeadVar__3_3)
{
  {
    MR_bool libs__compiler_util__succeeded;
    MR_Integer libs__compiler_util__Cast_HeadVar1_4 = (MR_Integer) libs__compiler_util__HeadVar__2_2;
    MR_Integer libs__compiler_util__Cast_HeadVar2_5 = (MR_Integer) libs__compiler_util__HeadVar__3_3;

    {
      mercury__private_builtin__builtin_compare_int_3_p_0(libs__compiler_util__HeadVar__1_1, libs__compiler_util__Cast_HeadVar1_4, libs__compiler_util__Cast_HeadVar2_5);
    }
  }
}

MR_bool MR_CALL 
libs__compiler_util____Unify____warning_or_error_0_0(
  MR_Word libs__compiler_util__HeadVar__2_1,
  MR_Word libs__compiler_util__HeadVar__2_2)
{
  {
    MR_bool libs__compiler_util__succeeded = (libs__compiler_util__HeadVar__2_1 == libs__compiler_util__HeadVar__2_2);

    return libs__compiler_util__succeeded;
  }
}

void MR_CALL 
libs__compiler_util__report_warning_to_stream_5_p_0(
  MR_Word libs__compiler_util__Globals_6,
  MR_Word libs__compiler_util__Stream_7,
  MR_String libs__compiler_util__Message_8)
{
  {
    MR_bool libs__compiler_util__succeeded;
    MR_Word libs__compiler_util__HaltAtWarn_18;

    {
      libs__globals__lookup_bool_option_3_p_0(libs__compiler_util__Globals_6, (MR_Integer) 3, &libs__compiler_util__HaltAtWarn_18);
    }
    switch (libs__compiler_util__HaltAtWarn_18) {
      default: /*NOTREACHED*/ MR_assert(0);
      case (MR_Integer) 0:
        {
        }
        break;
      case (MR_Integer) 1:
        {
          {
            mercury__io__set_exit_status_3_p_0((MR_Integer) 1);
          }
        }
        break;
    }
    {
      mercury__io__write_string_4_p_0(libs__compiler_util__Stream_7, libs__compiler_util__Message_8);
    }
  }
}

void MR_CALL 
libs__compiler_util__report_warning_4_p_0(
  MR_Word libs__compiler_util__Globals_5,
  MR_String libs__compiler_util__Message_6)
{
  {
    MR_bool libs__compiler_util__succeeded;
    MR_Word libs__compiler_util__HaltAtWarn_16;

    {
      libs__globals__lookup_bool_option_3_p_0(libs__compiler_util__Globals_5, (MR_Integer) 3, &libs__compiler_util__HaltAtWarn_16);
    }
    switch (libs__compiler_util__HaltAtWarn_16) {
      default: /*NOTREACHED*/ MR_assert(0);
      case (MR_Integer) 0:
        {
        }
        break;
      case (MR_Integer) 1:
        {
          {
            mercury__io__set_exit_status_3_p_0((MR_Integer) 1);
          }
        }
        break;
    }
    {
      mercury__io__write_string_3_p_0(libs__compiler_util__Message_6);
    }
  }
}

void MR_CALL 
libs__compiler_util__record_warning_3_p_0(
  MR_Word libs__compiler_util__Globals_4)
{
  {
    MR_bool libs__compiler_util__succeeded;
    MR_Word libs__compiler_util__HaltAtWarn_6;

    {
      libs__globals__lookup_bool_option_3_p_0(libs__compiler_util__Globals_4, (MR_Integer) 3, &libs__compiler_util__HaltAtWarn_6);
    }
    switch (libs__compiler_util__HaltAtWarn_6) {
      default: /*NOTREACHED*/ MR_assert(0);
      case (MR_Integer) 0:
        {
        }
        break;
      case (MR_Integer) 1:
        {
          {
            mercury__io__set_exit_status_3_p_0((MR_Integer) 1);
          }
        }
        break;
    }
  }
}

void MR_CALL 
libs__compiler_util__add_warning_4_p_0(
  MR_Word libs__compiler_util__Phase_5,
  MR_Word libs__compiler_util__Pieces_6,
  MR_Word libs__compiler_util__STATE_VARIABLE_Specs_0_10,
  MR_Word * libs__compiler_util__STATE_VARIABLE_Specs_11)
{
  {
    MR_bool libs__compiler_util__succeeded;
    MR_Word libs__compiler_util__Msg_8;
    MR_Word libs__compiler_util__Spec_9;
    MR_Word libs__compiler_util__Var_15;
    MR_Word libs__compiler_util__Var_16;
    MR_Word libs__compiler_util__Var_19;

    {
      libs__compiler_util__Var_16 = (MR_Word) MR_new_object(MR_Word, ((MR_Integer) 1 * sizeof(MR_Word)), NULL, NULL);
      MR_hl_field(MR_mktag(0), libs__compiler_util__Var_16, 0) = ((MR_Box) (libs__compiler_util__Pieces_6));
    }
    {
      libs__compiler_util__Var_15 = (MR_Word) MR_mkword(MR_mktag(1), MR_new_object(MR_Word, ((MR_Integer) 2 * sizeof(MR_Word)), NULL, NULL));
      MR_hl_field(MR_mktag(1), libs__compiler_util__Var_15, 0) = ((MR_Box) (libs__compiler_util__Var_16));
      MR_hl_field(MR_mktag(1), libs__compiler_util__Var_15, 1) = ((MR_Box) (MR_mkword(MR_mktag(0), MR_mkbody((MR_Integer) 0))));
    }
    {
      libs__compiler_util__Msg_8 = (MR_Word) MR_mkword(MR_mktag(1), MR_new_object(MR_Word, ((MR_Integer) 4 * sizeof(MR_Word)), NULL, NULL));
      MR_hl_field(MR_mktag(1), libs__compiler_util__Msg_8, 0) = ((MR_Box) (MR_mkword(MR_mktag(0), MR_mkbody((MR_Integer) 0))));
      MR_hl_field(MR_mktag(1), libs__compiler_util__Msg_8, 1) = ((MR_Box) ((MR_Integer) 1));
      MR_hl_field(MR_mktag(1), libs__compiler_util__Msg_8, 2) = ((MR_Box) ((MR_Integer) 0));
      MR_hl_field(MR_mktag(1), libs__compiler_util__Msg_8, 3) = ((MR_Box) (libs__compiler_util__Var_15));
    }
    {
      libs__compiler_util__Var_19 = (MR_Word) MR_mkword(MR_mktag(1), MR_new_object(MR_Word, ((MR_Integer) 2 * sizeof(MR_Word)), NULL, NULL));
      MR_hl_field(MR_mktag(1), libs__compiler_util__Var_19, 0) = ((MR_Box) (libs__compiler_util__Msg_8));
      MR_hl_field(MR_mktag(1), libs__compiler_util__Var_19, 1) = ((MR_Box) (MR_mkword(MR_mktag(0), MR_mkbody((MR_Integer) 0))));
    }
    {
      libs__compiler_util__Spec_9 = (MR_Word) MR_new_object(MR_Word, ((MR_Integer) 3 * sizeof(MR_Word)), NULL, NULL);
      MR_hl_field(MR_mktag(0), libs__compiler_util__Spec_9, 0) = ((MR_Box) (MR_mkword(MR_mktag(0), MR_mkbody((MR_Integer) 1))));
      MR_hl_field(MR_mktag(0), libs__compiler_util__Spec_9, 1) = ((MR_Box) (libs__compiler_util__Phase_5));
      MR_hl_field(MR_mktag(0), libs__compiler_util__Spec_9, 2) = ((MR_Box) (libs__compiler_util__Var_19));
    }
    {
      MR_Word base;
      base = (MR_Word) MR_mkword(MR_mktag(1), MR_new_object(MR_Word, ((MR_Integer) 2 * sizeof(MR_Word)), NULL, NULL));
      *libs__compiler_util__STATE_VARIABLE_Specs_11 = base;
      MR_hl_field(MR_mktag(1), base, 0) = ((MR_Box) (libs__compiler_util__Spec_9));
      MR_hl_field(MR_mktag(1), base, 1) = ((MR_Box) (libs__compiler_util__STATE_VARIABLE_Specs_0_10));
    }
  }
}

void MR_CALL 
libs__compiler_util__add_error_4_p_0(
  MR_Word libs__compiler_util__Phase_5,
  MR_Word libs__compiler_util__Pieces_6,
  MR_Word libs__compiler_util__STATE_VARIABLE_Specs_0_10,
  MR_Word * libs__compiler_util__STATE_VARIABLE_Specs_11)
{
  {
    MR_bool libs__compiler_util__succeeded;
    MR_Word libs__compiler_util__Msg_8;
    MR_Word libs__compiler_util__Spec_9;
    MR_Word libs__compiler_util__Var_15;
    MR_Word libs__compiler_util__Var_16;
    MR_Word libs__compiler_util__Var_19;

    {
      libs__compiler_util__Var_16 = (MR_Word) MR_new_object(MR_Word, ((MR_Integer) 1 * sizeof(MR_Word)), NULL, NULL);
      MR_hl_field(MR_mktag(0), libs__compiler_util__Var_16, 0) = ((MR_Box) (libs__compiler_util__Pieces_6));
    }
    {
      libs__compiler_util__Var_15 = (MR_Word) MR_mkword(MR_mktag(1), MR_new_object(MR_Word, ((MR_Integer) 2 * sizeof(MR_Word)), NULL, NULL));
      MR_hl_field(MR_mktag(1), libs__compiler_util__Var_15, 0) = ((MR_Box) (libs__compiler_util__Var_16));
      MR_hl_field(MR_mktag(1), libs__compiler_util__Var_15, 1) = ((MR_Box) (MR_mkword(MR_mktag(0), MR_mkbody((MR_Integer) 0))));
    }
    {
      libs__compiler_util__Msg_8 = (MR_Word) MR_mkword(MR_mktag(1), MR_new_object(MR_Word, ((MR_Integer) 4 * sizeof(MR_Word)), NULL, NULL));
      MR_hl_field(MR_mktag(1), libs__compiler_util__Msg_8, 0) = ((MR_Box) (MR_mkword(MR_mktag(0), MR_mkbody((MR_Integer) 0))));
      MR_hl_field(MR_mktag(1), libs__compiler_util__Msg_8, 1) = ((MR_Box) ((MR_Integer) 1));
      MR_hl_field(MR_mktag(1), libs__compiler_util__Msg_8, 2) = ((MR_Box) ((MR_Integer) 0));
      MR_hl_field(MR_mktag(1), libs__compiler_util__Msg_8, 3) = ((MR_Box) (libs__compiler_util__Var_15));
    }
    {
      libs__compiler_util__Var_19 = (MR_Word) MR_mkword(MR_mktag(1), MR_new_object(MR_Word, ((MR_Integer) 2 * sizeof(MR_Word)), NULL, NULL));
      MR_hl_field(MR_mktag(1), libs__compiler_util__Var_19, 0) = ((MR_Box) (libs__compiler_util__Msg_8));
      MR_hl_field(MR_mktag(1), libs__compiler_util__Var_19, 1) = ((MR_Box) (MR_mkword(MR_mktag(0), MR_mkbody((MR_Integer) 0))));
    }
    {
      libs__compiler_util__Spec_9 = (MR_Word) MR_new_object(MR_Word, ((MR_Integer) 3 * sizeof(MR_Word)), NULL, NULL);
      MR_hl_field(MR_mktag(0), libs__compiler_util__Spec_9, 0) = ((MR_Box) (MR_mkword(MR_mktag(0), MR_mkbody((MR_Integer) 0))));
      MR_hl_field(MR_mktag(0), libs__compiler_util__Spec_9, 1) = ((MR_Box) (libs__compiler_util__Phase_5));
      MR_hl_field(MR_mktag(0), libs__compiler_util__Spec_9, 2) = ((MR_Box) (libs__compiler_util__Var_19));
    }
    {
      MR_Word base;
      base = (MR_Word) MR_mkword(MR_mktag(1), MR_new_object(MR_Word, ((MR_Integer) 2 * sizeof(MR_Word)), NULL, NULL));
      *libs__compiler_util__STATE_VARIABLE_Specs_11 = base;
      MR_hl_field(MR_mktag(1), base, 0) = ((MR_Box) (libs__compiler_util__Spec_9));
      MR_hl_field(MR_mktag(1), base, 1) = ((MR_Box) (libs__compiler_util__STATE_VARIABLE_Specs_0_10));
    }
  }
}

void MR_CALL 
libs__compiler_util__warning_or_error_severity_2_p_0(
  MR_Word libs__compiler_util__HeadVar__1_1,
  MR_Word * libs__compiler_util__HeadVar__2_2)
{
  {
    MR_bool libs__compiler_util__succeeded;

    switch (libs__compiler_util__HeadVar__1_1) {
      default: /*NOTREACHED*/ MR_assert(0);
      case (MR_Integer) 1:
        *libs__compiler_util__HeadVar__2_2 = (MR_Word) MR_mkword(MR_mktag(0), MR_mkbody((MR_Integer) 0));
        break;
      case (MR_Integer) 0:
        *libs__compiler_util__HeadVar__2_2 = (MR_Word) MR_mkword(MR_mktag(0), MR_mkbody((MR_Integer) 1));
        break;
    }
  }
}

MR_bool MR_CALL 
libs__compiler_util__warning_or_error_string_2_p_1(
  MR_Word * libs__compiler_util__HeadVar__1_1,
  MR_String libs__compiler_util__HeadVar__2_2)
{
  {
    MR_bool libs__compiler_util__succeeded;

    if ((strcmp(libs__compiler_util__HeadVar__2_2, (MR_String) "warn") == 0))
      {
        *libs__compiler_util__HeadVar__1_1 = (MR_Integer) 0;
        libs__compiler_util__succeeded = MR_TRUE;
      }
    else
    if ((strcmp(libs__compiler_util__HeadVar__2_2, (MR_String) "error") == 0))
      {
        *libs__compiler_util__HeadVar__1_1 = (MR_Integer) 1;
        libs__compiler_util__succeeded = MR_TRUE;
      }
    else
      libs__compiler_util__succeeded = MR_FALSE;
    return libs__compiler_util__succeeded;
  }
}

void MR_CALL 
libs__compiler_util__warning_or_error_string_2_p_0(
  MR_Word libs__compiler_util__HeadVar__1_1,
  MR_String * libs__compiler_util__HeadVar__2_2)
{
  {
    MR_bool libs__compiler_util__succeeded;

    switch (libs__compiler_util__HeadVar__1_1) {
      default: /*NOTREACHED*/ MR_assert(0);
      case (MR_Integer) 1:
        *libs__compiler_util__HeadVar__2_2 = (MR_String) "error";
        break;
      case (MR_Integer) 0:
        *libs__compiler_util__HeadVar__2_2 = (MR_String) "warn";
        break;
    }
  }
}

void mercury__libs__compiler_util__init(void)
{
}

void mercury__libs__compiler_util__init_type_tables(void)
{
	static MR_bool initialised = MR_FALSE;
	if (initialised) return;
	initialised = MR_TRUE;

	MR_register_type_ctor_info(&libs__compiler_util__libs__compiler_util__type_ctor_info_warning_or_error_0);
}

void mercury__libs__compiler_util__init_debugger(void)
{
	MR_fatal_error("debugger initialization in MLDS grade");
}

// Ensure everything is compiled with the same grade.
const char *mercury__libs__compiler_util__grade_check(void)
{
    return &MR_GRADE_VAR;
}

/* :- end_module libs.compiler_util. */

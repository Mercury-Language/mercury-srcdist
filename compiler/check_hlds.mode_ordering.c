/*
** Automatically generated from `mode_ordering.m'
** by the Mercury compiler,
** version 11.07-beta-2011-12-16, configured for x86_64-unknown-linux-gnu.
** Do not edit.
**
** The autoconfigured grade settings governing
** the generation of this C file were
**
** TAG_BITS=2
** UNBOXED_FLOAT=no
** HIGHLEVEL_CODE=no
**
** END_OF_C_GRADE_INFO
*/

/*
INIT mercury__check_hlds__mode_ordering__init
ENDINIT
*/

#define MR_ALLOW_RESET
#include "mercury_imp.h"
#line 526 "../library/io.int"
#include "io.mh"

#line 28 "check_hlds.mode_ordering.c"
#line 536 "../library/io.int"
#include "time.mh"

#line 32 "check_hlds.mode_ordering.c"
#line 537 "../library/io.int"
#include "string.mh"

#line 36 "check_hlds.mode_ordering.c"
#line 31 "../library/bitmap.int2"
#include "bitmap.mh"

#line 40 "check_hlds.mode_ordering.c"
#line 33 "../library/array.int2"
#include "array.mh"

#line 44 "check_hlds.mode_ordering.c"
#line 39 "../mdbcomp/mdbcomp.rtti_access.int2"
#include "mdbcomp.rtti_access.mh"

#line 48 "check_hlds.mode_ordering.c"
#line 49 "check_hlds.mode_ordering.c"
#include "check_hlds.mode_ordering.mh"

#line 52 "check_hlds.mode_ordering.c"
#line 53 "check_hlds.mode_ordering.c"
#ifndef CHECK_HLDS__MODE_ORDERING_DECL_GUARD
#define CHECK_HLDS__MODE_ORDERING_DECL_GUARD

#line 57 "check_hlds.mode_ordering.c"
#line 58 "check_hlds.mode_ordering.c"

#endif
#line 61 "check_hlds.mode_ordering.c"

#ifdef _MSC_VER
#define MR_STATIC_LINKAGE extern
#else
#define MR_STATIC_LINKAGE static
#endif

struct mercury_type_0 {
	MR_Word * f1[3];
};
MR_STATIC_LINKAGE const struct mercury_type_0 mercury_common_0[];

struct mercury_type_1 {
	MR_Word * f1[2];
};
MR_STATIC_LINKAGE const struct mercury_type_1 mercury_common_1[];

struct mercury_type_2 {
	MR_Word * f1[2];
	MR_Integer f2;
	MR_Word * f3[4];
};
MR_STATIC_LINKAGE const struct mercury_type_2 mercury_common_2[];

struct mercury_type_3 {
	MR_Word * f1[2];
	MR_Integer f2;
	MR_Word * f3[5];
};
MR_STATIC_LINKAGE const struct mercury_type_3 mercury_common_3[];

struct mercury_type_4 {
	MR_Word * f1[2];
	MR_Integer f2;
	MR_Word * f3[3];
};
MR_STATIC_LINKAGE const struct mercury_type_4 mercury_common_4[];

struct mercury_type_5 {
	MR_Word * f1;
	MR_Code * f2;
	MR_Integer f3;
};
MR_STATIC_LINKAGE const struct mercury_type_5 mercury_common_5[];

struct mercury_type_6 {
	MR_Word * f1[2];
	MR_Integer f2;
	MR_Word * f3[6];
};
MR_STATIC_LINKAGE const struct mercury_type_6 mercury_common_6[];

struct mercury_type_7 {
	MR_Word * f1[2];
	MR_Integer f2;
	MR_Word * f3[7];
};
MR_STATIC_LINKAGE const struct mercury_type_7 mercury_common_7[];

struct mercury_type_8 {
	MR_Word * f1[2];
	MR_Integer f2;
	MR_Word * f3[8];
};
MR_STATIC_LINKAGE const struct mercury_type_8 mercury_common_8[];

extern const MR_TypeCtorInfo_Struct
	mercury_data_check_hlds__mode_ordering__type_ctor_info_mode_ordering_info_0,
	mercury_data_check_hlds__mode_ordering__type_ctor_info_pred_constraint_info_0,
	mercury_data_check_hlds__mode_ordering__type_ctor_info_pred_constraint_map_0;
MR_decl_label3(check_hlds__mode_ordering__IntroducedFrom__pred__find_matching_proc_2__569__1_7_0, 1,3,4)
MR_decl_label4(check_hlds__mode_ordering__IntroducedFrom__pred__mode_order_goal_2__251__1_5_0, 1,4,2,7)
MR_decl_label4(f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_105_110_102_101_114_95_112_114_111_99_95_95_91_54_93_95_48_8_0, 4,2,7,8)
MR_decl_label9(f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_112_114_101_100_95_95_91_50_93_95_48_5_0, 4,5,6,9,12,11,8,17,19)
MR_decl_label10(check_hlds__mode_ordering__combine_mode_vars_sets_3_0, 2,3,4,5,7,8,9,10,11,12)
MR_decl_label5(check_hlds__mode_ordering__combine_mode_vars_sets_3_0, 13,14,15,16,17)
MR_decl_label1(check_hlds__mode_ordering__enter_lambda_goal_3_0, 2)
MR_decl_label10(check_hlds__mode_ordering__find_matching_proc_7_0, 2,3,4,5,6,7,10,9,13,12)
MR_decl_label10(check_hlds__mode_ordering__find_matching_proc_2_8_0, 81,3,4,5,12,16,20,21,25,26)
MR_decl_label10(check_hlds__mode_ordering__find_matching_proc_2_8_0, 14,29,30,34,35,39,13,10,44,7)
MR_decl_label1(check_hlds__mode_ordering__find_matching_proc_2_8_0, 1)
MR_decl_label7(check_hlds__mode_ordering__goal_info_copy_mode_var_sets_3_0, 2,3,4,5,6,7,8)
MR_decl_label1(check_hlds__mode_ordering__leave_lambda_goal_2_0, 2)
MR_decl_label10(check_hlds__mode_ordering__mode_order_conj_3_0, 4,5,8,10,11,13,16,17,19,18)
MR_decl_label9(check_hlds__mode_ordering__mode_order_disj_3_0, 24,4,5,6,7,8,9,10,11)
MR_decl_label10(check_hlds__mode_ordering__mode_order_disj_2_3_0, 2,3,4,5,7,8,9,10,11,12)
MR_decl_label5(check_hlds__mode_ordering__mode_order_disj_2_3_0, 13,14,15,16,17)
MR_decl_label1(check_hlds__mode_ordering__mode_order_goal_4_0, 2)
MR_decl_label10(check_hlds__mode_ordering__mode_order_goal_2_6_0, 7,8,5,11,12,13,14,16,17,18)
MR_decl_label10(check_hlds__mode_ordering__mode_order_goal_2_6_0, 19,20,21,22,3,26,27,24,30,31)
MR_decl_label10(check_hlds__mode_ordering__mode_order_goal_2_6_0, 32,35,36,38,39,40,41,42,43,44)
MR_decl_label10(check_hlds__mode_ordering__mode_order_goal_2_6_0, 45,29,48,49,47,52,54,55,56,57)
MR_decl_label10(check_hlds__mode_ordering__mode_order_goal_2_6_0, 58,59,60,61,51,64,65,63,68,73)
MR_decl_label10(check_hlds__mode_ordering__mode_order_goal_2_6_0, 75,77,71,80,81,82,70,85,86,87)
MR_decl_label10(check_hlds__mode_ordering__mode_order_goal_2_6_0, 89,92,93,94,84,97,100,101,95,105)
MR_decl_label10(check_hlds__mode_ordering__mode_order_goal_2_6_0, 108,109,103,113,116,120,121,122,123,67)
MR_decl_label3(check_hlds__mode_ordering__mode_order_goal_2_6_0, 125,127,129)
MR_decl_label10(check_hlds__mode_ordering__mode_ordering_check_proc_8_0, 2,3,4,5,6,7,8,9,11,12)
MR_decl_label1(check_hlds__mode_ordering__mode_ordering_check_proc_8_0, 14)
MR_decl_label1(check_hlds__mode_ordering__mode_ordering_scc_4_0, 2)
MR_decl_label7(check_hlds__mode_ordering__set_atomic_prod_vars_5_0, 2,4,8,3,11,12,13)
MR_decl_label10(check_hlds__mode_ordering__union_mode_vars_set_3_0, 2,3,4,5,7,8,9,10,11,12)
MR_decl_label5(check_hlds__mode_ordering__union_mode_vars_set_3_0, 13,14,15,16,17)
MR_decl_label5(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__423__1_3_0, 2,3,5,4,9)
MR_decl_label2(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__438__1_3_0, 5,6)
MR_decl_label2(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__444__1_3_0, 5,6)
MR_decl_label6(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__450__1_5_0, 2,6,7,8,10,11)
MR_decl_label3(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__457__1_4_0, 4,6,2)
MR_decl_label3(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__465__1_4_0, 4,6,2)
MR_decl_label7(__Unify___check_hlds__mode_ordering__mode_ordering_info_0_0, 5,9,11,13,15,19,1)
MR_decl_label3(__Unify___check_hlds__mode_ordering__pred_constraint_info_0_0, 4,8,1)
MR_decl_label8(__Compare___check_hlds__mode_ordering__mode_ordering_info_0_0, 3,2,6,13,17,21,25,75)
MR_decl_label4(__Compare___check_hlds__mode_ordering__pred_constraint_info_0_0, 3,2,5,21)
MR_def_extern_entry(check_hlds__mode_ordering__mode_ordering_6_0)
MR_decl_static(check_hlds__mode_ordering__mode_ordering_scc_4_0)
MR_decl_static(check_hlds__mode_ordering__mode_ordering_pred_5_0)
MR_decl_static(check_hlds__mode_ordering__enter_lambda_goal_3_0)
MR_decl_static(check_hlds__mode_ordering__leave_lambda_goal_2_0)
MR_decl_static(check_hlds__mode_ordering__goal_info_copy_mode_var_sets_3_0)
MR_decl_static(check_hlds__mode_ordering__mode_order_disj_3_0)
MR_decl_static(check_hlds__mode_ordering__combine_mode_vars_sets_3_0)
MR_decl_static(check_hlds__mode_ordering__union_mode_vars_sets_3_0)
MR_decl_static(check_hlds__mode_ordering__mode_order_conj_3_0)
MR_decl_static(check_hlds__mode_ordering__set_atomic_prod_vars_5_0)
MR_decl_static(check_hlds__mode_ordering__find_matching_proc_2_8_0)
MR_decl_static(check_hlds__mode_ordering__find_matching_proc_7_0)
MR_decl_static(check_hlds__mode_ordering__mode_order_goal_4_0)
MR_decl_static(check_hlds__mode_ordering__mode_order_goal_2_6_0)
MR_decl_static(check_hlds__mode_ordering__mode_ordering_check_proc_8_0)
MR_decl_static(check_hlds__mode_ordering__mode_ordering_infer_proc_8_0)
MR_decl_static(check_hlds__mode_ordering__mode_order_disj_2_3_0)
MR_decl_static(check_hlds__mode_ordering__union_mode_vars_set_3_0)
MR_decl_static(__Unify___check_hlds__mode_ordering__mode_ordering_info_0_0)
MR_decl_static(__Compare___check_hlds__mode_ordering__mode_ordering_info_0_0)
MR_def_extern_entry(__Unify___check_hlds__mode_ordering__pred_constraint_info_0_0)
MR_def_extern_entry(__Compare___check_hlds__mode_ordering__pred_constraint_info_0_0)
MR_def_extern_entry(__Unify___check_hlds__mode_ordering__pred_constraint_map_0_0)
MR_def_extern_entry(__Compare___check_hlds__mode_ordering__pred_constraint_map_0_0)
MR_decl_static(check_hlds__mode_ordering__IntroducedFrom__pred__mode_order_goal_2__251__1_5_0)
MR_decl_static(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__423__1_3_0)
MR_decl_static(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__439__1_3_0)
MR_decl_static(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__438__1_3_0)
MR_decl_static(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__445__1_3_0)
MR_decl_static(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__444__1_3_0)
MR_decl_static(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__457__1_4_0)
MR_decl_static(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__465__1_4_0)
MR_decl_static(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__450__1_5_0)
MR_decl_static(check_hlds__mode_ordering__IntroducedFrom__pred__find_matching_proc_2__569__1_7_0)
MR_decl_static(f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_112_114_101_100_95_95_91_50_93_95_48_5_0)
MR_decl_static(f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_105_110_102_101_114_95_112_114_111_99_95_95_91_54_93_95_48_8_0)

extern const MR_TypeCtorInfo_Struct mercury_data_tree234__type_ctor_info_tree234_2;
extern const MR_TypeCtorInfo_Struct mercury_data_builtin__type_ctor_info_int_0;
extern const MR_TypeCtorInfo_Struct mercury_data_check_hlds__mode_ordering__type_ctor_info_pred_constraint_info_0;
extern const MR_TypeCtorInfo_Struct mercury_data_builtin__type_ctor_info_int_0;
extern const MR_TypeCtorInfo_Struct mercury_data_hlds__hlds_goal__type_ctor_info_hlds_goal_0;
extern const MR_TypeCtorInfo_Struct mercury_data_mdbcomp__goal_path__type_ctor_info_goal_id_0;
extern const MR_TypeCtorInfo_Struct mercury_data_mdbcomp__goal_path__type_ctor_info_forward_goal_path_0;
extern const MR_TypeCtorInfo_Struct mercury_data_hlds__hlds_goal__type_ctor_info_hlds_goal_0;
extern const MR_TypeCtorInfo_Struct mercury_data_hlds__inst_graph__type_ctor_info_node_0;
static const struct mercury_type_0 mercury_common_0[7] =
{
{
{
MR_CTOR_ADDR(tree234, tree234, 2),
MR_INT_CTOR_ADDR,
MR_CTOR0_ADDR(check_hlds__mode_ordering, pred_constraint_info)
}
},
{
{
MR_CTOR_ADDR(tree234, tree234, 2),
MR_INT_CTOR_ADDR,
MR_CTOR0_ADDR(hlds__hlds_goal, hlds_goal)
}
},
{
{
MR_CTOR_ADDR(tree234, tree234, 2),
MR_CTOR0_ADDR(mdbcomp__goal_path, goal_id),
MR_CTOR0_ADDR(mdbcomp__goal_path, forward_goal_path)
}
},
{
{
MR_CTOR_ADDR(tree234, tree234, 2),
MR_INT_CTOR_ADDR,
MR_CTOR0_ADDR(hlds__hlds_goal, hlds_goal)
}
},
{
{
MR_CTOR_ADDR(tree234, tree234, 2),
MR_TAG_COMMON(0,1,2),
MR_INT_CTOR_ADDR
}
},
{
{
MR_CTOR_ADDR(tree234, tree234, 2),
MR_COMMON(1,3),
MR_INT_CTOR_ADDR
}
},
{
{
MR_CTOR_ADDR(tree234, tree234, 2),
MR_COMMON(1,3),
MR_CTOR0_ADDR(hlds__inst_graph, node)
}
},
};

extern const MR_TypeCtorInfo_Struct mercury_data_list__type_ctor_info_list_1;
extern const MR_TypeCtorInfo_Struct mercury_data_hlds__hlds_pred__type_ctor_info_pred_id_0;
extern const MR_TypeCtorInfo_Struct mercury_data_term__type_ctor_info_var_1;
extern const MR_TypeCtorInfo_Struct mercury_data_parse_tree__prog_data__type_ctor_info_prog_var_type_0;
extern const MR_TypeCtorInfo_Struct mercury_data_term__type_ctor_info_var_1;
extern const MR_TypeCtorInfo_Struct mercury_data_parse_tree__prog_data__type_ctor_info_prog_var_type_0;
extern const MR_TypeCtorInfo_Struct mercury_data_digraph__type_ctor_info_digraph_1;
extern const MR_TypeCtorInfo_Struct mercury_data_stack__type_ctor_info_stack_1;
extern const MR_TypeCtorInfo_Struct mercury_data_mdbcomp__goal_path__type_ctor_info_goal_id_0;
extern const MR_TypeCtorInfo_Struct mercury_data_set_ordlist__type_ctor_info_set_ordlist_1;
extern const MR_TypeCtorInfo_Struct mercury_data_mode_robdd__tfeirn__type_ctor_info_tfeirn_1;
extern const MR_TypeCtorInfo_Struct mercury_data_check_hlds__mode_constraint_robdd__type_ctor_info_mc_type_0;
extern const MR_TypeCtorInfo_Struct mercury_data_digraph__type_ctor_info_digraph_key_1;
extern const MR_TypeCtorInfo_Struct mercury_data_check_hlds__mode_constraint_robdd__type_ctor_info_mc_type_0;
static const struct mercury_type_1 mercury_common_1[15] =
{
{
{
MR_LIST_CTOR_ADDR,
MR_INT_CTOR_ADDR
}
},
{
{
MR_LIST_CTOR_ADDR,
MR_CTOR0_ADDR(hlds__hlds_pred, pred_id)
}
},
{
{
MR_CTOR1_ADDR(term, var),
MR_CTOR0_ADDR(parse_tree__prog_data, prog_var_type)
}
},
{
{
MR_CTOR1_ADDR(term, var),
MR_CTOR0_ADDR(parse_tree__prog_data, prog_var_type)
}
},
{
{
MR_CTOR1_ADDR(digraph, digraph),
MR_INT_CTOR_ADDR
}
},
{
{
MR_CTOR1_ADDR(digraph, digraph),
MR_INT_CTOR_ADDR
}
},
{
{
MR_CTOR1_ADDR(stack, stack),
MR_CTOR0_ADDR(mdbcomp__goal_path, goal_id)
}
},
{
{
MR_CTOR1_ADDR(set_ordlist, set_ordlist),
MR_TAG_COMMON(0,1,2)
}
},
{
{
MR_LIST_CTOR_ADDR,
MR_COMMON(1,3)
}
},
{
{
MR_CTOR1_ADDR(mode_robdd__tfeirn, tfeirn),
MR_CTOR0_ADDR(check_hlds__mode_constraint_robdd, mc_type)
}
},
{
{
MR_CTOR1_ADDR(term, var),
MR_CTOR0_ADDR(parse_tree__prog_data, prog_var_type)
}
},
{
{
MR_CTOR1_ADDR(set_ordlist, set_ordlist),
MR_COMMON(1,3)
}
},
{
{
MR_CTOR1_ADDR(digraph, digraph_key),
MR_INT_CTOR_ADDR
}
},
{
{
MR_CTOR1_ADDR(mode_robdd__tfeirn, tfeirn),
MR_CTOR0_ADDR(check_hlds__mode_constraint_robdd, mc_type)
}
},
{
{
MR_LIST_CTOR_ADDR,
MR_TAG_COMMON(0,1,13)
}
},
};

static const MR_UserClosureId
mercury_data__closure_layout__check_hlds__mode_ordering__mode_ordering_6_0_1;
extern const MR_TypeCtorInfo_Struct mercury_data_hlds__hlds_module__type_ctor_info_module_info_0;
static const MR_UserClosureId
mercury_data__closure_layout__check_hlds__mode_ordering__mode_order_conj_3_0_1;
extern const MR_TypeCtorInfo_Struct mercury_data_hlds__hlds_goal__type_ctor_info_hlds_goal_0;
static const MR_UserClosureId
mercury_data__closure_layout__check_hlds__mode_ordering__mode_order_conj_3_0_2;
extern const MR_TypeCtorInfo_Struct mercury_data_builtin__type_ctor_info_int_0;
static const MR_UserClosureId
mercury_data__closure_layout__check_hlds__mode_ordering__mode_order_conj_3_0_3;
static const MR_UserClosureId
mercury_data__closure_layout__check_hlds__mode_ordering__mode_order_goal_2_6_0_1;
extern const MR_TypeCtorInfo_Struct mercury_data_check_hlds__mode_ordering__type_ctor_info_mode_ordering_info_0;
static const MR_UserClosureId
mercury_data__closure_layout__check_hlds__mode_ordering__mode_order_goal_2_6_0_2;
static const MR_UserClosureId
mercury_data__closure_layout__check_hlds__mode_ordering__mode_order_goal_2_6_0_3;
static const MR_UserClosureId
mercury_data__closure_layout__fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__438__1_3_0_1;
static const MR_UserClosureId
mercury_data__closure_layout__fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__444__1_3_0_1;
static const struct mercury_type_2 mercury_common_2[9] =
{
{
{
(MR_Word *) &mercury_data__closure_layout__check_hlds__mode_ordering__mode_ordering_6_0_1,
(MR_Word *) (MR_Integer) 0
},
4,
{
MR_COMMON(0,0),
MR_COMMON(1,0),
MR_CTOR0_ADDR(hlds__hlds_module, module_info),
MR_CTOR0_ADDR(hlds__hlds_module, module_info)
}
},
{
{
(MR_Word *) &mercury_data__closure_layout__check_hlds__mode_ordering__mode_order_conj_3_0_1,
(MR_Word *) (MR_Integer) 0
},
4,
{
MR_COMMON(0,2),
MR_CTOR0_ADDR(hlds__hlds_goal, hlds_goal),
MR_COMMON(0,3),
MR_COMMON(0,3)
}
},
{
{
(MR_Word *) &mercury_data__closure_layout__check_hlds__mode_ordering__mode_order_conj_3_0_2,
(MR_Word *) (MR_Integer) 0
},
4,
{
MR_INT_CTOR_ADDR,
MR_CTOR0_ADDR(hlds__hlds_goal, hlds_goal),
MR_COMMON(0,5),
MR_COMMON(0,5)
}
},
{
{
(MR_Word *) &mercury_data__closure_layout__check_hlds__mode_ordering__mode_order_conj_3_0_3,
(MR_Word *) (MR_Integer) 0
},
4,
{
MR_INT_CTOR_ADDR,
MR_CTOR0_ADDR(hlds__hlds_goal, hlds_goal),
MR_COMMON(0,5),
MR_COMMON(0,5)
}
},
{
{
(MR_Word *) &mercury_data__closure_layout__check_hlds__mode_ordering__mode_order_goal_2_6_0_1,
(MR_Word *) (MR_Integer) 0
},
4,
{
MR_CTOR0_ADDR(hlds__hlds_goal, hlds_goal),
MR_CTOR0_ADDR(hlds__hlds_goal, hlds_goal),
MR_CTOR0_ADDR(check_hlds__mode_ordering, mode_ordering_info),
MR_CTOR0_ADDR(check_hlds__mode_ordering, mode_ordering_info)
}
},
{
{
(MR_Word *) &mercury_data__closure_layout__check_hlds__mode_ordering__mode_order_goal_2_6_0_2,
(MR_Word *) (MR_Integer) 0
},
4,
{
MR_CTOR0_ADDR(hlds__hlds_goal, hlds_goal),
MR_CTOR0_ADDR(hlds__hlds_goal, hlds_goal),
MR_CTOR0_ADDR(check_hlds__mode_ordering, mode_ordering_info),
MR_CTOR0_ADDR(check_hlds__mode_ordering, mode_ordering_info)
}
},
{
{
(MR_Word *) &mercury_data__closure_layout__check_hlds__mode_ordering__mode_order_goal_2_6_0_3,
(MR_Word *) (MR_Integer) 0
},
4,
{
MR_CTOR0_ADDR(hlds__hlds_goal, hlds_goal),
MR_CTOR0_ADDR(hlds__hlds_goal, hlds_goal),
MR_CTOR0_ADDR(check_hlds__mode_ordering, mode_ordering_info),
MR_CTOR0_ADDR(check_hlds__mode_ordering, mode_ordering_info)
}
},
{
{
(MR_Word *) &mercury_data__closure_layout__fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__438__1_3_0_1,
(MR_Word *) (MR_Integer) 0
},
4,
{
MR_INT_CTOR_ADDR,
MR_COMMON(1,10),
MR_COMMON(0,5),
MR_COMMON(0,5)
}
},
{
{
(MR_Word *) &mercury_data__closure_layout__fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__444__1_3_0_1,
(MR_Word *) (MR_Integer) 0
},
4,
{
MR_INT_CTOR_ADDR,
MR_COMMON(1,10),
MR_COMMON(0,5),
MR_COMMON(0,5)
}
},
};

static const MR_UserClosureId
mercury_data__closure_layout__check_hlds__mode_ordering__mode_ordering_scc_4_0_1;
static const MR_UserClosureId
mercury_data__closure_layout__check_hlds__mode_ordering__mode_order_goal_2_6_0_5;
static const MR_UserClosureId
mercury_data__closure_layout__check_hlds__mode_ordering__mode_order_goal_2_6_0_6;
static const MR_UserClosureId
mercury_data__closure_layout__check_hlds__mode_ordering__mode_order_goal_2_6_0_7;
static const MR_UserClosureId
mercury_data__closure_layout__fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__450__1_5_0_1;
static const MR_UserClosureId
mercury_data__closure_layout__fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__450__1_5_0_2;
static const struct mercury_type_3 mercury_common_3[6] =
{
{
{
(MR_Word *) &mercury_data__closure_layout__check_hlds__mode_ordering__mode_ordering_scc_4_0_1,
(MR_Word *) (MR_Integer) 0
},
5,
{
MR_COMMON(0,0),
MR_COMMON(1,0),
MR_INT_CTOR_ADDR,
MR_CTOR0_ADDR(hlds__hlds_module, module_info),
MR_CTOR0_ADDR(hlds__hlds_module, module_info)
}
},
{
{
(MR_Word *) &mercury_data__closure_layout__check_hlds__mode_ordering__mode_order_goal_2_6_0_5,
(MR_Word *) (MR_Integer) 0
},
5,
{
MR_COMMON(1,10),
MR_COMMON(0,6),
MR_COMMON(1,10),
MR_COMMON(1,11),
MR_COMMON(1,10)
}
},
{
{
(MR_Word *) &mercury_data__closure_layout__check_hlds__mode_ordering__mode_order_goal_2_6_0_6,
(MR_Word *) (MR_Integer) 0
},
5,
{
MR_COMMON(1,10),
MR_COMMON(0,6),
MR_COMMON(1,10),
MR_COMMON(1,11),
MR_COMMON(1,10)
}
},
{
{
(MR_Word *) &mercury_data__closure_layout__check_hlds__mode_ordering__mode_order_goal_2_6_0_7,
(MR_Word *) (MR_Integer) 0
},
5,
{
MR_COMMON(1,10),
MR_COMMON(0,6),
MR_COMMON(1,10),
MR_COMMON(1,11),
MR_COMMON(1,10)
}
},
{
{
(MR_Word *) &mercury_data__closure_layout__fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__450__1_5_0_1,
(MR_Word *) (MR_Integer) 0
},
5,
{
MR_COMMON(0,5),
MR_COMMON(1,12),
MR_COMMON(1,10),
MR_COMMON(1,5),
MR_COMMON(1,5)
}
},
{
{
(MR_Word *) &mercury_data__closure_layout__fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__450__1_5_0_2,
(MR_Word *) (MR_Integer) 0
},
5,
{
MR_COMMON(0,5),
MR_COMMON(1,12),
MR_COMMON(1,10),
MR_COMMON(1,5),
MR_COMMON(1,5)
}
},
};

static const MR_UserClosureId
mercury_data__closure_layout__check_hlds__mode_ordering__mode_order_disj_3_0_1;
extern const MR_TypeCtorInfo_Struct mercury_data_hlds__hlds_goal__type_ctor_info_hlds_goal_info_0;
static const MR_UserClosureId
mercury_data__closure_layout__check_hlds__mode_ordering__union_mode_vars_sets_3_0_1;
static const MR_UserClosureId
mercury_data__closure_layout__check_hlds__mode_ordering__mode_order_goal_2_6_0_4;
static const struct mercury_type_4 mercury_common_4[3] =
{
{
{
(MR_Word *) &mercury_data__closure_layout__check_hlds__mode_ordering__mode_order_disj_3_0_1,
(MR_Word *) (MR_Integer) 0
},
3,
{
MR_CTOR0_ADDR(hlds__hlds_goal, hlds_goal),
MR_CTOR0_ADDR(hlds__hlds_goal, hlds_goal_info),
MR_CTOR0_ADDR(hlds__hlds_goal, hlds_goal_info)
}
},
{
{
(MR_Word *) &mercury_data__closure_layout__check_hlds__mode_ordering__union_mode_vars_sets_3_0_1,
(MR_Word *) (MR_Integer) 0
},
3,
{
MR_CTOR0_ADDR(hlds__hlds_goal, hlds_goal),
MR_CTOR0_ADDR(hlds__hlds_goal, hlds_goal_info),
MR_CTOR0_ADDR(hlds__hlds_goal, hlds_goal_info)
}
},
{
{
(MR_Word *) &mercury_data__closure_layout__check_hlds__mode_ordering__mode_order_goal_2_6_0_4,
(MR_Word *) (MR_Integer) 0
},
3,
{
MR_COMMON(0,6),
MR_COMMON(1,8),
MR_COMMON(1,10)
}
},
};

static const struct mercury_type_5 mercury_common_5[7] =
{
{
MR_COMMON(4,0),
MR_ENTRY_AP(check_hlds__mode_ordering__mode_order_disj_2_3_0),
0
},
{
MR_COMMON(4,1),
MR_ENTRY_AP(check_hlds__mode_ordering__union_mode_vars_set_3_0),
0
},
{
MR_COMMON(2,2),
MR_ENTRY_AP(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__438__1_3_0),
0
},
{
MR_COMMON(2,3),
MR_ENTRY_AP(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__444__1_3_0),
0
},
{
MR_COMMON(2,4),
MR_ENTRY_AP(check_hlds__mode_ordering__mode_order_goal_4_0),
0
},
{
MR_COMMON(2,5),
MR_ENTRY_AP(check_hlds__mode_ordering__mode_order_goal_4_0),
0
},
{
MR_COMMON(2,6),
MR_ENTRY_AP(check_hlds__mode_ordering__mode_order_goal_4_0),
0
},
};

static const MR_UserClosureId
mercury_data__closure_layout__check_hlds__mode_ordering__mode_order_conj_3_0_4;
static const struct mercury_type_6 mercury_common_6[1] =
{
{
{
(MR_Word *) &mercury_data__closure_layout__check_hlds__mode_ordering__mode_order_conj_3_0_4,
(MR_Word *) (MR_Integer) 0
},
6,
{
MR_COMMON(0,5),
MR_COMMON(0,5),
MR_INT_CTOR_ADDR,
MR_CTOR0_ADDR(hlds__hlds_goal, hlds_goal),
MR_COMMON(1,5),
MR_COMMON(1,5)
}
},
};

static const MR_UserClosureId
mercury_data__closure_layout__check_hlds__mode_ordering__find_matching_proc_2_8_0_1;
extern const MR_TypeCtorInfo_Struct mercury_data_check_hlds__mode_constraint_robdd__type_ctor_info_mode_constraint_info_0;
static const struct mercury_type_7 mercury_common_7[1] =
{
{
{
(MR_Word *) &mercury_data__closure_layout__check_hlds__mode_ordering__find_matching_proc_2_8_0_1,
(MR_Word *) (MR_Integer) 0
},
7,
{
MR_COMMON(1,8),
MR_COMMON(0,6),
MR_COMMON(0,6),
MR_CTOR0_ADDR(check_hlds__mode_constraint_robdd, mode_constraint_info),
MR_COMMON(1,8),
MR_COMMON(1,9),
MR_COMMON(1,10)
}
},
};

static const MR_UserClosureId
mercury_data__closure_layout__f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_112_114_101_100_95_95_91_50_93_95_48_5_0_1;
extern const MR_TypeCtorInfo_Struct mercury_data_hlds__hlds_pred__type_ctor_info_pred_info_0;
static const MR_UserClosureId
mercury_data__closure_layout__f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_112_114_101_100_95_95_91_50_93_95_48_5_0_2;
static const struct mercury_type_8 mercury_common_8[2] =
{
{
{
(MR_Word *) &mercury_data__closure_layout__f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_112_114_101_100_95_95_91_50_93_95_48_5_0_1,
(MR_Word *) (MR_Integer) 0
},
8,
{
MR_CTOR0_ADDR(hlds__hlds_module, module_info),
MR_COMMON(0,0),
MR_INT_CTOR_ADDR,
MR_CTOR0_ADDR(check_hlds__mode_constraint_robdd, mode_constraint_info),
MR_COMMON(1,9),
MR_COMMON(1,9),
MR_CTOR0_ADDR(hlds__hlds_pred, pred_info),
MR_CTOR0_ADDR(hlds__hlds_pred, pred_info)
}
},
{
{
(MR_Word *) &mercury_data__closure_layout__f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_112_114_101_100_95_95_91_50_93_95_48_5_0_2,
(MR_Word *) (MR_Integer) 0
},
8,
{
MR_CTOR0_ADDR(hlds__hlds_module, module_info),
MR_COMMON(0,0),
MR_INT_CTOR_ADDR,
MR_CTOR0_ADDR(check_hlds__mode_constraint_robdd, mode_constraint_info),
MR_COMMON(1,9),
MR_INT_CTOR_ADDR,
MR_CTOR0_ADDR(hlds__hlds_pred, pred_info),
MR_CTOR0_ADDR(hlds__hlds_pred, pred_info)
}
},
};

static const MR_FA_TypeInfo_Struct1 mercury_data_term__ti_var_1parse_tree__prog_data__type_ctor_info_prog_var_type_0 = {
	&mercury_data_term__type_ctor_info_var_1,
{	(MR_TypeInfo) &mercury_data_parse_tree__prog_data__type_ctor_info_prog_var_type_0
}};

static const MR_FA_TypeInfo_Struct2 mercury_data_tree234__ti_tree234_2term__ti_var_1parse_tree__prog_data__type_ctor_info_prog_var_type_0hlds__inst_graph__type_ctor_info_node_0 = {
	&mercury_data_tree234__type_ctor_info_tree234_2,
{	(MR_TypeInfo) &mercury_data_term__ti_var_1parse_tree__prog_data__type_ctor_info_prog_var_type_0,
	(MR_TypeInfo) &mercury_data_hlds__inst_graph__type_ctor_info_node_0
}};

static const MR_FA_TypeInfo_Struct1 mercury_data_stack__ti_stack_1mdbcomp__goal_path__type_ctor_info_goal_id_0 = {
	&mercury_data_stack__type_ctor_info_stack_1,
{	(MR_TypeInfo) &mercury_data_mdbcomp__goal_path__type_ctor_info_goal_id_0
}};

static const MR_FA_TypeInfo_Struct1 mercury_data_set_ordlist__ti_set_ordlist_1term__ti_var_1parse_tree__prog_data__type_ctor_info_prog_var_type_0 = {
	&mercury_data_set_ordlist__type_ctor_info_set_ordlist_1,
{	(MR_TypeInfo) &mercury_data_term__ti_var_1parse_tree__prog_data__type_ctor_info_prog_var_type_0
}};

static const MR_FA_TypeInfo_Struct2 mercury_data_tree234__ti_tree234_2stack__ti_stack_1mdbcomp__goal_path__type_ctor_info_goal_id_0set_ordlist__ti_set_ordlist_1term__ti_var_1parse_tree__prog_data__type_ctor_info_prog_var_type_0 = {
	&mercury_data_tree234__type_ctor_info_tree234_2,
{	(MR_TypeInfo) &mercury_data_stack__ti_stack_1mdbcomp__goal_path__type_ctor_info_goal_id_0,
	(MR_TypeInfo) &mercury_data_set_ordlist__ti_set_ordlist_1term__ti_var_1parse_tree__prog_data__type_ctor_info_prog_var_type_0
}};
extern const MR_TypeCtorInfo_Struct mercury_data_hlds__hlds_module__type_ctor_info_module_info_0;

static const MR_FA_TypeInfo_Struct2 mercury_data_tree234__ti_tree234_2builtin__type_ctor_info_int_0check_hlds__mode_ordering__type_ctor_info_pred_constraint_info_0 = {
	&mercury_data_tree234__type_ctor_info_tree234_2,
{	(MR_TypeInfo) &mercury_data_builtin__type_ctor_info_int_0,
	(MR_TypeInfo) &mercury_data_check_hlds__mode_ordering__type_ctor_info_pred_constraint_info_0
}};

static const MR_FA_TypeInfo_Struct2 mercury_data_tree234__ti_tree234_2mdbcomp__goal_path__type_ctor_info_goal_id_0mdbcomp__goal_path__type_ctor_info_forward_goal_path_0 = {
	&mercury_data_tree234__type_ctor_info_tree234_2,
{	(MR_TypeInfo) &mercury_data_mdbcomp__goal_path__type_ctor_info_goal_id_0,
	(MR_TypeInfo) &mercury_data_mdbcomp__goal_path__type_ctor_info_forward_goal_path_0
}};
extern const MR_TypeCtorInfo_Struct mercury_data_hlds__hlds_module__type_ctor_info_module_info_0;

const MR_PseudoTypeInfo mercury_data_check_hlds__mode_ordering__field_types_mode_ordering_info_0_0[] = {
	(MR_PseudoTypeInfo) &mercury_data_tree234__ti_tree234_2term__ti_var_1parse_tree__prog_data__type_ctor_info_prog_var_type_0hlds__inst_graph__type_ctor_info_node_0,
	(MR_PseudoTypeInfo) &mercury_data_tree234__ti_tree234_2stack__ti_stack_1mdbcomp__goal_path__type_ctor_info_goal_id_0set_ordlist__ti_set_ordlist_1term__ti_var_1parse_tree__prog_data__type_ctor_info_prog_var_type_0,
	(MR_PseudoTypeInfo) &mercury_data_stack__ti_stack_1mdbcomp__goal_path__type_ctor_info_goal_id_0,
	(MR_PseudoTypeInfo) &mercury_data_hlds__hlds_module__type_ctor_info_module_info_0,
	(MR_PseudoTypeInfo) &mercury_data_tree234__ti_tree234_2builtin__type_ctor_info_int_0check_hlds__mode_ordering__type_ctor_info_pred_constraint_info_0,
	(MR_PseudoTypeInfo) &mercury_data_tree234__ti_tree234_2mdbcomp__goal_path__type_ctor_info_goal_id_0mdbcomp__goal_path__type_ctor_info_forward_goal_path_0
};

const MR_ConstString mercury_data_check_hlds__mode_ordering__field_names_mode_ordering_info_0_0[] = {
	"moi_inst_graph",
	"moi_prodvars_map",
	"moi_lambda_nesting",
	"moi_module_info",
	"moi_pred_constraint_map",
	"moi_goal_path_map"
};

static const MR_DuFunctorDesc mercury_data_check_hlds__mode_ordering__du_functor_desc_mode_ordering_info_0_0 = {
	"mode_ordering_info",
	6,
	0,
	MR_SECTAG_NONE,
	0,
	-1,
	0,
	(MR_PseudoTypeInfo *) mercury_data_check_hlds__mode_ordering__field_types_mode_ordering_info_0_0,
	mercury_data_check_hlds__mode_ordering__field_names_mode_ordering_info_0_0,
	NULL
};

const MR_DuFunctorDescPtr mercury_data_check_hlds__mode_ordering__du_stag_ordered_mode_ordering_info_0_0[] = {
	&mercury_data_check_hlds__mode_ordering__du_functor_desc_mode_ordering_info_0_0

};

const MR_DuPtagLayout mercury_data_check_hlds__mode_ordering__du_ptag_ordered_mode_ordering_info_0[] = {
	{ 1, MR_SECTAG_NONE,
	mercury_data_check_hlds__mode_ordering__du_stag_ordered_mode_ordering_info_0_0 }

};

const MR_DuFunctorDescPtr mercury_data_check_hlds__mode_ordering__du_name_ordered_mode_ordering_info_0[] = {
	&mercury_data_check_hlds__mode_ordering__du_functor_desc_mode_ordering_info_0_0
};

const MR_Integer mercury_data_check_hlds__mode_ordering__functor_number_map_mode_ordering_info_0[] = {
	0 };
	
const MR_TypeCtorInfo_Struct mercury_data_check_hlds__mode_ordering__type_ctor_info_mode_ordering_info_0 = {
	0,
	14,
	1,
	MR_TYPECTOR_REP_DU,
	MR_MAYBE_STATIC_CODE(MR_ENTRY_AP(__Unify___check_hlds__mode_ordering__mode_ordering_info_0_0)),
	MR_MAYBE_STATIC_CODE(MR_ENTRY_AP(__Compare___check_hlds__mode_ordering__mode_ordering_info_0_0)),
	"check_hlds.mode_ordering",
	"mode_ordering_info",
	{ (void *)mercury_data_check_hlds__mode_ordering__du_name_ordered_mode_ordering_info_0 },
	{ (void *)mercury_data_check_hlds__mode_ordering__du_ptag_ordered_mode_ordering_info_0 },
	1,
	4,
	mercury_data_check_hlds__mode_ordering__functor_number_map_mode_ordering_info_0
};

static const MR_FA_TypeInfo_Struct1 mercury_data_mode_robdd__tfeirn__ti_tfeirn_1check_hlds__mode_constraint_robdd__type_ctor_info_mc_type_0 = {
	&mercury_data_mode_robdd__tfeirn__type_ctor_info_tfeirn_1,
{	(MR_TypeInfo) &mercury_data_check_hlds__mode_constraint_robdd__type_ctor_info_mc_type_0
}};
extern const MR_TypeCtorInfo_Struct mercury_data_check_hlds__mode_constraint_robdd__type_ctor_info_mode_constraint_info_0;
extern const MR_TypeCtorInfo_Struct mercury_data_check_hlds__mode_constraint_robdd__type_ctor_info_mode_constraint_info_0;

const MR_PseudoTypeInfo mercury_data_check_hlds__mode_ordering__field_types_pred_constraint_info_0_0[] = {
	(MR_PseudoTypeInfo) &mercury_data_mode_robdd__tfeirn__ti_tfeirn_1check_hlds__mode_constraint_robdd__type_ctor_info_mc_type_0,
	(MR_PseudoTypeInfo) &mercury_data_check_hlds__mode_constraint_robdd__type_ctor_info_mode_constraint_info_0
};

const MR_ConstString mercury_data_check_hlds__mode_ordering__field_names_pred_constraint_info_0_0[] = {
	"pci_mode_constraint",
	"pci_mci"
};

static const MR_DuFunctorDesc mercury_data_check_hlds__mode_ordering__du_functor_desc_pred_constraint_info_0_0 = {
	"pci",
	2,
	0,
	MR_SECTAG_NONE,
	0,
	-1,
	0,
	(MR_PseudoTypeInfo *) mercury_data_check_hlds__mode_ordering__field_types_pred_constraint_info_0_0,
	mercury_data_check_hlds__mode_ordering__field_names_pred_constraint_info_0_0,
	NULL
};

const MR_DuFunctorDescPtr mercury_data_check_hlds__mode_ordering__du_stag_ordered_pred_constraint_info_0_0[] = {
	&mercury_data_check_hlds__mode_ordering__du_functor_desc_pred_constraint_info_0_0

};

const MR_DuPtagLayout mercury_data_check_hlds__mode_ordering__du_ptag_ordered_pred_constraint_info_0[] = {
	{ 1, MR_SECTAG_NONE,
	mercury_data_check_hlds__mode_ordering__du_stag_ordered_pred_constraint_info_0_0 }

};

const MR_DuFunctorDescPtr mercury_data_check_hlds__mode_ordering__du_name_ordered_pred_constraint_info_0[] = {
	&mercury_data_check_hlds__mode_ordering__du_functor_desc_pred_constraint_info_0_0
};

const MR_Integer mercury_data_check_hlds__mode_ordering__functor_number_map_pred_constraint_info_0[] = {
	0 };
	
const MR_TypeCtorInfo_Struct mercury_data_check_hlds__mode_ordering__type_ctor_info_pred_constraint_info_0 = {
	0,
	14,
	1,
	MR_TYPECTOR_REP_DU,
	MR_MAYBE_STATIC_CODE(MR_ENTRY_AP(__Unify___check_hlds__mode_ordering__pred_constraint_info_0_0)),
	MR_MAYBE_STATIC_CODE(MR_ENTRY_AP(__Compare___check_hlds__mode_ordering__pred_constraint_info_0_0)),
	"check_hlds.mode_ordering",
	"pred_constraint_info",
	{ (void *)mercury_data_check_hlds__mode_ordering__du_name_ordered_pred_constraint_info_0 },
	{ (void *)mercury_data_check_hlds__mode_ordering__du_ptag_ordered_pred_constraint_info_0 },
	1,
	4,
	mercury_data_check_hlds__mode_ordering__functor_number_map_pred_constraint_info_0
};

const MR_TypeCtorInfo_Struct mercury_data_check_hlds__mode_ordering__type_ctor_info_pred_constraint_map_0 = {
	0,
	14,
	-1,
	MR_TYPECTOR_REP_EQUIV_GROUND,
	MR_MAYBE_STATIC_CODE(MR_ENTRY_AP(__Unify___check_hlds__mode_ordering__pred_constraint_map_0_0)),
	MR_MAYBE_STATIC_CODE(MR_ENTRY_AP(__Compare___check_hlds__mode_ordering__pred_constraint_map_0_0)),
	"check_hlds.mode_ordering",
	"pred_constraint_map",
	{ 0 },
	{ (void *)&mercury_data_tree234__ti_tree234_2builtin__type_ctor_info_int_0check_hlds__mode_ordering__type_ctor_info_pred_constraint_info_0 },
	-1,
	0,
	NULL
};


static const MR_UserClosureId
mercury_data__closure_layout__f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_112_114_101_100_95_95_91_50_93_95_48_5_0_2 = {
{
MR_PREDICATE,
"check_hlds.mode_ordering",
"check_hlds.mode_ordering",
"mode_ordering_check_proc",
8,
0
},
"check_hlds.mode_ordering",
"mode_ordering.m",
100,
"20"
};

static const MR_UserClosureId
mercury_data__closure_layout__f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_112_114_101_100_95_95_91_50_93_95_48_5_0_1 = {
{
MR_PREDICATE,
"check_hlds.mode_ordering",
"check_hlds.mode_ordering",
"mode_ordering_infer_proc",
8,
0
},
"check_hlds.mode_ordering",
"mode_ordering.m",
89,
"15"
};

static const MR_UserClosureId
mercury_data__closure_layout__fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__450__1_5_0_2 = {
{
MR_FUNCTION,
"check_hlds.mode_ordering",
"check_hlds.mode_ordering",
"lambda_mode_ordering_m_465",
5,
0
},
"check_hlds.mode_ordering",
"mode_ordering.m",
461,
"88"
};

static const MR_UserClosureId
mercury_data__closure_layout__fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__450__1_5_0_1 = {
{
MR_FUNCTION,
"check_hlds.mode_ordering",
"check_hlds.mode_ordering",
"lambda_mode_ordering_m_457",
5,
0
},
"check_hlds.mode_ordering",
"mode_ordering.m",
453,
"73"
};

static const MR_UserClosureId
mercury_data__closure_layout__fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__444__1_3_0_1 = {
{
MR_FUNCTION,
"check_hlds.mode_ordering",
"check_hlds.mode_ordering",
"lambda_mode_ordering_m_445",
4,
0
},
"check_hlds.mode_ordering",
"mode_ordering.m",
445,
"53"
};

static const MR_UserClosureId
mercury_data__closure_layout__fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__438__1_3_0_1 = {
{
MR_FUNCTION,
"check_hlds.mode_ordering",
"check_hlds.mode_ordering",
"lambda_mode_ordering_m_439",
4,
0
},
"check_hlds.mode_ordering",
"mode_ordering.m",
439,
"35"
};

static const MR_UserClosureId
mercury_data__closure_layout__check_hlds__mode_ordering__mode_order_goal_2_6_0_7 = {
{
MR_PREDICATE,
"check_hlds.mode_ordering",
"check_hlds.mode_ordering",
"lambda_mode_ordering_m_251",
5,
0
},
"check_hlds.mode_ordering",
"mode_ordering.m",
251,
"89"
};

static const MR_UserClosureId
mercury_data__closure_layout__check_hlds__mode_ordering__mode_order_goal_2_6_0_6 = {
{
MR_PREDICATE,
"check_hlds.mode_ordering",
"check_hlds.mode_ordering",
"lambda_mode_ordering_m_251",
5,
0
},
"check_hlds.mode_ordering",
"mode_ordering.m",
251,
"89"
};

static const MR_UserClosureId
mercury_data__closure_layout__check_hlds__mode_ordering__mode_order_goal_2_6_0_5 = {
{
MR_PREDICATE,
"check_hlds.mode_ordering",
"check_hlds.mode_ordering",
"lambda_mode_ordering_m_251",
5,
0
},
"check_hlds.mode_ordering",
"mode_ordering.m",
251,
"89"
};

static const MR_UserClosureId
mercury_data__closure_layout__check_hlds__mode_ordering__mode_order_goal_2_6_0_4 = {
{
MR_PREDICATE,
"hlds.inst_graph",
"hlds.inst_graph",
"reachable_from_list",
3,
0
},
"check_hlds.mode_ordering",
"mode_ordering.m",
294,
"129"
};

static const MR_UserClosureId
mercury_data__closure_layout__check_hlds__mode_ordering__mode_order_goal_2_6_0_3 = {
{
MR_PREDICATE,
"check_hlds.mode_ordering",
"check_hlds.mode_ordering",
"mode_order_goal",
4,
0
},
"check_hlds.mode_ordering",
"mode_ordering.m",
306,
"138"
};

static const MR_UserClosureId
mercury_data__closure_layout__check_hlds__mode_ordering__mode_order_goal_2_6_0_2 = {
{
MR_PREDICATE,
"check_hlds.mode_ordering",
"check_hlds.mode_ordering",
"mode_order_goal",
4,
0
},
"check_hlds.mode_ordering",
"mode_ordering.m",
195,
"14"
};

static const MR_UserClosureId
mercury_data__closure_layout__check_hlds__mode_ordering__mode_order_goal_2_6_0_1 = {
{
MR_PREDICATE,
"check_hlds.mode_ordering",
"check_hlds.mode_ordering",
"mode_order_goal",
4,
0
},
"check_hlds.mode_ordering",
"mode_ordering.m",
207,
"29"
};

static const MR_UserClosureId
mercury_data__closure_layout__check_hlds__mode_ordering__find_matching_proc_2_8_0_1 = {
{
MR_PREDICATE,
"check_hlds.mode_ordering",
"check_hlds.mode_ordering",
"lambda_mode_ordering_m_569",
7,
0
},
"check_hlds.mode_ordering",
"mode_ordering.m",
569,
"52"
};

static const MR_UserClosureId
mercury_data__closure_layout__check_hlds__mode_ordering__mode_order_conj_3_0_4 = {
{
MR_FUNCTION,
"check_hlds.mode_ordering",
"check_hlds.mode_ordering",
"lambda_mode_ordering_m_450",
6,
0
},
"check_hlds.mode_ordering",
"mode_ordering.m",
450,
"64"
};

static const MR_UserClosureId
mercury_data__closure_layout__check_hlds__mode_ordering__mode_order_conj_3_0_3 = {
{
MR_FUNCTION,
"check_hlds.mode_ordering",
"check_hlds.mode_ordering",
"lambda_mode_ordering_m_444",
4,
0
},
"check_hlds.mode_ordering",
"mode_ordering.m",
444,
"46"
};

static const MR_UserClosureId
mercury_data__closure_layout__check_hlds__mode_ordering__mode_order_conj_3_0_2 = {
{
MR_FUNCTION,
"check_hlds.mode_ordering",
"check_hlds.mode_ordering",
"lambda_mode_ordering_m_438",
4,
0
},
"check_hlds.mode_ordering",
"mode_ordering.m",
438,
"28"
};

static const MR_UserClosureId
mercury_data__closure_layout__check_hlds__mode_ordering__mode_order_conj_3_0_1 = {
{
MR_FUNCTION,
"check_hlds.mode_ordering",
"check_hlds.mode_ordering",
"lambda_mode_ordering_m_423",
4,
0
},
"check_hlds.mode_ordering",
"mode_ordering.m",
423,
"6"
};

static const MR_UserClosureId
mercury_data__closure_layout__check_hlds__mode_ordering__union_mode_vars_sets_3_0_1 = {
{
MR_PREDICATE,
"check_hlds.mode_ordering",
"check_hlds.mode_ordering",
"union_mode_vars_set",
3,
0
},
"check_hlds.mode_ordering",
"mode_ordering.m",
389,
"5"
};

static const MR_UserClosureId
mercury_data__closure_layout__check_hlds__mode_ordering__mode_order_disj_3_0_1 = {
{
MR_PREDICATE,
"check_hlds.mode_ordering",
"check_hlds.mode_ordering",
"mode_order_disj_2",
3,
0
},
"check_hlds.mode_ordering",
"mode_ordering.m",
359,
"12"
};

static const MR_UserClosureId
mercury_data__closure_layout__check_hlds__mode_ordering__mode_ordering_scc_4_0_1 = {
{
MR_PREDICATE,
"check_hlds.mode_ordering",
"check_hlds.mode_ordering",
"mode_ordering_pred",
5,
0
},
"check_hlds.mode_ordering",
"mode_ordering.m",
76,
"7"
};

static const MR_UserClosureId
mercury_data__closure_layout__check_hlds__mode_ordering__mode_ordering_6_0_1 = {
{
MR_PREDICATE,
"check_hlds.mode_ordering",
"check_hlds.mode_ordering",
"mode_ordering_scc",
4,
0
},
"check_hlds.mode_ordering",
"mode_ordering.m",
68,
"8"
};


MR_decl_entry(list__foldl_4_0);

MR_BEGIN_MODULE(check_hlds__mode_ordering_module0)
	MR_init_entry1(check_hlds__mode_ordering__mode_ordering_6_0);
	MR_INIT_PROC_LAYOUT_ADDR(mercury__check_hlds__mode_ordering__mode_ordering_6_0);
MR_BEGIN_CODE

/*-------------------------------------------------------------------------*/
/* code for 'mode_ordering'/6 mode 0 */
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_local_thread_engine_base
#endif
MR_define_entry(mercury__check_hlds__mode_ordering__mode_ordering_6_0);
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1, MR_tempr2, MR_tempr3;
	MR_tag_alloc_heap(MR_tempr1, 0, (MR_Integer) 4);
	MR_tfield(0, MR_tempr1, 0) = (MR_Word) MR_COMMON(2,0);
	MR_tfield(0, MR_tempr1, 1) = (MR_Word) MR_ENTRY_AP(check_hlds__mode_ordering__mode_ordering_scc_4_0);
	MR_tfield(0, MR_tempr1, 2) = (MR_Integer) 1;
	MR_tfield(0, MR_tempr1, 3) = MR_r1;
	MR_r1 = (MR_Word) MR_TAG_COMMON(0,1,1);
	MR_tempr2 = MR_r2;
	MR_r2 = (MR_Word) MR_CTOR0_ADDR(hlds__hlds_module, module_info);
	MR_tempr3 = MR_r3;
	MR_r3 = MR_tempr1;
	MR_r4 = MR_tempr2;
	MR_r5 = MR_tempr3;
	MR_np_tailcall_ent(list__foldl_4_0);
	}
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_thread_engine_base
#endif
MR_END_MODULE

MR_decl_entry(check_hlds__clause_to_proc__copy_module_clauses_to_procs_3_0);

MR_BEGIN_MODULE(check_hlds__mode_ordering_module1)
	MR_init_entry1(check_hlds__mode_ordering__mode_ordering_scc_4_0);
	MR_INIT_PROC_LAYOUT_ADDR(mercury__check_hlds__mode_ordering__mode_ordering_scc_4_0);
	MR_init_label1(check_hlds__mode_ordering__mode_ordering_scc_4_0,2)
MR_BEGIN_CODE

/*-------------------------------------------------------------------------*/
/* code for 'mode_ordering_scc'/4 mode 0 */
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_local_thread_engine_base
#endif
MR_def_static(check_hlds__mode_ordering__mode_ordering_scc_4_0)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_incr_sp(3);
	MR_sv(3) = (MR_Word) MR_succip;
	MR_sv(1) = MR_r1;
	MR_sv(2) = MR_r2;
	MR_r1 = MR_r2;
	MR_r2 = MR_r3;
	MR_np_call_localret_ent(check_hlds__clause_to_proc__copy_module_clauses_to_procs_3_0,
		check_hlds__mode_ordering__mode_ordering_scc_4_0_i2);
MR_def_label(check_hlds__mode_ordering__mode_ordering_scc_4_0,2)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1, MR_tempr2, MR_tempr3;
	MR_tag_alloc_heap(MR_tempr1, 0, (MR_Integer) 5);
	MR_r3 = MR_tempr1;
	MR_tfield(0, MR_tempr1, 0) = (MR_Word) MR_COMMON(3,0);
	MR_tfield(0, MR_tempr1, 1) = (MR_Word) MR_ENTRY_AP(check_hlds__mode_ordering__mode_ordering_pred_5_0);
	MR_tfield(0, MR_tempr1, 2) = (MR_Integer) 2;
	MR_tfield(0, MR_tempr1, 3) = MR_sv(1);
	MR_tempr3 = MR_sv(2);
	MR_tfield(0, MR_tempr1, 4) = MR_tempr3;
	MR_tempr2 = MR_r1;
	MR_r1 = (MR_Word) MR_CTOR0_ADDR(hlds__hlds_pred, pred_id);
	MR_r2 = (MR_Word) MR_CTOR0_ADDR(hlds__hlds_module, module_info);
	MR_r4 = MR_tempr3;
	MR_r5 = MR_tempr2;
	MR_succip_word = MR_sv(3);
	MR_decr_sp(3);
	MR_np_tailcall_ent(list__foldl_4_0);
	}
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_thread_engine_base
#endif
MR_END_MODULE


MR_BEGIN_MODULE(check_hlds__mode_ordering_module2)
	MR_init_entry1(check_hlds__mode_ordering__mode_ordering_pred_5_0);
	MR_INIT_PROC_LAYOUT_ADDR(mercury__check_hlds__mode_ordering__mode_ordering_pred_5_0);
MR_BEGIN_CODE

/*-------------------------------------------------------------------------*/
/* code for 'mode_ordering_pred'/5 mode 0 */
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_local_thread_engine_base
#endif
MR_def_static(check_hlds__mode_ordering__mode_ordering_pred_5_0)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_r2 = MR_r3;
	MR_r3 = MR_r4;
	MR_np_tailcall_ent(f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_112_114_101_100_95_95_91_50_93_95_48_5_0);
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_thread_engine_base
#endif
MR_END_MODULE

MR_decl_entry(fn__stack__push_2_0);

MR_BEGIN_MODULE(check_hlds__mode_ordering_module3)
	MR_init_entry1(check_hlds__mode_ordering__enter_lambda_goal_3_0);
	MR_INIT_PROC_LAYOUT_ADDR(mercury__check_hlds__mode_ordering__enter_lambda_goal_3_0);
	MR_init_label1(check_hlds__mode_ordering__enter_lambda_goal_3_0,2)
MR_BEGIN_CODE

/*-------------------------------------------------------------------------*/
/* code for 'enter_lambda_goal'/3 mode 0 */
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_local_thread_engine_base
#endif
MR_def_static(check_hlds__mode_ordering__enter_lambda_goal_3_0)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_incr_sp(2);
	MR_sv(2) = (MR_Word) MR_succip;
	MR_sv(1) = MR_r2;
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = (MR_Word) MR_CTOR0_ADDR(mdbcomp__goal_path, goal_id);
	MR_r2 = MR_tfield(0, MR_r2, 2);
	MR_r3 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__stack__push_2_0,
		check_hlds__mode_ordering__enter_lambda_goal_3_0_i2);
MR_def_label(check_hlds__mode_ordering__enter_lambda_goal_3_0,2)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_tag_alloc_heap(MR_r2, 0, (MR_Integer) 6);
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_sv(1);
	MR_tfield(0, MR_r2, 0) = MR_tfield(0, MR_tempr1, 0);
	MR_tfield(0, MR_r2, 1) = MR_tfield(0, MR_tempr1, 1);
	MR_tfield(0, MR_r2, 2) = MR_r1;
	MR_tfield(0, MR_r2, 3) = MR_tfield(0, MR_tempr1, 3);
	MR_tfield(0, MR_r2, 4) = MR_tfield(0, MR_tempr1, 4);
	MR_tfield(0, MR_r2, 5) = MR_tfield(0, MR_tempr1, 5);
	MR_r1 = MR_r2;
	MR_decr_sp_and_return(2);
	}
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_thread_engine_base
#endif
MR_END_MODULE

MR_decl_entry(stack__det_pop_3_0);

MR_BEGIN_MODULE(check_hlds__mode_ordering_module4)
	MR_init_entry1(check_hlds__mode_ordering__leave_lambda_goal_2_0);
	MR_INIT_PROC_LAYOUT_ADDR(mercury__check_hlds__mode_ordering__leave_lambda_goal_2_0);
	MR_init_label1(check_hlds__mode_ordering__leave_lambda_goal_2_0,2)
MR_BEGIN_CODE

/*-------------------------------------------------------------------------*/
/* code for 'leave_lambda_goal'/2 mode 0 */
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_local_thread_engine_base
#endif
MR_def_static(check_hlds__mode_ordering__leave_lambda_goal_2_0)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_incr_sp(2);
	MR_sv(2) = (MR_Word) MR_succip;
	MR_sv(1) = MR_r1;
	MR_r1 = (MR_Word) MR_CTOR0_ADDR(mdbcomp__goal_path, goal_id);
	MR_r2 = MR_tfield(0, MR_sv(1), 2);
	MR_np_call_localret_ent(stack__det_pop_3_0,
		check_hlds__mode_ordering__leave_lambda_goal_2_0_i2);
MR_def_label(check_hlds__mode_ordering__leave_lambda_goal_2_0,2)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_tag_alloc_heap(MR_r1, 0, (MR_Integer) 6);
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_sv(1);
	MR_tfield(0, MR_r1, 0) = MR_tfield(0, MR_tempr1, 0);
	MR_tfield(0, MR_r1, 1) = MR_tfield(0, MR_tempr1, 1);
	MR_tfield(0, MR_r1, 2) = MR_r2;
	MR_tfield(0, MR_r1, 3) = MR_tfield(0, MR_tempr1, 3);
	MR_tfield(0, MR_r1, 4) = MR_tfield(0, MR_tempr1, 4);
	MR_tfield(0, MR_r1, 5) = MR_tfield(0, MR_tempr1, 5);
	MR_decr_sp_and_return(2);
	}
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_thread_engine_base
#endif
MR_END_MODULE

MR_decl_entry(fn__hlds__hlds_goal__producing_vars_1_0);
MR_decl_entry(fn__f_104_108_100_115_95_95_104_108_100_115_95_103_111_97_108_95_95_112_114_111_100_117_99_105_110_103_95_118_97_114_115_32_58_61_2_0);
MR_decl_entry(fn__hlds__hlds_goal__consuming_vars_1_0);
MR_decl_entry(fn__f_104_108_100_115_95_95_104_108_100_115_95_103_111_97_108_95_95_99_111_110_115_117_109_105_110_103_95_118_97_114_115_32_58_61_2_0);
MR_decl_entry(fn__hlds__hlds_goal__make_visible_vars_1_0);
MR_decl_entry(fn__f_104_108_100_115_95_95_104_108_100_115_95_103_111_97_108_95_95_109_97_107_101_95_118_105_115_105_98_108_101_95_118_97_114_115_32_58_61_2_0);
MR_decl_entry(fn__hlds__hlds_goal__need_visible_vars_1_0);
MR_decl_entry(fn__f_104_108_100_115_95_95_104_108_100_115_95_103_111_97_108_95_95_110_101_101_100_95_118_105_115_105_98_108_101_95_118_97_114_115_32_58_61_2_0);

MR_BEGIN_MODULE(check_hlds__mode_ordering_module5)
	MR_init_entry1(check_hlds__mode_ordering__goal_info_copy_mode_var_sets_3_0);
	MR_INIT_PROC_LAYOUT_ADDR(mercury__check_hlds__mode_ordering__goal_info_copy_mode_var_sets_3_0);
	MR_init_label7(check_hlds__mode_ordering__goal_info_copy_mode_var_sets_3_0,2,3,4,5,6,7,8)
MR_BEGIN_CODE

/*-------------------------------------------------------------------------*/
/* code for 'goal_info_copy_mode_var_sets'/3 mode 0 */
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_local_thread_engine_base
#endif
MR_def_static(check_hlds__mode_ordering__goal_info_copy_mode_var_sets_3_0)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_incr_sp(3);
	MR_sv(3) = (MR_Word) MR_succip;
	MR_sv(1) = MR_r1;
	MR_sv(2) = MR_r2;
	MR_np_call_localret_ent(fn__hlds__hlds_goal__producing_vars_1_0,
		check_hlds__mode_ordering__goal_info_copy_mode_var_sets_3_0_i2);
MR_def_label(check_hlds__mode_ordering__goal_info_copy_mode_var_sets_3_0,2)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(2);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__f_104_108_100_115_95_95_104_108_100_115_95_103_111_97_108_95_95_112_114_111_100_117_99_105_110_103_95_118_97_114_115_32_58_61_2_0,
		check_hlds__mode_ordering__goal_info_copy_mode_var_sets_3_0_i3);
MR_def_label(check_hlds__mode_ordering__goal_info_copy_mode_var_sets_3_0,3)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(2) = MR_r1;
	MR_r1 = MR_sv(1);
	MR_np_call_localret_ent(fn__hlds__hlds_goal__consuming_vars_1_0,
		check_hlds__mode_ordering__goal_info_copy_mode_var_sets_3_0_i4);
MR_def_label(check_hlds__mode_ordering__goal_info_copy_mode_var_sets_3_0,4)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(2);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__f_104_108_100_115_95_95_104_108_100_115_95_103_111_97_108_95_95_99_111_110_115_117_109_105_110_103_95_118_97_114_115_32_58_61_2_0,
		check_hlds__mode_ordering__goal_info_copy_mode_var_sets_3_0_i5);
MR_def_label(check_hlds__mode_ordering__goal_info_copy_mode_var_sets_3_0,5)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(2) = MR_r1;
	MR_r1 = MR_sv(1);
	MR_np_call_localret_ent(fn__hlds__hlds_goal__make_visible_vars_1_0,
		check_hlds__mode_ordering__goal_info_copy_mode_var_sets_3_0_i6);
MR_def_label(check_hlds__mode_ordering__goal_info_copy_mode_var_sets_3_0,6)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(2);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__f_104_108_100_115_95_95_104_108_100_115_95_103_111_97_108_95_95_109_97_107_101_95_118_105_115_105_98_108_101_95_118_97_114_115_32_58_61_2_0,
		check_hlds__mode_ordering__goal_info_copy_mode_var_sets_3_0_i7);
MR_def_label(check_hlds__mode_ordering__goal_info_copy_mode_var_sets_3_0,7)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_r2 = MR_sv(1);
	MR_sv(1) = MR_r1;
	MR_r1 = MR_r2;
	MR_np_call_localret_ent(fn__hlds__hlds_goal__need_visible_vars_1_0,
		check_hlds__mode_ordering__goal_info_copy_mode_var_sets_3_0_i8);
MR_def_label(check_hlds__mode_ordering__goal_info_copy_mode_var_sets_3_0,8)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(1);
	MR_r2 = MR_tempr1;
	MR_succip_word = MR_sv(3);
	MR_decr_sp(3);
	MR_np_tailcall_ent(fn__f_104_108_100_115_95_95_104_108_100_115_95_103_111_97_108_95_95_110_101_101_100_95_118_105_115_105_98_108_101_95_118_97_114_115_32_58_61_2_0);
	}
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_thread_engine_base
#endif
MR_END_MODULE

extern const MR_TypeCtorInfo_Struct mercury_data_hlds__hlds_goal__type_ctor_info_hlds_goal_info_0;

MR_BEGIN_MODULE(check_hlds__mode_ordering_module6)
	MR_init_entry1(check_hlds__mode_ordering__mode_order_disj_3_0);
	MR_INIT_PROC_LAYOUT_ADDR(mercury__check_hlds__mode_ordering__mode_order_disj_3_0);
	MR_init_label9(check_hlds__mode_ordering__mode_order_disj_3_0,24,4,5,6,7,8,9,10,11)
MR_BEGIN_CODE

/*-------------------------------------------------------------------------*/
/* code for 'mode_order_disj'/3 mode 0 */
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_local_thread_engine_base
#endif
MR_def_static(check_hlds__mode_ordering__mode_order_disj_3_0)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (MR_LTAGS_TESTR(MR_r1,0,0)) {
		MR_GOTO_LAB(check_hlds__mode_ordering__mode_order_disj_3_0_i24);
	}
	MR_r1 = MR_r2;
	MR_proceed();
MR_def_label(check_hlds__mode_ordering__mode_order_disj_3_0,24)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_incr_sp(4);
	MR_sv(4) = (MR_Word) MR_succip;
	MR_sv(1) = MR_r2;
	MR_sv(2) = MR_tfield(0, MR_tfield(1, MR_r1, 0), 1);
	MR_sv(3) = MR_tfield(1, MR_r1, 1);
	MR_r1 = MR_sv(2);
	MR_np_call_localret_ent(fn__hlds__hlds_goal__producing_vars_1_0,
		check_hlds__mode_ordering__mode_order_disj_3_0_i4);
MR_def_label(check_hlds__mode_ordering__mode_order_disj_3_0,4)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(1);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__f_104_108_100_115_95_95_104_108_100_115_95_103_111_97_108_95_95_112_114_111_100_117_99_105_110_103_95_118_97_114_115_32_58_61_2_0,
		check_hlds__mode_ordering__mode_order_disj_3_0_i5);
MR_def_label(check_hlds__mode_ordering__mode_order_disj_3_0,5)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(1) = MR_r1;
	MR_r1 = MR_sv(2);
	MR_np_call_localret_ent(fn__hlds__hlds_goal__consuming_vars_1_0,
		check_hlds__mode_ordering__mode_order_disj_3_0_i6);
MR_def_label(check_hlds__mode_ordering__mode_order_disj_3_0,6)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(1);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__f_104_108_100_115_95_95_104_108_100_115_95_103_111_97_108_95_95_99_111_110_115_117_109_105_110_103_95_118_97_114_115_32_58_61_2_0,
		check_hlds__mode_ordering__mode_order_disj_3_0_i7);
MR_def_label(check_hlds__mode_ordering__mode_order_disj_3_0,7)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(1) = MR_r1;
	MR_r1 = MR_sv(2);
	MR_np_call_localret_ent(fn__hlds__hlds_goal__make_visible_vars_1_0,
		check_hlds__mode_ordering__mode_order_disj_3_0_i8);
MR_def_label(check_hlds__mode_ordering__mode_order_disj_3_0,8)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(1);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__f_104_108_100_115_95_95_104_108_100_115_95_103_111_97_108_95_95_109_97_107_101_95_118_105_115_105_98_108_101_95_118_97_114_115_32_58_61_2_0,
		check_hlds__mode_ordering__mode_order_disj_3_0_i9);
MR_def_label(check_hlds__mode_ordering__mode_order_disj_3_0,9)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(1) = MR_r1;
	MR_r1 = MR_sv(2);
	MR_np_call_localret_ent(fn__hlds__hlds_goal__need_visible_vars_1_0,
		check_hlds__mode_ordering__mode_order_disj_3_0_i10);
MR_def_label(check_hlds__mode_ordering__mode_order_disj_3_0,10)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(1);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__f_104_108_100_115_95_95_104_108_100_115_95_103_111_97_108_95_95_110_101_101_100_95_118_105_115_105_98_108_101_95_118_97_114_115_32_58_61_2_0,
		check_hlds__mode_ordering__mode_order_disj_3_0_i11);
MR_def_label(check_hlds__mode_ordering__mode_order_disj_3_0,11)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = (MR_Word) MR_CTOR0_ADDR(hlds__hlds_goal, hlds_goal);
	MR_r2 = (MR_Word) MR_CTOR0_ADDR(hlds__hlds_goal, hlds_goal_info);
	MR_r3 = (MR_Word) MR_TAG_COMMON(0,5,0);
	MR_r4 = MR_sv(3);
	MR_r5 = MR_tempr1;
	MR_succip_word = MR_sv(4);
	MR_decr_sp(4);
	MR_np_tailcall_ent(list__foldl_4_0);
	}
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_thread_engine_base
#endif
MR_END_MODULE

MR_decl_entry(fn__f_115_101_116_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_111_114_95_102_117_110_99_95_95_105_110_116_101_114_115_101_99_116_95_95_91_84_32_61_32_118_97_114_40_86_95_50_41_93_95_48_95_49_2_0);
MR_decl_entry(fn__f_115_101_116_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_111_114_95_102_117_110_99_95_95_117_110_105_111_110_95_95_91_84_32_61_32_118_97_114_40_86_95_50_41_93_95_48_95_49_2_0);

MR_BEGIN_MODULE(check_hlds__mode_ordering_module7)
	MR_init_entry1(check_hlds__mode_ordering__combine_mode_vars_sets_3_0);
	MR_INIT_PROC_LAYOUT_ADDR(mercury__check_hlds__mode_ordering__combine_mode_vars_sets_3_0);
	MR_init_label10(check_hlds__mode_ordering__combine_mode_vars_sets_3_0,2,3,4,5,7,8,9,10,11,12)
	MR_init_label5(check_hlds__mode_ordering__combine_mode_vars_sets_3_0,13,14,15,16,17)
MR_BEGIN_CODE

/*-------------------------------------------------------------------------*/
/* code for 'combine_mode_vars_sets'/3 mode 0 */
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_local_thread_engine_base
#endif
MR_def_static(check_hlds__mode_ordering__combine_mode_vars_sets_3_0)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_incr_sp(9);
	MR_sv(9) = (MR_Word) MR_succip;
	MR_sv(1) = MR_r1;
	MR_sv(6) = MR_r2;
	MR_r1 = MR_r2;
	MR_np_call_localret_ent(fn__hlds__hlds_goal__producing_vars_1_0,
		check_hlds__mode_ordering__combine_mode_vars_sets_3_0_i2);
MR_def_label(check_hlds__mode_ordering__combine_mode_vars_sets_3_0,2)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(2) = MR_r1;
	MR_r1 = MR_sv(6);
	MR_np_call_localret_ent(fn__hlds__hlds_goal__consuming_vars_1_0,
		check_hlds__mode_ordering__combine_mode_vars_sets_3_0_i3);
MR_def_label(check_hlds__mode_ordering__combine_mode_vars_sets_3_0,3)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(3) = MR_r1;
	MR_r1 = MR_sv(6);
	MR_np_call_localret_ent(fn__hlds__hlds_goal__make_visible_vars_1_0,
		check_hlds__mode_ordering__combine_mode_vars_sets_3_0_i4);
MR_def_label(check_hlds__mode_ordering__combine_mode_vars_sets_3_0,4)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(4) = MR_r1;
	MR_r1 = MR_sv(6);
	MR_np_call_localret_ent(fn__hlds__hlds_goal__need_visible_vars_1_0,
		check_hlds__mode_ordering__combine_mode_vars_sets_3_0_i5);
MR_def_label(check_hlds__mode_ordering__combine_mode_vars_sets_3_0,5)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(5) = MR_r1;
	MR_sv(7) = (MR_Word) MR_CTOR0_ADDR(parse_tree__prog_data, prog_var_type);
	MR_sv(8) = (MR_Word) MR_TAG_COMMON(0,1,2);
	MR_r1 = MR_sv(1);
	MR_np_call_localret_ent(fn__hlds__hlds_goal__producing_vars_1_0,
		check_hlds__mode_ordering__combine_mode_vars_sets_3_0_i7);
MR_def_label(check_hlds__mode_ordering__combine_mode_vars_sets_3_0,7)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(7);
	MR_r2 = MR_sv(8);
	MR_r3 = MR_sv(2);
	MR_r4 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__f_115_101_116_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_111_114_95_102_117_110_99_95_95_105_110_116_101_114_115_101_99_116_95_95_91_84_32_61_32_118_97_114_40_86_95_50_41_93_95_48_95_49_2_0,
		check_hlds__mode_ordering__combine_mode_vars_sets_3_0_i8);
MR_def_label(check_hlds__mode_ordering__combine_mode_vars_sets_3_0,8)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(6);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__f_104_108_100_115_95_95_104_108_100_115_95_103_111_97_108_95_95_112_114_111_100_117_99_105_110_103_95_118_97_114_115_32_58_61_2_0,
		check_hlds__mode_ordering__combine_mode_vars_sets_3_0_i9);
MR_def_label(check_hlds__mode_ordering__combine_mode_vars_sets_3_0,9)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(2) = MR_r1;
	MR_r1 = MR_sv(1);
	MR_np_call_localret_ent(fn__hlds__hlds_goal__consuming_vars_1_0,
		check_hlds__mode_ordering__combine_mode_vars_sets_3_0_i10);
MR_def_label(check_hlds__mode_ordering__combine_mode_vars_sets_3_0,10)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(7);
	MR_r2 = MR_sv(8);
	MR_r3 = MR_sv(3);
	MR_r4 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__f_115_101_116_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_111_114_95_102_117_110_99_95_95_117_110_105_111_110_95_95_91_84_32_61_32_118_97_114_40_86_95_50_41_93_95_48_95_49_2_0,
		check_hlds__mode_ordering__combine_mode_vars_sets_3_0_i11);
MR_def_label(check_hlds__mode_ordering__combine_mode_vars_sets_3_0,11)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(2);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__f_104_108_100_115_95_95_104_108_100_115_95_103_111_97_108_95_95_99_111_110_115_117_109_105_110_103_95_118_97_114_115_32_58_61_2_0,
		check_hlds__mode_ordering__combine_mode_vars_sets_3_0_i12);
MR_def_label(check_hlds__mode_ordering__combine_mode_vars_sets_3_0,12)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(2) = MR_r1;
	MR_r1 = MR_sv(1);
	MR_np_call_localret_ent(fn__hlds__hlds_goal__make_visible_vars_1_0,
		check_hlds__mode_ordering__combine_mode_vars_sets_3_0_i13);
MR_def_label(check_hlds__mode_ordering__combine_mode_vars_sets_3_0,13)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(7);
	MR_r2 = MR_sv(8);
	MR_r3 = MR_sv(4);
	MR_r4 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__f_115_101_116_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_111_114_95_102_117_110_99_95_95_105_110_116_101_114_115_101_99_116_95_95_91_84_32_61_32_118_97_114_40_86_95_50_41_93_95_48_95_49_2_0,
		check_hlds__mode_ordering__combine_mode_vars_sets_3_0_i14);
MR_def_label(check_hlds__mode_ordering__combine_mode_vars_sets_3_0,14)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(2);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__f_104_108_100_115_95_95_104_108_100_115_95_103_111_97_108_95_95_109_97_107_101_95_118_105_115_105_98_108_101_95_118_97_114_115_32_58_61_2_0,
		check_hlds__mode_ordering__combine_mode_vars_sets_3_0_i15);
MR_def_label(check_hlds__mode_ordering__combine_mode_vars_sets_3_0,15)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_r2 = MR_sv(1);
	MR_sv(1) = MR_r1;
	MR_r1 = MR_r2;
	MR_np_call_localret_ent(fn__hlds__hlds_goal__need_visible_vars_1_0,
		check_hlds__mode_ordering__combine_mode_vars_sets_3_0_i16);
MR_def_label(check_hlds__mode_ordering__combine_mode_vars_sets_3_0,16)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(7);
	MR_r2 = MR_sv(8);
	MR_r3 = MR_sv(5);
	MR_r4 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__f_115_101_116_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_111_114_95_102_117_110_99_95_95_117_110_105_111_110_95_95_91_84_32_61_32_118_97_114_40_86_95_50_41_93_95_48_95_49_2_0,
		check_hlds__mode_ordering__combine_mode_vars_sets_3_0_i17);
MR_def_label(check_hlds__mode_ordering__combine_mode_vars_sets_3_0,17)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(1);
	MR_r2 = MR_tempr1;
	MR_succip_word = MR_sv(9);
	MR_decr_sp(9);
	MR_np_tailcall_ent(fn__f_104_108_100_115_95_95_104_108_100_115_95_103_111_97_108_95_95_110_101_101_100_95_118_105_115_105_98_108_101_95_118_97_114_115_32_58_61_2_0);
	}
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_thread_engine_base
#endif
MR_END_MODULE


MR_BEGIN_MODULE(check_hlds__mode_ordering_module8)
	MR_init_entry1(check_hlds__mode_ordering__union_mode_vars_sets_3_0);
	MR_INIT_PROC_LAYOUT_ADDR(mercury__check_hlds__mode_ordering__union_mode_vars_sets_3_0);
MR_BEGIN_CODE

/*-------------------------------------------------------------------------*/
/* code for 'union_mode_vars_sets'/3 mode 0 */
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_local_thread_engine_base
#endif
MR_def_static(check_hlds__mode_ordering__union_mode_vars_sets_3_0)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1, MR_tempr2;
	MR_tempr1 = MR_r1;
	MR_r1 = (MR_Word) MR_CTOR0_ADDR(hlds__hlds_goal, hlds_goal);
	MR_tempr2 = MR_r2;
	MR_r2 = (MR_Word) MR_CTOR0_ADDR(hlds__hlds_goal, hlds_goal_info);
	MR_r3 = (MR_Word) MR_TAG_COMMON(0,5,1);
	MR_r4 = MR_tempr1;
	MR_r5 = MR_tempr2;
	MR_np_tailcall_ent(list__foldl_4_0);
	}
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_thread_engine_base
#endif
MR_END_MODULE

MR_decl_entry(fn__map__init_0_0);
MR_decl_entry(fn__list__foldl_3_0);
MR_decl_entry(fn__map__foldl_3_0);
MR_decl_entry(fn__digraph__init_0_0);
MR_decl_entry(digraph__tsort_2_0);
MR_decl_entry(fn__map__apply_to_list_2_0);
MR_decl_entry(require__unexpected_3_0);

MR_BEGIN_MODULE(check_hlds__mode_ordering_module9)
	MR_init_entry1(check_hlds__mode_ordering__mode_order_conj_3_0);
	MR_INIT_PROC_LAYOUT_ADDR(mercury__check_hlds__mode_ordering__mode_order_conj_3_0);
	MR_init_label10(check_hlds__mode_ordering__mode_order_conj_3_0,4,5,8,10,11,13,16,17,19,18)
MR_BEGIN_CODE

/*-------------------------------------------------------------------------*/
/* code for 'mode_order_conj'/3 mode 0 */
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_local_thread_engine_base
#endif
MR_def_static(check_hlds__mode_ordering__mode_order_conj_3_0)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_incr_sp(7);
	MR_sv(7) = (MR_Word) MR_succip;
	{
	MR_Word MR_tempr1;
	MR_tag_alloc_heap(MR_tempr1, 0, (MR_Integer) 4);
	MR_sv(2) = MR_tempr1;
	MR_tfield(0, MR_tempr1, 0) = (MR_Word) MR_COMMON(2,1);
	MR_tfield(0, MR_tempr1, 1) = (MR_Word) MR_ENTRY_AP(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__423__1_3_0);
	MR_tfield(0, MR_tempr1, 2) = (MR_Integer) 1;
	MR_tfield(0, MR_tempr1, 3) = MR_r1;
	MR_sv(1) = MR_r2;
	MR_sv(3) = (MR_Word) MR_CTOR0_ADDR(hlds__hlds_goal, hlds_goal);
	MR_sv(4) = (MR_Word) MR_INT_CTOR_ADDR;
	MR_sv(5) = (MR_Word) MR_CTOR_ADDR(tree234, tree234, 2);
	MR_sv(6) = (MR_Word) MR_TAG_COMMON(0,0,1);
	MR_r1 = MR_sv(4);
	MR_r2 = MR_sv(3);
	}
	MR_np_call_localret_ent(fn__map__init_0_0,
		check_hlds__mode_ordering__mode_order_conj_3_0_i4);
MR_def_label(check_hlds__mode_ordering__mode_order_conj_3_0,4)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(3);
	MR_r2 = MR_sv(6);
	MR_r3 = MR_sv(2);
	MR_r4 = MR_sv(1);
	MR_r5 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__list__foldl_3_0,
		check_hlds__mode_ordering__mode_order_conj_3_0_i5);
MR_def_label(check_hlds__mode_ordering__mode_order_conj_3_0,5)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(1) = MR_r1;
	MR_sv(2) = (MR_Word) MR_TAG_COMMON(0,1,2);
	MR_sv(5) = (MR_Word) MR_TAG_COMMON(0,0,4);
	MR_r1 = MR_sv(2);
	MR_r2 = MR_sv(4);
	MR_np_call_localret_ent(fn__map__init_0_0,
		check_hlds__mode_ordering__mode_order_conj_3_0_i8);
MR_def_label(check_hlds__mode_ordering__mode_order_conj_3_0,8)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(4);
	MR_r2 = MR_sv(3);
	MR_r3 = MR_sv(5);
	MR_r4 = (MR_Word) MR_TAG_COMMON(0,5,2);
	MR_r5 = MR_sv(1);
	MR_r6 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__map__foldl_3_0,
		check_hlds__mode_ordering__mode_order_conj_3_0_i10);
MR_def_label(check_hlds__mode_ordering__mode_order_conj_3_0,10)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(2) = MR_r1;
	MR_r1 = (MR_Word) MR_TAG_COMMON(0,1,2);
	MR_r2 = MR_sv(4);
	MR_np_call_localret_ent(fn__map__init_0_0,
		check_hlds__mode_ordering__mode_order_conj_3_0_i11);
MR_def_label(check_hlds__mode_ordering__mode_order_conj_3_0,11)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(4);
	MR_r2 = MR_sv(3);
	MR_r3 = MR_sv(5);
	MR_r4 = (MR_Word) MR_TAG_COMMON(0,5,3);
	MR_r5 = MR_sv(1);
	MR_r6 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__map__foldl_3_0,
		check_hlds__mode_ordering__mode_order_conj_3_0_i13);
MR_def_label(check_hlds__mode_ordering__mode_order_conj_3_0,13)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_tag_alloc_heap(MR_r2, 0, (MR_Integer) 5);
	MR_tfield(0, MR_r2, 0) = (MR_Word) MR_COMMON(6,0);
	MR_tfield(0, MR_r2, 1) = (MR_Word) MR_ENTRY_AP(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__450__1_5_0);
	MR_tfield(0, MR_r2, 2) = (MR_Integer) 2;
	MR_tfield(0, MR_r2, 3) = MR_sv(2);
	MR_tfield(0, MR_r2, 4) = MR_r1;
	MR_sv(2) = MR_r2;
	MR_sv(5) = (MR_Word) MR_TAG_COMMON(0,1,4);
	MR_r1 = MR_sv(4);
	MR_np_call_localret_ent(fn__digraph__init_0_0,
		check_hlds__mode_ordering__mode_order_conj_3_0_i16);
MR_def_label(check_hlds__mode_ordering__mode_order_conj_3_0,16)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(4);
	MR_r2 = MR_sv(3);
	MR_r3 = MR_sv(5);
	MR_r4 = MR_sv(2);
	MR_r5 = MR_sv(1);
	MR_r6 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__map__foldl_3_0,
		check_hlds__mode_ordering__mode_order_conj_3_0_i17);
MR_def_label(check_hlds__mode_ordering__mode_order_conj_3_0,17)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(4);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(digraph__tsort_2_0,
		check_hlds__mode_ordering__mode_order_conj_3_0_i19);
MR_def_label(check_hlds__mode_ordering__mode_order_conj_3_0,19)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (!(MR_r1)) {
		MR_GOTO_LAB(check_hlds__mode_ordering__mode_order_conj_3_0_i18);
	}
	MR_r1 = MR_sv(4);
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r2;
	MR_r2 = MR_sv(3);
	MR_r3 = MR_tempr1;
	MR_r4 = MR_sv(1);
	MR_succip_word = MR_sv(7);
	MR_decr_sp(7);
	MR_np_tailcall_ent(fn__map__apply_to_list_2_0);
	}
MR_def_label(check_hlds__mode_ordering__mode_order_conj_3_0,18)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_r1 = (MR_Word) MR_string_const("check_hlds.mode_ordering", 24);
	MR_r2 = (MR_Word) MR_string_const("predicate \140check_hlds.mode_ordering.mode_order_conj\'/3", 54);
	MR_r3 = (MR_Word) MR_string_const("tsort failed", 12);
	MR_succip_word = MR_sv(7);
	MR_decr_sp(7);
	MR_np_tailcall_ent(require__unexpected_3_0);
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_thread_engine_base
#endif
MR_END_MODULE

MR_decl_entry(fn__hlds__hlds_goal__goal_info_get_goal_id_1_0);
MR_decl_entry(map__search_3_0);
MR_decl_entry(fn__set__init_0_0);
MR_decl_entry(hlds__hlds_goal__goal_info_set_producing_vars_3_0);

MR_BEGIN_MODULE(check_hlds__mode_ordering_module10)
	MR_init_entry1(check_hlds__mode_ordering__set_atomic_prod_vars_5_0);
	MR_INIT_PROC_LAYOUT_ADDR(mercury__check_hlds__mode_ordering__set_atomic_prod_vars_5_0);
	MR_init_label7(check_hlds__mode_ordering__set_atomic_prod_vars_5_0,2,4,8,3,11,12,13)
MR_BEGIN_CODE

/*-------------------------------------------------------------------------*/
/* code for 'set_atomic_prod_vars'/5 mode 0 */
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_local_thread_engine_base
#endif
MR_def_static(check_hlds__mode_ordering__set_atomic_prod_vars_5_0)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_incr_sp(5);
	MR_sv(5) = (MR_Word) MR_succip;
	MR_sv(1) = MR_tfield(0, MR_r2, 2);
	MR_sv(2) = MR_tfield(0, MR_r2, 1);
	MR_sv(3) = MR_r1;
	MR_sv(4) = MR_r2;
	MR_np_call_localret_ent(fn__hlds__hlds_goal__goal_info_get_goal_id_1_0,
		check_hlds__mode_ordering__set_atomic_prod_vars_5_0_i2);
MR_def_label(check_hlds__mode_ordering__set_atomic_prod_vars_5_0,2)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1, MR_tempr2;
	MR_tempr1 = MR_sv(1);
	MR_sv(1) = (MR_Word) MR_CTOR0_ADDR(mdbcomp__goal_path, goal_id);
	MR_tempr2 = MR_r1;
	MR_r1 = MR_sv(1);
	MR_r2 = MR_tempr1;
	MR_r3 = MR_tempr2;
	}
	MR_np_call_localret_ent(fn__stack__push_2_0,
		check_hlds__mode_ordering__set_atomic_prod_vars_5_0_i4);
MR_def_label(check_hlds__mode_ordering__set_atomic_prod_vars_5_0,4)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = (MR_Word) MR_TAG_COMMON(0,1,6);
	MR_r2 = (MR_Word) MR_TAG_COMMON(0,1,7);
	MR_r3 = MR_sv(2);
	MR_r4 = MR_tempr1;
	}
	MR_np_call_localret_ent(map__search_3_0,
		check_hlds__mode_ordering__set_atomic_prod_vars_5_0_i8);
MR_def_label(check_hlds__mode_ordering__set_atomic_prod_vars_5_0,8)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (!(MR_r1)) {
		MR_GOTO_LAB(check_hlds__mode_ordering__set_atomic_prod_vars_5_0_i3);
	}
	MR_r1 = MR_r2;
	MR_r2 = MR_sv(3);
	MR_GOTO_LAB(check_hlds__mode_ordering__set_atomic_prod_vars_5_0_i12);
MR_def_label(check_hlds__mode_ordering__set_atomic_prod_vars_5_0,3)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_r1 = (MR_Word) MR_TAG_COMMON(0,1,2);
	MR_np_call_localret_ent(fn__set__init_0_0,
		check_hlds__mode_ordering__set_atomic_prod_vars_5_0_i11);
MR_def_label(check_hlds__mode_ordering__set_atomic_prod_vars_5_0,11)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_r2 = MR_sv(3);
MR_def_label(check_hlds__mode_ordering__set_atomic_prod_vars_5_0,12)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(1) = MR_r1;
	MR_np_call_localret_ent(hlds__hlds_goal__goal_info_set_producing_vars_3_0,
		check_hlds__mode_ordering__set_atomic_prod_vars_5_0_i13);
MR_def_label(check_hlds__mode_ordering__set_atomic_prod_vars_5_0,13)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(1);
	MR_r2 = MR_tempr1;
	MR_r3 = MR_sv(4);
	MR_decr_sp_and_return(5);
	}
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_thread_engine_base
#endif
MR_END_MODULE

MR_decl_entry(hlds__hlds_pred__proc_info_get_headvars_2_0);
MR_decl_entry(hlds__hlds_pred__proc_info_head_modes_constraint_2_0);
MR_decl_entry(fn__f_109_111_100_101_95_114_111_98_100_100_95_95_116_102_101_105_114_110_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_111_114_95_102_117_110_99_95_95_101_110_115_117_114_101_95_110_111_114_109_97_108_105_115_101_100_95_95_91_84_32_61_32_109_99_95_116_121_112_101_93_95_48_95_49_1_0);
MR_decl_entry(hlds__inst_graph__corresponding_nodes_from_lists_6_0);
MR_decl_entry(f_115_101_116_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_95_99_111_110_116_97_105_110_115_95_95_91_84_32_61_32_118_97_114_40_86_95_50_41_93_95_48_95_49_2_0);
MR_decl_entry(fn__check_hlds__mode_constraint_robdd__mode_constraint_var_2_0);
MR_decl_entry(f_109_111_100_101_95_114_111_98_100_100_95_95_116_102_101_105_114_110_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_111_114_95_102_117_110_99_95_95_118_97_114_95_101_110_116_97_105_108_101_100_95_95_91_84_32_61_32_109_99_95_116_121_112_101_93_95_48_95_49_2_0);
MR_declare_entry(MR_do_redo);
MR_decl_entry(fn__solutions__solutions_set_1_1);

MR_BEGIN_MODULE(check_hlds__mode_ordering_module11)
	MR_init_entry1(check_hlds__mode_ordering__find_matching_proc_2_8_0);
	MR_INIT_PROC_LAYOUT_ADDR(mercury__check_hlds__mode_ordering__find_matching_proc_2_8_0);
	MR_init_label10(check_hlds__mode_ordering__find_matching_proc_2_8_0,81,3,4,5,12,16,20,21,25,26)
	MR_init_label10(check_hlds__mode_ordering__find_matching_proc_2_8_0,14,29,30,34,35,39,13,10,44,7)
	MR_init_label1(check_hlds__mode_ordering__find_matching_proc_2_8_0,1)
MR_BEGIN_CODE

/*-------------------------------------------------------------------------*/
/* code for 'find_matching_proc_2'/8 mode 0 */
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_local_thread_engine_base
#endif
MR_def_static(check_hlds__mode_ordering__find_matching_proc_2_8_0)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_incr_sp(15);
	MR_sv(15) = (MR_Word) MR_succip;
MR_def_label(check_hlds__mode_ordering__find_matching_proc_2_8_0,81)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (MR_LTAGS_TEST(MR_r1,0,0)) {
		MR_GOTO_LAB(check_hlds__mode_ordering__find_matching_proc_2_8_0_i1);
	}
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_tfield(1, MR_r1, 0);
	MR_sv(1) = MR_tfield(0, MR_tempr1, 0);
	MR_sv(2) = MR_tfield(0, MR_tempr1, 1);
	MR_sv(3) = MR_tfield(1, MR_r1, 1);
	MR_sv(4) = MR_r2;
	MR_sv(5) = MR_r3;
	MR_sv(6) = MR_r4;
	MR_sv(7) = MR_r5;
	MR_sv(8) = MR_r6;
	MR_r1 = MR_sv(2);
	}
	MR_np_call_localret_ent(hlds__hlds_pred__proc_info_get_headvars_2_0,
		check_hlds__mode_ordering__find_matching_proc_2_8_0_i3);
MR_def_label(check_hlds__mode_ordering__find_matching_proc_2_8_0,3)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_r2 = MR_sv(2);
	MR_sv(2) = MR_r1;
	MR_r1 = MR_r2;
	MR_np_call_localret_ent(hlds__hlds_pred__proc_info_head_modes_constraint_2_0,
		check_hlds__mode_ordering__find_matching_proc_2_8_0_i4);
MR_def_label(check_hlds__mode_ordering__find_matching_proc_2_8_0,4)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_np_call_localret_ent(fn__f_109_111_100_101_95_114_111_98_100_100_95_95_116_102_101_105_114_110_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_111_114_95_102_117_110_99_95_95_101_110_115_117_114_101_95_110_111_114_109_97_108_105_115_101_100_95_95_91_84_32_61_32_109_99_95_116_121_112_101_93_95_48_95_49_1_0,
		check_hlds__mode_ordering__find_matching_proc_2_8_0_i5);
MR_def_label(check_hlds__mode_ordering__find_matching_proc_2_8_0,5)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(12) = (MR_Word) MR_redoip_slot(MR_maxfr);
	MR_sv(13) = (MR_Word) MR_redofr_slot(MR_maxfr);
	MR_save_maxfr(MR_sv(14));
	MR_redofr_slot_word(MR_maxfr) = (MR_Word) MR_curfr;
	MR_redoip_slot_word(MR_maxfr) = (MR_Word) MR_LABEL_AP(check_hlds__mode_ordering__find_matching_proc_2_8_0_i10);
	MR_sv(9) = MR_r1;
	MR_r1 = MR_sv(6);
	MR_r2 = MR_sv(7);
	MR_r3 = MR_sv(5);
	MR_r4 = MR_sv(2);
	MR_np_call_localret_ent(hlds__inst_graph__corresponding_nodes_from_lists_6_0,
		check_hlds__mode_ordering__find_matching_proc_2_8_0_i12);
MR_def_label(check_hlds__mode_ordering__find_matching_proc_2_8_0,12)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(10) = MR_r1;
	MR_sv(11) = MR_r2;
	MR_r1 = (MR_Word) MR_CTOR0_ADDR(parse_tree__prog_data, prog_var_type);
	MR_r2 = (MR_Word) MR_TAG_COMMON(0,1,2);
	MR_r3 = MR_sv(4);
	MR_r4 = MR_sv(10);
	MR_np_call_localret_ent(f_115_101_116_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_95_99_111_110_116_97_105_110_115_95_95_91_84_32_61_32_118_97_114_40_86_95_50_41_93_95_48_95_49_2_0,
		check_hlds__mode_ordering__find_matching_proc_2_8_0_i16);
MR_def_label(check_hlds__mode_ordering__find_matching_proc_2_8_0,16)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (!(MR_r1)) {
		MR_GOTO_LAB(check_hlds__mode_ordering__find_matching_proc_2_8_0_i14);
	}
	MR_tag_alloc_heap(MR_r2, 1, (MR_Integer) 1);
	MR_tfield(1, MR_r2, 0) = MR_sv(11);
	MR_r1 = MR_sv(8);
	MR_np_call_localret_ent(fn__check_hlds__mode_constraint_robdd__mode_constraint_var_2_0,
		check_hlds__mode_ordering__find_matching_proc_2_8_0_i20);
MR_def_label(check_hlds__mode_ordering__find_matching_proc_2_8_0,20)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(9);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(f_109_111_100_101_95_114_111_98_100_100_95_95_116_102_101_105_114_110_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_111_114_95_102_117_110_99_95_95_118_97_114_95_101_110_116_97_105_108_101_100_95_95_91_84_32_61_32_109_99_95_116_121_112_101_93_95_48_95_49_2_0,
		check_hlds__mode_ordering__find_matching_proc_2_8_0_i21);
MR_def_label(check_hlds__mode_ordering__find_matching_proc_2_8_0,21)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (!(MR_r1)) {
		MR_GOTO_LAB(check_hlds__mode_ordering__find_matching_proc_2_8_0_i13);
	}
	MR_tag_alloc_heap(MR_r2, 0, (MR_Integer) 1);
	MR_tfield(0, MR_r2, 0) = MR_sv(11);
	MR_r1 = MR_sv(8);
	MR_np_call_localret_ent(fn__check_hlds__mode_constraint_robdd__mode_constraint_var_2_0,
		check_hlds__mode_ordering__find_matching_proc_2_8_0_i25);
MR_def_label(check_hlds__mode_ordering__find_matching_proc_2_8_0,25)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(9);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(f_109_111_100_101_95_114_111_98_100_100_95_95_116_102_101_105_114_110_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_111_114_95_102_117_110_99_95_95_118_97_114_95_101_110_116_97_105_108_101_100_95_95_91_84_32_61_32_109_99_95_116_121_112_101_93_95_48_95_49_2_0,
		check_hlds__mode_ordering__find_matching_proc_2_8_0_i26);
MR_def_label(check_hlds__mode_ordering__find_matching_proc_2_8_0,26)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (MR_r1) {
		MR_GOTO_LAB(check_hlds__mode_ordering__find_matching_proc_2_8_0_i13);
	}
MR_def_label(check_hlds__mode_ordering__find_matching_proc_2_8_0,14)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_tag_alloc_heap(MR_r2, 1, (MR_Integer) 1);
	MR_tfield(1, MR_r2, 0) = MR_sv(11);
	MR_r1 = MR_sv(8);
	MR_np_call_localret_ent(fn__check_hlds__mode_constraint_robdd__mode_constraint_var_2_0,
		check_hlds__mode_ordering__find_matching_proc_2_8_0_i29);
MR_def_label(check_hlds__mode_ordering__find_matching_proc_2_8_0,29)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(9);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(f_109_111_100_101_95_114_111_98_100_100_95_95_116_102_101_105_114_110_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_111_114_95_102_117_110_99_95_95_118_97_114_95_101_110_116_97_105_108_101_100_95_95_91_84_32_61_32_109_99_95_116_121_112_101_93_95_48_95_49_2_0,
		check_hlds__mode_ordering__find_matching_proc_2_8_0_i30);
MR_def_label(check_hlds__mode_ordering__find_matching_proc_2_8_0,30)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (!(MR_r1)) {
		MR_GOTO(MR_ENTRY(MR_do_redo));
	}
	MR_tag_alloc_heap(MR_r2, 0, (MR_Integer) 1);
	MR_tfield(0, MR_r2, 0) = MR_sv(11);
	MR_r1 = MR_sv(8);
	MR_np_call_localret_ent(fn__check_hlds__mode_constraint_robdd__mode_constraint_var_2_0,
		check_hlds__mode_ordering__find_matching_proc_2_8_0_i34);
MR_def_label(check_hlds__mode_ordering__find_matching_proc_2_8_0,34)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(9);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(f_109_111_100_101_95_114_111_98_100_100_95_95_116_102_101_105_114_110_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_111_114_95_102_117_110_99_95_95_118_97_114_95_101_110_116_97_105_108_101_100_95_95_91_84_32_61_32_109_99_95_116_121_112_101_93_95_48_95_49_2_0,
		check_hlds__mode_ordering__find_matching_proc_2_8_0_i35);
MR_def_label(check_hlds__mode_ordering__find_matching_proc_2_8_0,35)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (MR_r1) {
		MR_GOTO(MR_ENTRY(MR_do_redo));
	}
	MR_r1 = (MR_Word) MR_CTOR0_ADDR(parse_tree__prog_data, prog_var_type);
	MR_r2 = (MR_Word) MR_TAG_COMMON(0,1,2);
	MR_r3 = MR_sv(4);
	MR_r4 = MR_sv(10);
	MR_np_call_localret_ent(f_115_101_116_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_95_99_111_110_116_97_105_110_115_95_95_91_84_32_61_32_118_97_114_40_86_95_50_41_93_95_48_95_49_2_0,
		check_hlds__mode_ordering__find_matching_proc_2_8_0_i39);
MR_def_label(check_hlds__mode_ordering__find_matching_proc_2_8_0,39)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (MR_r1) {
		MR_GOTO(MR_ENTRY(MR_do_redo));
	}
MR_def_label(check_hlds__mode_ordering__find_matching_proc_2_8_0,13)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_restore_maxfr(MR_sv(14));
	MR_redoip_slot_word(MR_maxfr) = MR_sv(12);
	MR_redofr_slot_word(MR_maxfr) = MR_sv(13);
	MR_GOTO_LAB(check_hlds__mode_ordering__find_matching_proc_2_8_0_i7);
MR_def_label(check_hlds__mode_ordering__find_matching_proc_2_8_0,10)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_r1 = MR_sv(9);
	MR_redoip_slot_word(MR_maxfr) = MR_sv(12);
	MR_redofr_slot_word(MR_maxfr) = MR_sv(13);
	MR_tag_alloc_heap(MR_r2, 0, (MR_Integer) 9);
	MR_tfield(0, MR_r2, 0) = (MR_Word) MR_COMMON(7,0);
	MR_tfield(0, MR_r2, 1) = (MR_Word) MR_ENTRY_AP(check_hlds__mode_ordering__IntroducedFrom__pred__find_matching_proc_2__569__1_7_0);
	MR_tfield(0, MR_r2, 2) = (MR_Integer) 6;
	MR_tfield(0, MR_r2, 3) = MR_sv(5);
	MR_tfield(0, MR_r2, 4) = MR_sv(6);
	MR_tfield(0, MR_r2, 5) = MR_sv(7);
	MR_tfield(0, MR_r2, 6) = MR_sv(8);
	MR_tfield(0, MR_r2, 7) = MR_sv(2);
	MR_tfield(0, MR_r2, 8) = MR_r1;
	MR_r1 = (MR_Word) MR_TAG_COMMON(0,1,2);
	MR_np_call_localret_ent(fn__solutions__solutions_set_1_1,
		check_hlds__mode_ordering__find_matching_proc_2_8_0_i44);
MR_def_label(check_hlds__mode_ordering__find_matching_proc_2_8_0,44)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_r2 = MR_sv(1);
	MR_r3 = MR_r1;
	MR_r1 = MR_TRUE;
	MR_decr_sp_and_return(15);
MR_def_label(check_hlds__mode_ordering__find_matching_proc_2_8_0,7)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_r1 = MR_sv(3);
	MR_r2 = MR_sv(4);
	MR_r3 = MR_sv(5);
	MR_r4 = MR_sv(6);
	MR_r5 = MR_sv(7);
	MR_r6 = MR_sv(8);
	MR_succip_word = MR_sv(15);
	MR_GOTO_LAB(check_hlds__mode_ordering__find_matching_proc_2_8_0_i81);
MR_def_label(check_hlds__mode_ordering__find_matching_proc_2_8_0,1)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_r1 = MR_FALSE;
	MR_decr_sp_and_return(15);
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_thread_engine_base
#endif
MR_END_MODULE

MR_decl_entry(map__lookup_3_0);
MR_decl_entry(hlds__hlds_module__module_info_pred_info_3_0);
MR_decl_entry(hlds__hlds_pred__pred_info_get_inst_graph_info_2_0);
MR_decl_entry(fn__hlds__inst_graph__interface_inst_graph_1_0);
MR_decl_entry(hlds__hlds_pred__pred_info_get_procedures_2_0);
extern const MR_TypeCtorInfo_Struct mercury_data_hlds__hlds_pred__type_ctor_info_proc_id_0;
extern const MR_TypeCtorInfo_Struct mercury_data_hlds__hlds_pred__type_ctor_info_proc_info_0;
MR_decl_entry(map__to_assoc_list_2_0);
MR_decl_entry(hlds__hlds_pred__pred_info_infer_modes_1_0);

MR_BEGIN_MODULE(check_hlds__mode_ordering_module12)
	MR_init_entry1(check_hlds__mode_ordering__find_matching_proc_7_0);
	MR_INIT_PROC_LAYOUT_ADDR(mercury__check_hlds__mode_ordering__find_matching_proc_7_0);
	MR_init_label10(check_hlds__mode_ordering__find_matching_proc_7_0,2,3,4,5,6,7,10,9,13,12)
MR_BEGIN_CODE

/*-------------------------------------------------------------------------*/
/* code for 'find_matching_proc'/7 mode 0 */
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_local_thread_engine_base
#endif
MR_def_static(check_hlds__mode_ordering__find_matching_proc_7_0)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_incr_sp(8);
	MR_sv(8) = (MR_Word) MR_succip;
	MR_sv(1) = MR_r1;
	MR_sv(2) = MR_r2;
	MR_sv(3) = MR_r3;
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r4;
	MR_sv(4) = MR_tfield(0, MR_tempr1, 3);
	MR_sv(5) = MR_tfield(0, MR_tempr1, 0);
	MR_sv(7) = MR_tempr1;
	MR_r1 = (MR_Word) MR_CTOR0_ADDR(hlds__hlds_pred, pred_id);
	MR_r2 = (MR_Word) MR_CTOR0_ADDR(check_hlds__mode_ordering, pred_constraint_info);
	MR_r3 = MR_tfield(0, MR_tempr1, 4);
	MR_r4 = MR_sv(1);
	}
	MR_np_call_localret_ent(map__lookup_3_0,
		check_hlds__mode_ordering__find_matching_proc_7_0_i2);
MR_def_label(check_hlds__mode_ordering__find_matching_proc_7_0,2)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_sv(1);
	MR_sv(1) = MR_tfield(0, MR_r1, 1);
	MR_r1 = MR_sv(4);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(hlds__hlds_module__module_info_pred_info_3_0,
		check_hlds__mode_ordering__find_matching_proc_7_0_i3);
MR_def_label(check_hlds__mode_ordering__find_matching_proc_7_0,3)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(4) = MR_r1;
	MR_np_call_localret_ent(hlds__hlds_pred__pred_info_get_inst_graph_info_2_0,
		check_hlds__mode_ordering__find_matching_proc_7_0_i4);
MR_def_label(check_hlds__mode_ordering__find_matching_proc_7_0,4)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_np_call_localret_ent(fn__hlds__inst_graph__interface_inst_graph_1_0,
		check_hlds__mode_ordering__find_matching_proc_7_0_i5);
MR_def_label(check_hlds__mode_ordering__find_matching_proc_7_0,5)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(6) = MR_r1;
	MR_r1 = MR_sv(4);
	MR_np_call_localret_ent(hlds__hlds_pred__pred_info_get_procedures_2_0,
		check_hlds__mode_ordering__find_matching_proc_7_0_i6);
MR_def_label(check_hlds__mode_ordering__find_matching_proc_7_0,6)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = (MR_Word) MR_CTOR0_ADDR(hlds__hlds_pred, proc_id);
	MR_r2 = (MR_Word) MR_CTOR0_ADDR(hlds__hlds_pred, proc_info);
	MR_r3 = MR_tempr1;
	}
	MR_np_call_localret_ent(map__to_assoc_list_2_0,
		check_hlds__mode_ordering__find_matching_proc_7_0_i7);
MR_def_label(check_hlds__mode_ordering__find_matching_proc_7_0,7)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_r2 = MR_sv(3);
	MR_r3 = MR_sv(2);
	MR_r4 = MR_sv(5);
	MR_r5 = MR_sv(6);
	MR_r6 = MR_sv(1);
	MR_np_call_localret_ent(check_hlds__mode_ordering__find_matching_proc_2_8_0,
		check_hlds__mode_ordering__find_matching_proc_7_0_i10);
MR_def_label(check_hlds__mode_ordering__find_matching_proc_7_0,10)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (!(MR_r1)) {
		MR_GOTO_LAB(check_hlds__mode_ordering__find_matching_proc_7_0_i9);
	}
	MR_r1 = MR_r2;
	MR_r2 = MR_r3;
	MR_r3 = MR_sv(7);
	MR_decr_sp_and_return(8);
MR_def_label(check_hlds__mode_ordering__find_matching_proc_7_0,9)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_r1 = MR_sv(4);
	MR_np_call_localret_ent(hlds__hlds_pred__pred_info_infer_modes_1_0,
		check_hlds__mode_ordering__find_matching_proc_7_0_i13);
MR_def_label(check_hlds__mode_ordering__find_matching_proc_7_0,13)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (!(MR_r1)) {
		MR_GOTO_LAB(check_hlds__mode_ordering__find_matching_proc_7_0_i12);
	}
	MR_r1 = (MR_Word) MR_string_const("check_hlds.mode_ordering", 24);
	MR_r2 = (MR_Word) MR_string_const("predicate \140check_hlds.mode_ordering.find_matching_proc\'/7", 57);
	MR_r3 = (MR_Word) MR_string_const("infer_modes NYI", 15);
	MR_succip_word = MR_sv(8);
	MR_decr_sp(8);
	MR_np_tailcall_ent(require__unexpected_3_0);
MR_def_label(check_hlds__mode_ordering__find_matching_proc_7_0,12)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_r1 = (MR_Word) MR_string_const("check_hlds.mode_ordering", 24);
	MR_r2 = (MR_Word) MR_string_const("predicate \140check_hlds.mode_ordering.find_matching_proc\'/7", 57);
	MR_r3 = (MR_Word) MR_string_const("unexpected mode error", 21);
	MR_succip_word = MR_sv(8);
	MR_decr_sp(8);
	MR_np_tailcall_ent(require__unexpected_3_0);
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_thread_engine_base
#endif
MR_END_MODULE


MR_BEGIN_MODULE(check_hlds__mode_ordering_module13)
	MR_init_entry1(check_hlds__mode_ordering__mode_order_goal_4_0);
	MR_INIT_PROC_LAYOUT_ADDR(mercury__check_hlds__mode_ordering__mode_order_goal_4_0);
	MR_init_label1(check_hlds__mode_ordering__mode_order_goal_4_0,2)
MR_BEGIN_CODE

/*-------------------------------------------------------------------------*/
/* code for 'mode_order_goal'/4 mode 0 */
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_local_thread_engine_base
#endif
MR_def_static(check_hlds__mode_ordering__mode_order_goal_4_0)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_incr_sp(1);
	MR_sv(1) = (MR_Word) MR_succip;
	{
	MR_Word MR_tempr1, MR_tempr2;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_tfield(0, MR_r1, 0);
	MR_tempr2 = MR_r2;
	MR_r2 = MR_tfield(0, MR_tempr1, 1);
	MR_r3 = MR_tempr2;
	}
	MR_np_call_localret_ent(check_hlds__mode_ordering__mode_order_goal_2_6_0,
		check_hlds__mode_ordering__mode_order_goal_4_0_i2);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_4_0,2)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tag_alloc_heap(MR_tempr1, 0, (MR_Integer) 2);
	MR_tfield(0, MR_tempr1, 0) = MR_r1;
	MR_tfield(0, MR_tempr1, 1) = MR_r2;
	MR_r1 = MR_tempr1;
	MR_r2 = MR_r3;
	MR_decr_sp_and_return(1);
	}
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_thread_engine_base
#endif
MR_END_MODULE

MR_decl_entry(list__map_foldl_5_1);
MR_decl_entry(fn__f_115_101_116_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_111_114_95_102_117_110_99_95_95_100_105_102_102_101_114_101_110_99_101_95_95_91_84_32_61_32_118_97_114_40_86_95_50_41_93_95_48_95_49_2_0);
MR_decl_entry(fn__f_115_101_116_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_111_114_95_102_117_110_99_95_95_108_105_115_116_95_116_111_95_115_101_116_95_95_91_84_32_61_32_118_97_114_40_86_95_50_41_93_95_48_95_49_1_0);
MR_decl_entry(hlds__hlds_goal__goal_info_set_consuming_vars_3_0);
MR_decl_entry(hlds__hlds_goal__goal_info_set_make_visible_vars_3_0);
MR_decl_entry(hlds__hlds_goal__goal_info_set_need_visible_vars_3_0);
MR_decl_entry(fn__set__make_singleton_set_1_0);
MR_decl_entry(hlds__inst_graph__reachable_from_list_3_0);

MR_BEGIN_MODULE(check_hlds__mode_ordering_module14)
	MR_init_entry1(check_hlds__mode_ordering__mode_order_goal_2_6_0);
	MR_INIT_PROC_LAYOUT_ADDR(mercury__check_hlds__mode_ordering__mode_order_goal_2_6_0);
	MR_init_label10(check_hlds__mode_ordering__mode_order_goal_2_6_0,7,8,5,11,12,13,14,16,17,18)
	MR_init_label10(check_hlds__mode_ordering__mode_order_goal_2_6_0,19,20,21,22,3,26,27,24,30,31)
	MR_init_label10(check_hlds__mode_ordering__mode_order_goal_2_6_0,32,35,36,38,39,40,41,42,43,44)
	MR_init_label10(check_hlds__mode_ordering__mode_order_goal_2_6_0,45,29,48,49,47,52,54,55,56,57)
	MR_init_label10(check_hlds__mode_ordering__mode_order_goal_2_6_0,58,59,60,61,51,64,65,63,68,73)
	MR_init_label10(check_hlds__mode_ordering__mode_order_goal_2_6_0,75,77,71,80,81,82,70,85,86,87)
	MR_init_label10(check_hlds__mode_ordering__mode_order_goal_2_6_0,89,92,93,94,84,97,100,101,95,105)
	MR_init_label10(check_hlds__mode_ordering__mode_order_goal_2_6_0,108,109,103,113,116,120,121,122,123,67)
	MR_init_label3(check_hlds__mode_ordering__mode_order_goal_2_6_0,125,127,129)
MR_BEGIN_CODE

/*-------------------------------------------------------------------------*/
/* code for 'mode_order_goal_2'/6 mode 0 */
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_local_thread_engine_base
#endif
MR_def_static(check_hlds__mode_ordering__mode_order_goal_2_6_0)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (MR_RTAGS_TESTR(MR_r1,3,1)) {
		MR_GOTO_LAB(check_hlds__mode_ordering__mode_order_goal_2_6_0_i3);
	}
	MR_incr_sp(16);
	MR_sv(16) = (MR_Word) MR_succip;
	{
	MR_Word MR_tempr1, MR_tempr2, MR_tempr3;
	MR_tempr1 = MR_tfield(3, MR_r1, 1);
	MR_r7 = MR_tempr1;
	if (MR_INT_NE(MR_tempr1,1)) {
		MR_GOTO_LAB(check_hlds__mode_ordering__mode_order_goal_2_6_0_i5);
	}
	MR_sv(1) = MR_tempr1;
	MR_sv(4) = MR_r2;
	MR_tempr2 = MR_r1;
	MR_r1 = (MR_Word) MR_CTOR0_ADDR(hlds__hlds_goal, hlds_goal);
	MR_r2 = MR_r1;
	MR_tempr3 = MR_r3;
	MR_r3 = (MR_Word) MR_CTOR0_ADDR(check_hlds__mode_ordering, mode_ordering_info);
	MR_r4 = (MR_Word) MR_TAG_COMMON(0,5,4);
	MR_r5 = MR_tfield(3, MR_tempr2, 2);
	MR_r6 = MR_tempr3;
	}
	MR_np_call_localret_ent(list__map_foldl_5_1,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i7);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,7)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(2) = MR_r1;
	MR_sv(8) = MR_r2;
	MR_r2 = MR_sv(4);
	MR_np_call_localret_ent(check_hlds__mode_ordering__union_mode_vars_sets_3_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i8);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,8)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1, MR_tempr2;
	MR_tag_alloc_heap(MR_tempr1, 3, (MR_Integer) 3);
	MR_tfield(3, MR_tempr1, 0) = (MR_Integer) 1;
	MR_tfield(3, MR_tempr1, 1) = MR_sv(1);
	MR_tfield(3, MR_tempr1, 2) = MR_sv(2);
	MR_tempr2 = MR_r1;
	MR_r1 = MR_tempr1;
	MR_r2 = MR_tempr2;
	MR_r3 = MR_sv(8);
	MR_decr_sp_and_return(16);
	}
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,5)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(1) = MR_r7;
	MR_sv(4) = MR_r2;
	{
	MR_Word MR_tempr1, MR_tempr2;
	MR_tempr1 = MR_r1;
	MR_r1 = (MR_Word) MR_CTOR0_ADDR(hlds__hlds_goal, hlds_goal);
	MR_r2 = MR_r1;
	MR_tempr2 = MR_r3;
	MR_r3 = (MR_Word) MR_CTOR0_ADDR(check_hlds__mode_ordering, mode_ordering_info);
	MR_r4 = (MR_Word) MR_TAG_COMMON(0,5,5);
	MR_r5 = MR_tfield(3, MR_tempr1, 2);
	MR_r6 = MR_tempr2;
	}
	MR_np_call_localret_ent(list__map_foldl_5_1,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i11);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,11)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(8) = MR_r2;
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_tfield(0, MR_r2, 5);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(check_hlds__mode_ordering__mode_order_conj_3_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i12);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,12)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(6) = MR_r1;
	MR_r2 = MR_sv(4);
	MR_np_call_localret_ent(check_hlds__mode_ordering__union_mode_vars_sets_3_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i13);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,13)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(3) = MR_r1;
	MR_np_call_localret_ent(fn__hlds__hlds_goal__consuming_vars_1_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i14);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,14)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(2) = MR_r1;
	MR_sv(4) = (MR_Word) MR_CTOR0_ADDR(parse_tree__prog_data, prog_var_type);
	MR_sv(5) = (MR_Word) MR_TAG_COMMON(0,1,2);
	MR_r1 = MR_sv(3);
	MR_np_call_localret_ent(fn__hlds__hlds_goal__producing_vars_1_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i16);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,16)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(4);
	MR_r2 = MR_sv(5);
	MR_r3 = MR_sv(2);
	MR_r4 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__f_115_101_116_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_111_114_95_102_117_110_99_95_95_100_105_102_102_101_114_101_110_99_101_95_95_91_84_32_61_32_118_97_114_40_86_95_50_41_93_95_48_95_49_2_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i17);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,17)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(3);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__f_104_108_100_115_95_95_104_108_100_115_95_103_111_97_108_95_95_99_111_110_115_117_109_105_110_103_95_118_97_114_115_32_58_61_2_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i18);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,18)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(3) = MR_r1;
	MR_np_call_localret_ent(fn__hlds__hlds_goal__need_visible_vars_1_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i19);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,19)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(2) = MR_r1;
	MR_r1 = MR_sv(3);
	MR_np_call_localret_ent(fn__hlds__hlds_goal__make_visible_vars_1_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i20);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,20)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(4);
	MR_r2 = MR_sv(5);
	MR_r3 = MR_sv(2);
	MR_r4 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__f_115_101_116_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_111_114_95_102_117_110_99_95_95_100_105_102_102_101_114_101_110_99_101_95_95_91_84_32_61_32_118_97_114_40_86_95_50_41_93_95_48_95_49_2_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i21);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,21)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(3);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__f_104_108_100_115_95_95_104_108_100_115_95_103_111_97_108_95_95_110_101_101_100_95_118_105_115_105_98_108_101_95_118_97_114_115_32_58_61_2_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i22);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,22)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1, MR_tempr2;
	MR_tag_alloc_heap(MR_tempr1, 3, (MR_Integer) 3);
	MR_tfield(3, MR_tempr1, 0) = (MR_Integer) 1;
	MR_tfield(3, MR_tempr1, 1) = MR_sv(1);
	MR_tfield(3, MR_tempr1, 2) = MR_sv(6);
	MR_tempr2 = MR_r1;
	MR_r1 = MR_tempr1;
	MR_r2 = MR_tempr2;
	MR_r3 = MR_sv(8);
	MR_decr_sp_and_return(16);
	}
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,3)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (MR_RTAGS_TESTR(MR_r1,3,2)) {
		MR_GOTO_LAB(check_hlds__mode_ordering__mode_order_goal_2_6_0_i24);
	}
	MR_incr_sp(16);
	MR_sv(16) = (MR_Word) MR_succip;
	MR_sv(4) = MR_r2;
	MR_sv(1) = MR_r1;
	MR_r1 = (MR_Word) MR_CTOR0_ADDR(hlds__hlds_goal, hlds_goal);
	MR_r2 = MR_r1;
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r3;
	MR_r3 = (MR_Word) MR_CTOR0_ADDR(check_hlds__mode_ordering, mode_ordering_info);
	MR_r4 = (MR_Word) MR_TAG_COMMON(0,5,6);
	MR_r5 = MR_tfield(3, MR_sv(1), 1);
	MR_r6 = MR_tempr1;
	}
	MR_np_call_localret_ent(list__map_foldl_5_1,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i26);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,26)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(8) = MR_r2;
	MR_sv(1) = MR_r1;
	MR_r2 = MR_sv(4);
	MR_np_call_localret_ent(check_hlds__mode_ordering__mode_order_disj_3_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i27);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,27)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1, MR_tempr2;
	MR_tag_alloc_heap(MR_tempr1, 3, (MR_Integer) 2);
	MR_tfield(3, MR_tempr1, 0) = (MR_Integer) 2;
	MR_tfield(3, MR_tempr1, 1) = MR_sv(1);
	MR_tempr2 = MR_r1;
	MR_r1 = MR_tempr1;
	MR_r2 = MR_tempr2;
	MR_r3 = MR_sv(8);
	MR_decr_sp_and_return(16);
	}
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,24)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (MR_RTAGS_TESTR(MR_r1,3,6)) {
		MR_GOTO_LAB(check_hlds__mode_ordering__mode_order_goal_2_6_0_i29);
	}
	MR_incr_sp(16);
	MR_sv(16) = (MR_Word) MR_succip;
	MR_sv(1) = MR_tfield(3, MR_r1, 1);
	MR_sv(2) = MR_tfield(3, MR_r1, 3);
	MR_sv(3) = MR_tfield(3, MR_r1, 4);
	MR_sv(4) = MR_r2;
	MR_r1 = MR_tfield(3, MR_r1, 2);
	MR_r2 = MR_r3;
	MR_np_call_localret_ent(check_hlds__mode_ordering__mode_order_goal_4_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i30);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,30)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_sv(2);
	MR_sv(2) = MR_r1;
	MR_r1 = MR_tempr1;
	}
	MR_np_call_localret_ent(check_hlds__mode_ordering__mode_order_goal_4_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i31);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,31)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_sv(3);
	MR_sv(3) = MR_r1;
	MR_r1 = MR_tempr1;
	}
	MR_np_call_localret_ent(check_hlds__mode_ordering__mode_order_goal_4_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i32);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,32)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1, MR_tempr2, MR_tempr3;
	MR_tag_alloc_heap(MR_tempr1, 1, (MR_Integer) 2);
	MR_tfield(1, MR_tempr1, 0) = MR_sv(3);
	MR_tfield(1, MR_tempr1, 1) = (MR_Word) MR_tbmkword(0, 0);
	MR_tag_alloc_heap(MR_tempr2, 1, (MR_Integer) 2);
	MR_tfield(1, MR_tempr2, 0) = MR_sv(2);
	MR_tfield(1, MR_tempr2, 1) = MR_tempr1;
	MR_tempr3 = MR_sv(4);
	MR_sv(4) = MR_r1;
	MR_sv(8) = MR_r2;
	MR_r1 = MR_tempr2;
	MR_r2 = MR_tempr3;
	}
	MR_np_call_localret_ent(check_hlds__mode_ordering__union_mode_vars_sets_3_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i35);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,35)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(5) = MR_r1;
	MR_np_call_localret_ent(fn__hlds__hlds_goal__consuming_vars_1_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i36);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,36)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(6) = MR_r1;
	MR_sv(7) = (MR_Word) MR_CTOR0_ADDR(parse_tree__prog_data, prog_var_type);
	MR_sv(9) = (MR_Word) MR_TAG_COMMON(0,1,2);
	MR_r1 = MR_sv(5);
	MR_np_call_localret_ent(fn__hlds__hlds_goal__producing_vars_1_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i38);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,38)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(7);
	MR_r2 = MR_sv(9);
	MR_r3 = MR_sv(6);
	MR_r4 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__f_115_101_116_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_111_114_95_102_117_110_99_95_95_100_105_102_102_101_114_101_110_99_101_95_95_91_84_32_61_32_118_97_114_40_86_95_50_41_93_95_48_95_49_2_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i39);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,39)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(5);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__f_104_108_100_115_95_95_104_108_100_115_95_103_111_97_108_95_95_99_111_110_115_117_109_105_110_103_95_118_97_114_115_32_58_61_2_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i40);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,40)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(5) = MR_r1;
	MR_np_call_localret_ent(fn__hlds__hlds_goal__need_visible_vars_1_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i41);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,41)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(6) = MR_r1;
	MR_r1 = MR_sv(5);
	MR_np_call_localret_ent(fn__hlds__hlds_goal__make_visible_vars_1_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i42);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,42)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(7);
	MR_r2 = MR_sv(9);
	MR_r3 = MR_sv(6);
	MR_r4 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__f_115_101_116_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_111_114_95_102_117_110_99_95_95_100_105_102_102_101_114_101_110_99_101_95_95_91_84_32_61_32_118_97_114_40_86_95_50_41_93_95_48_95_49_2_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i43);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,43)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(5);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__f_104_108_100_115_95_95_104_108_100_115_95_103_111_97_108_95_95_110_101_101_100_95_118_105_115_105_98_108_101_95_118_97_114_115_32_58_61_2_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i44);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,44)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_tfield(0, MR_sv(4), 1);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(check_hlds__mode_ordering__combine_mode_vars_sets_3_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i45);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,45)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1, MR_tempr2;
	MR_tag_alloc_heap(MR_tempr1, 3, (MR_Integer) 5);
	MR_tfield(3, MR_tempr1, 0) = (MR_Integer) 6;
	MR_tfield(3, MR_tempr1, 1) = MR_sv(1);
	MR_tfield(3, MR_tempr1, 2) = MR_sv(2);
	MR_tfield(3, MR_tempr1, 3) = MR_sv(3);
	MR_tfield(3, MR_tempr1, 4) = MR_sv(4);
	MR_tempr2 = MR_r1;
	MR_r1 = MR_tempr1;
	MR_r2 = MR_tempr2;
	MR_r3 = MR_sv(8);
	MR_decr_sp_and_return(16);
	}
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,29)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (MR_RTAGS_TESTR(MR_r1,3,4)) {
		MR_GOTO_LAB(check_hlds__mode_ordering__mode_order_goal_2_6_0_i47);
	}
	MR_incr_sp(16);
	MR_sv(16) = (MR_Word) MR_succip;
	MR_sv(4) = MR_r2;
	MR_r1 = MR_tfield(3, MR_r1, 1);
	MR_r2 = MR_r3;
	MR_np_call_localret_ent(check_hlds__mode_ordering__mode_order_goal_4_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i48);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,48)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(8) = MR_r2;
	MR_sv(1) = MR_r1;
	MR_r1 = MR_tfield(0, MR_r1, 1);
	MR_r2 = MR_sv(4);
	MR_np_call_localret_ent(check_hlds__mode_ordering__goal_info_copy_mode_var_sets_3_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i49);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,49)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1, MR_tempr2;
	MR_tag_alloc_heap(MR_tempr1, 3, (MR_Integer) 2);
	MR_r3 = MR_tempr1;
	MR_tfield(3, MR_tempr1, 0) = (MR_Integer) 4;
	MR_tfield(3, MR_tempr1, 1) = MR_sv(1);
	MR_tempr2 = MR_r1;
	MR_r1 = MR_tempr1;
	MR_r2 = MR_tempr2;
	MR_r3 = MR_sv(8);
	MR_decr_sp_and_return(16);
	}
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,47)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (MR_PTAG_TESTR(MR_r1,1)) {
		MR_GOTO_LAB(check_hlds__mode_ordering__mode_order_goal_2_6_0_i51);
	}
	MR_incr_sp(16);
	MR_sv(16) = (MR_Word) MR_succip;
	MR_sv(1) = MR_r1;
	MR_sv(2) = MR_tfield(1, MR_r1, 0);
	MR_sv(3) = MR_tfield(1, MR_r1, 2);
	MR_r1 = MR_r2;
	MR_r2 = MR_r3;
	MR_np_call_localret_ent(check_hlds__mode_ordering__set_atomic_prod_vars_5_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i52);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,52)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(4) = MR_r1;
	MR_sv(5) = MR_r2;
	MR_sv(6) = MR_r3;
	MR_sv(7) = (MR_Word) MR_CTOR0_ADDR(parse_tree__prog_data, prog_var_type);
	MR_sv(9) = (MR_Word) MR_TAG_COMMON(0,1,2);
	MR_r1 = MR_sv(7);
	MR_r2 = MR_sv(9);
	MR_r3 = MR_sv(3);
	MR_np_call_localret_ent(fn__f_115_101_116_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_111_114_95_102_117_110_99_95_95_108_105_115_116_95_116_111_95_115_101_116_95_95_91_84_32_61_32_118_97_114_40_86_95_50_41_93_95_48_95_49_1_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i54);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,54)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(7);
	MR_r2 = MR_sv(9);
	MR_r3 = MR_tempr1;
	MR_r4 = MR_sv(4);
	}
	MR_np_call_localret_ent(fn__f_115_101_116_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_111_114_95_102_117_110_99_95_95_105_110_116_101_114_115_101_99_116_95_95_91_84_32_61_32_118_97_114_40_86_95_50_41_93_95_48_95_49_2_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i55);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,55)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_sv(2);
	MR_sv(2) = MR_r1;
	MR_r1 = MR_tempr1;
	MR_r2 = MR_sv(3);
	MR_r3 = MR_sv(4);
	MR_r4 = MR_sv(6);
	}
	MR_np_call_localret_ent(check_hlds__mode_ordering__find_matching_proc_7_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i56);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,56)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_sv(3);
	MR_sv(3) = MR_r1;
	MR_sv(4) = MR_r2;
	MR_sv(8) = MR_r3;
	MR_r1 = MR_sv(7);
	MR_r2 = MR_sv(9);
	MR_r3 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__f_115_101_116_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_111_114_95_102_117_110_99_95_95_108_105_115_116_95_116_111_95_115_101_116_95_95_91_84_32_61_32_118_97_114_40_86_95_50_41_93_95_48_95_49_1_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i57);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,57)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(7);
	MR_r2 = MR_sv(9);
	MR_r3 = MR_tempr1;
	MR_r4 = MR_sv(4);
	}
	MR_np_call_localret_ent(fn__f_115_101_116_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_111_114_95_102_117_110_99_95_95_105_110_116_101_114_115_101_99_116_95_95_91_84_32_61_32_118_97_114_40_86_95_50_41_93_95_48_95_49_2_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i58);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,58)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_sv(4);
	MR_sv(4) = MR_r1;
	MR_r1 = MR_tempr1;
	MR_r2 = MR_sv(5);
	}
	MR_np_call_localret_ent(hlds__hlds_goal__goal_info_set_consuming_vars_3_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i59);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,59)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(2);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(hlds__hlds_goal__goal_info_set_make_visible_vars_3_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i60);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,60)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(4);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(hlds__hlds_goal__goal_info_set_need_visible_vars_3_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i61);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,61)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1, MR_tempr2, MR_tempr3;
	MR_tag_alloc_heap(MR_tempr1, 1, (MR_Integer) 6);
	MR_r3 = MR_tempr1;
	MR_tempr3 = MR_sv(1);
	MR_tfield(1, MR_tempr1, 0) = MR_tfield(1, MR_tempr3, 0);
	MR_tfield(1, MR_tempr1, 1) = MR_sv(3);
	MR_tfield(1, MR_tempr1, 2) = MR_tfield(1, MR_tempr3, 2);
	MR_tfield(1, MR_tempr1, 3) = MR_tfield(1, MR_tempr3, 3);
	MR_tfield(1, MR_tempr1, 4) = MR_tfield(1, MR_tempr3, 4);
	MR_tfield(1, MR_tempr1, 5) = MR_tfield(1, MR_tempr3, 5);
	MR_tempr2 = MR_r1;
	MR_r1 = MR_tempr1;
	MR_r2 = MR_tempr2;
	MR_r3 = MR_sv(8);
	MR_decr_sp_and_return(16);
	}
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,51)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (MR_RTAGS_TESTR(MR_r1,3,5)) {
		MR_GOTO_LAB(check_hlds__mode_ordering__mode_order_goal_2_6_0_i63);
	}
	MR_incr_sp(16);
	MR_sv(16) = (MR_Word) MR_succip;
	MR_sv(1) = MR_tfield(3, MR_r1, 1);
	MR_sv(4) = MR_r2;
	MR_r1 = MR_tfield(3, MR_r1, 2);
	MR_r2 = MR_r3;
	MR_np_call_localret_ent(check_hlds__mode_ordering__mode_order_goal_4_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i64);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,64)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(8) = MR_r2;
	MR_sv(2) = MR_r1;
	MR_r1 = MR_tfield(0, MR_r1, 1);
	MR_r2 = MR_sv(4);
	MR_np_call_localret_ent(check_hlds__mode_ordering__goal_info_copy_mode_var_sets_3_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i65);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,65)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1, MR_tempr2;
	MR_tag_alloc_heap(MR_tempr1, 3, (MR_Integer) 3);
	MR_r3 = MR_tempr1;
	MR_tfield(3, MR_tempr1, 0) = (MR_Integer) 5;
	MR_tfield(3, MR_tempr1, 1) = MR_sv(1);
	MR_tfield(3, MR_tempr1, 2) = MR_sv(2);
	MR_tempr2 = MR_r1;
	MR_r1 = MR_tempr1;
	MR_r2 = MR_tempr2;
	MR_r3 = MR_sv(8);
	MR_decr_sp_and_return(16);
	}
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,63)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (MR_PTAG_TESTR(MR_r1,0)) {
		MR_GOTO_LAB(check_hlds__mode_ordering__mode_order_goal_2_6_0_i67);
	}
	MR_incr_sp(16);
	MR_sv(16) = (MR_Word) MR_succip;
	MR_sv(1) = MR_tfield(0, MR_r1, 0);
	MR_sv(2) = MR_tfield(0, MR_r1, 1);
	MR_sv(3) = MR_tfield(0, MR_r1, 2);
	MR_sv(4) = MR_tfield(0, MR_r1, 3);
	MR_sv(5) = MR_tfield(0, MR_r1, 4);
	MR_r1 = MR_r2;
	MR_r2 = MR_r3;
	MR_np_call_localret_ent(check_hlds__mode_ordering__set_atomic_prod_vars_5_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i68);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,68)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (MR_PTAG_TESTR(MR_sv(2),1)) {
		MR_GOTO_LAB(check_hlds__mode_ordering__mode_order_goal_2_6_0_i70);
	}
	{
	MR_Word MR_tempr1, MR_tempr2, MR_tempr3;
	MR_tempr3 = MR_sv(2);
	MR_tempr1 = MR_tfield(1, MR_tempr3, 2);
	MR_sv(6) = MR_tempr3;
	MR_sv(2) = MR_tempr1;
	MR_sv(14) = MR_r2;
	MR_sv(15) = MR_r3;
	MR_sv(8) = (MR_Word) MR_CTOR0_ADDR(parse_tree__prog_data, prog_var_type);
	MR_sv(9) = (MR_Word) MR_TAG_COMMON(0,1,2);
	MR_tempr2 = MR_r1;
	MR_r1 = MR_sv(8);
	MR_r2 = MR_sv(9);
	MR_r3 = MR_tempr2;
	MR_r4 = MR_sv(1);
	}
	MR_np_call_localret_ent(f_115_101_116_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_95_99_111_110_116_97_105_110_115_95_95_91_84_32_61_32_118_97_114_40_86_95_50_41_93_95_48_95_49_2_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i73);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,73)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (!(MR_r1)) {
		MR_GOTO_LAB(check_hlds__mode_ordering__mode_order_goal_2_6_0_i71);
	}
	MR_sv(7) = MR_sv(4);
	MR_r1 = MR_sv(9);
	MR_np_call_localret_ent(fn__set__init_0_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i75);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,75)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tag_alloc_heap(MR_tempr1, 1, (MR_Integer) 2);
	MR_r3 = MR_tempr1;
	MR_tfield(1, MR_tempr1, 0) = MR_sv(1);
	MR_tfield(1, MR_tempr1, 1) = MR_sv(2);
	MR_sv(2) = MR_r1;
	MR_r1 = MR_sv(8);
	MR_r2 = MR_sv(9);
	}
	MR_np_call_localret_ent(fn__f_115_101_116_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_111_114_95_102_117_110_99_95_95_108_105_115_116_95_116_111_95_115_101_116_95_95_91_84_32_61_32_118_97_114_40_86_95_50_41_93_95_48_95_49_1_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i77);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,77)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(9) = MR_r1;
	MR_r1 = (MR_Word) MR_TAG_COMMON(0,1,2);
	MR_np_call_localret_ent(fn__set__init_0_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i82);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,71)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(7) = MR_sv(4);
	MR_sv(4) = (MR_Word) MR_CTOR0_ADDR(parse_tree__prog_data, prog_var_type);
	MR_sv(8) = (MR_Word) MR_TAG_COMMON(0,1,2);
	MR_r1 = MR_sv(8);
	MR_r2 = MR_sv(1);
	MR_np_call_localret_ent(fn__set__make_singleton_set_1_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i80);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,80)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_sv(2);
	MR_sv(2) = MR_r1;
	MR_r1 = MR_sv(4);
	MR_r2 = MR_sv(8);
	MR_r3 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__f_115_101_116_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_111_114_95_102_117_110_99_95_95_108_105_115_116_95_116_111_95_115_101_116_95_95_91_84_32_61_32_118_97_114_40_86_95_50_41_93_95_48_95_49_1_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i81);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,81)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(9) = MR_r1;
	MR_r1 = MR_sv(8);
	MR_r2 = MR_sv(1);
	MR_np_call_localret_ent(fn__set__make_singleton_set_1_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i82);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,82)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(8) = MR_sv(15);
	MR_r2 = MR_sv(14);
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(2);
	MR_sv(2) = MR_tempr1;
	}
	MR_np_call_localret_ent(hlds__hlds_goal__goal_info_set_consuming_vars_3_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i121);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,70)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (MR_PTAG_TESTR(MR_sv(2),2)) {
		MR_GOTO_LAB(check_hlds__mode_ordering__mode_order_goal_2_6_0_i84);
	}
	{
	MR_Word MR_tempr1, MR_tempr2;
	MR_tempr1 = MR_sv(2);
	MR_tempr2 = MR_r3;
	MR_sv(2) = MR_tfield(0, MR_tempr2, 0);
	MR_sv(7) = MR_sv(4);
	MR_sv(4) = MR_tfield(2, MR_tempr1, 0);
	MR_sv(6) = MR_tfield(2, MR_tempr1, 1);
	MR_sv(8) = MR_tfield(2, MR_tempr1, 2);
	MR_sv(9) = MR_tfield(2, MR_tempr1, 4);
	MR_sv(10) = MR_tfield(2, MR_tempr1, 5);
	MR_sv(11) = MR_tfield(2, MR_tempr1, 7);
	MR_sv(12) = MR_tfield(2, MR_tempr1, 8);
	MR_sv(13) = MR_tfield(2, MR_tempr1, 6);
	MR_sv(14) = MR_r2;
	MR_sv(15) = MR_tempr2;
	MR_r1 = MR_r2;
	}
	MR_np_call_localret_ent(fn__hlds__hlds_goal__goal_info_get_goal_id_1_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i85);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,85)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_r2 = MR_sv(15);
	MR_np_call_localret_ent(check_hlds__mode_ordering__enter_lambda_goal_3_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i86);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,86)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(12);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(check_hlds__mode_ordering__mode_order_goal_4_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i87);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,87)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tag_alloc_heap(MR_tempr1, 2, (MR_Integer) 9);
	MR_tfield(2, MR_tempr1, 0) = MR_sv(4);
	MR_tfield(2, MR_tempr1, 1) = MR_sv(6);
	MR_tfield(2, MR_tempr1, 2) = MR_sv(8);
	MR_tfield(2, MR_tempr1, 4) = MR_sv(9);
	MR_tfield(2, MR_tempr1, 5) = MR_sv(10);
	MR_tfield(2, MR_tempr1, 6) = MR_sv(13);
	MR_tfield(2, MR_tempr1, 7) = MR_sv(11);
	MR_tfield(2, MR_tempr1, 8) = MR_r1;
	MR_sv(6) = MR_tempr1;
	MR_r1 = MR_r2;
	}
	MR_np_call_localret_ent(check_hlds__mode_ordering__leave_lambda_goal_2_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i89);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,89)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_tag_alloc_heap(MR_r2, 0, (MR_Integer) 5);
	MR_tfield(0, MR_r2, 0) = (MR_Word) MR_COMMON(4,2);
	MR_tfield(0, MR_r2, 1) = (MR_Word) MR_ENTRY_AP(hlds__inst_graph__reachable_from_list_3_0);
	MR_tfield(0, MR_r2, 2) = (MR_Integer) 2;
	MR_tfield(0, MR_r2, 3) = MR_sv(2);
	MR_tfield(0, MR_r2, 4) = MR_sv(9);
	MR_sv(8) = MR_r1;
	MR_sv(4) = (MR_Word) MR_CTOR0_ADDR(parse_tree__prog_data, prog_var_type);
	MR_sv(10) = (MR_Word) MR_TAG_COMMON(0,1,2);
	MR_r1 = MR_sv(10);
	MR_np_call_localret_ent(fn__solutions__solutions_set_1_1,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i92);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,92)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(2) = MR_r1;
	MR_r1 = MR_sv(10);
	MR_r2 = MR_sv(1);
	MR_np_call_localret_ent(fn__set__make_singleton_set_1_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i93);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,93)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(9) = MR_r1;
	MR_r1 = MR_sv(4);
	MR_r2 = MR_sv(10);
	MR_r3 = MR_tfield(2, MR_sv(6), 4);
	MR_np_call_localret_ent(fn__f_115_101_116_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_111_114_95_102_117_110_99_95_95_108_105_115_116_95_116_111_95_115_101_116_95_95_91_84_32_61_32_118_97_114_40_86_95_50_41_93_95_48_95_49_1_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i94);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,94)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_r2 = MR_sv(14);
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(2);
	MR_sv(2) = MR_tempr1;
	}
	MR_np_call_localret_ent(hlds__hlds_goal__goal_info_set_consuming_vars_3_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i121);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,84)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1, MR_tempr2, MR_tempr3;
	MR_tempr2 = MR_sv(2);
	MR_sv(4) = MR_tfield(0, MR_tempr2, 0);
	MR_tempr3 = MR_r3;
	MR_tempr1 = MR_tfield(0, MR_tempr3, 0);
	MR_sv(6) = MR_tempr2;
	MR_sv(2) = MR_tempr1;
	MR_sv(14) = MR_r2;
	MR_sv(15) = MR_tempr3;
	MR_sv(8) = MR_r1;
	MR_sv(10) = (MR_Word) MR_TAG_COMMON(0,1,2);
	MR_r1 = (MR_Word) MR_CTOR0_ADDR(parse_tree__prog_data, prog_var_type);
	MR_r2 = MR_sv(10);
	MR_r3 = MR_sv(8);
	MR_r4 = MR_sv(1);
	}
	MR_np_call_localret_ent(f_115_101_116_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_95_99_111_110_116_97_105_110_115_95_95_91_84_32_61_32_118_97_114_40_86_95_50_41_93_95_48_95_49_2_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i97);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,97)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (!(MR_r1)) {
		MR_GOTO_LAB(check_hlds__mode_ordering__mode_order_goal_2_6_0_i95);
	}
	{
	MR_Word MR_tempr1, MR_tempr2;
	MR_tag_alloc_heap(MR_tempr1, 2, (MR_Integer) 2);
	MR_sv(7) = MR_tempr1;
	MR_tempr2 = MR_sv(1);
	MR_tfield(2, MR_tempr1, 0) = MR_tempr2;
	MR_tfield(2, MR_tempr1, 1) = MR_sv(4);
	MR_r1 = MR_sv(10);
	MR_r2 = MR_tempr2;
	}
	MR_np_call_localret_ent(fn__set__make_singleton_set_1_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i100);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,100)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(9) = MR_r1;
	MR_r1 = MR_sv(10);
	MR_r2 = MR_sv(4);
	MR_np_call_localret_ent(fn__set__make_singleton_set_1_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i101);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,101)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_tag_alloc_heap(MR_r2, 0, (MR_Integer) 7);
	MR_tfield(0, MR_r2, 0) = (MR_Word) MR_COMMON(3,1);
	MR_tfield(0, MR_r2, 1) = (MR_Word) MR_ENTRY_AP(check_hlds__mode_ordering__IntroducedFrom__pred__mode_order_goal_2__251__1_5_0);
	MR_tfield(0, MR_r2, 2) = (MR_Integer) 4;
	MR_tfield(0, MR_r2, 3) = MR_sv(1);
	MR_tfield(0, MR_r2, 4) = MR_sv(2);
	MR_tfield(0, MR_r2, 5) = MR_sv(4);
	MR_tfield(0, MR_r2, 6) = MR_sv(8);
	MR_sv(2) = MR_r1;
	MR_r1 = MR_sv(10);
	MR_np_call_localret_ent(fn__solutions__solutions_set_1_1,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i120);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,95)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(10) = (MR_Word) MR_TAG_COMMON(0,1,2);
	MR_r1 = (MR_Word) MR_CTOR0_ADDR(parse_tree__prog_data, prog_var_type);
	MR_r2 = MR_sv(10);
	MR_r3 = MR_sv(8);
	MR_r4 = MR_sv(4);
	MR_np_call_localret_ent(f_115_101_116_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_95_99_111_110_116_97_105_110_115_95_95_91_84_32_61_32_118_97_114_40_86_95_50_41_93_95_48_95_49_2_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i105);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,105)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (!(MR_r1)) {
		MR_GOTO_LAB(check_hlds__mode_ordering__mode_order_goal_2_6_0_i103);
	}
	{
	MR_Word MR_tempr1, MR_tempr2;
	MR_tag_alloc_heap(MR_tempr1, 2, (MR_Integer) 2);
	MR_sv(7) = MR_tempr1;
	MR_tempr2 = MR_sv(4);
	MR_tfield(2, MR_tempr1, 0) = MR_tempr2;
	MR_tfield(2, MR_tempr1, 1) = MR_sv(1);
	MR_r1 = MR_sv(10);
	MR_r2 = MR_tempr2;
	}
	MR_np_call_localret_ent(fn__set__make_singleton_set_1_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i108);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,108)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(9) = MR_r1;
	MR_r1 = MR_sv(10);
	MR_r2 = MR_sv(1);
	MR_np_call_localret_ent(fn__set__make_singleton_set_1_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i109);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,109)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_tag_alloc_heap(MR_r2, 0, (MR_Integer) 7);
	MR_tfield(0, MR_r2, 0) = (MR_Word) MR_COMMON(3,2);
	MR_tfield(0, MR_r2, 1) = (MR_Word) MR_ENTRY_AP(check_hlds__mode_ordering__IntroducedFrom__pred__mode_order_goal_2__251__1_5_0);
	MR_tfield(0, MR_r2, 2) = (MR_Integer) 4;
	MR_tfield(0, MR_r2, 3) = MR_sv(1);
	MR_tfield(0, MR_r2, 4) = MR_sv(2);
	MR_tfield(0, MR_r2, 5) = MR_sv(4);
	MR_tfield(0, MR_r2, 6) = MR_sv(8);
	MR_sv(2) = MR_r1;
	MR_r1 = MR_sv(10);
	MR_np_call_localret_ent(fn__solutions__solutions_set_1_1,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i120);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,103)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tag_alloc_heap(MR_tempr1, 3, (MR_Integer) 3);
	MR_sv(7) = MR_tempr1;
	MR_tfield(3, MR_tempr1, 0) = (MR_Integer) 0;
	MR_tfield(3, MR_tempr1, 1) = MR_sv(1);
	MR_tfield(3, MR_tempr1, 2) = MR_sv(4);
	MR_sv(9) = (MR_Word) MR_CTOR0_ADDR(parse_tree__prog_data, prog_var_type);
	MR_sv(10) = (MR_Word) MR_TAG_COMMON(0,1,2);
	MR_r1 = MR_sv(10);
	}
	MR_np_call_localret_ent(fn__set__init_0_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i113);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,113)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1, MR_tempr2;
	MR_tag_alloc_heap(MR_tempr1, 1, (MR_Integer) 2);
	MR_tfield(1, MR_tempr1, 0) = MR_sv(4);
	MR_tfield(1, MR_tempr1, 1) = (MR_Word) MR_tbmkword(0, 0);
	MR_tag_alloc_heap(MR_tempr2, 1, (MR_Integer) 2);
	MR_r3 = MR_tempr2;
	MR_tfield(1, MR_tempr2, 0) = MR_sv(1);
	MR_tfield(1, MR_tempr2, 1) = MR_tempr1;
	MR_sv(9) = MR_r1;
	MR_r1 = (MR_Word) MR_CTOR0_ADDR(parse_tree__prog_data, prog_var_type);
	MR_r2 = MR_sv(10);
	}
	MR_np_call_localret_ent(fn__f_115_101_116_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_111_114_95_102_117_110_99_95_95_108_105_115_116_95_116_111_95_115_101_116_95_95_91_84_32_61_32_118_97_114_40_86_95_50_41_93_95_48_95_49_1_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i116);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,116)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_tag_alloc_heap(MR_r2, 0, (MR_Integer) 7);
	MR_tfield(0, MR_r2, 0) = (MR_Word) MR_COMMON(3,3);
	MR_tfield(0, MR_r2, 1) = (MR_Word) MR_ENTRY_AP(check_hlds__mode_ordering__IntroducedFrom__pred__mode_order_goal_2__251__1_5_0);
	MR_tfield(0, MR_r2, 2) = (MR_Integer) 4;
	MR_tfield(0, MR_r2, 3) = MR_sv(1);
	MR_tfield(0, MR_r2, 4) = MR_sv(2);
	MR_tfield(0, MR_r2, 5) = MR_sv(4);
	MR_tfield(0, MR_r2, 6) = MR_sv(8);
	MR_sv(2) = MR_r1;
	MR_r1 = MR_sv(10);
	MR_np_call_localret_ent(fn__solutions__solutions_set_1_1,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i120);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,120)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(8) = MR_sv(15);
	MR_r2 = MR_sv(14);
	MR_np_call_localret_ent(hlds__hlds_goal__goal_info_set_consuming_vars_3_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i121);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,121)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(9);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(hlds__hlds_goal__goal_info_set_make_visible_vars_3_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i122);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,122)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(2);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(hlds__hlds_goal__goal_info_set_need_visible_vars_3_0,
		check_hlds__mode_ordering__mode_order_goal_2_6_0_i123);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,123)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1, MR_tempr2;
	MR_tag_alloc_heap(MR_tempr1, 0, (MR_Integer) 5);
	MR_tfield(0, MR_tempr1, 0) = MR_sv(1);
	MR_tfield(0, MR_tempr1, 1) = MR_sv(6);
	MR_tfield(0, MR_tempr1, 2) = MR_sv(3);
	MR_tfield(0, MR_tempr1, 3) = MR_sv(7);
	MR_tfield(0, MR_tempr1, 4) = MR_sv(5);
	MR_tempr2 = MR_r1;
	MR_r1 = MR_tempr1;
	MR_r2 = MR_tempr2;
	MR_r3 = MR_sv(8);
	MR_decr_sp_and_return(16);
	}
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,67)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (MR_RTAGS_TESTR(MR_r1,3,0)) {
		MR_GOTO_LAB(check_hlds__mode_ordering__mode_order_goal_2_6_0_i125);
	}
	MR_r1 = (MR_Word) MR_string_const("check_hlds.mode_ordering", 24);
	MR_r2 = (MR_Word) MR_string_const("predicate \140check_hlds.mode_ordering.mode_order_goal_2\'/6", 56);
	MR_r3 = (MR_Word) MR_string_const("pragma_foreign_code NYI", 23);
	MR_np_tailcall_ent(require__unexpected_3_0);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,125)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (MR_PTAG_TESTR(MR_r1,2)) {
		MR_GOTO_LAB(check_hlds__mode_ordering__mode_order_goal_2_6_0_i127);
	}
	MR_r1 = (MR_Word) MR_string_const("check_hlds.mode_ordering", 24);
	MR_r2 = (MR_Word) MR_string_const("predicate \140check_hlds.mode_ordering.mode_order_goal_2\'/6", 56);
	MR_r3 = (MR_Word) MR_string_const("generic_call NYI", 16);
	MR_np_tailcall_ent(require__unexpected_3_0);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,127)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (MR_RTAGS_TESTR(MR_r1,3,7)) {
		MR_GOTO_LAB(check_hlds__mode_ordering__mode_order_goal_2_6_0_i129);
	}
	MR_r1 = (MR_Word) MR_string_const("check_hlds.mode_ordering", 24);
	MR_r2 = (MR_Word) MR_string_const("predicate \140check_hlds.mode_ordering.mode_order_goal_2\'/6", 56);
	MR_r3 = (MR_Word) MR_string_const("shorthand", 9);
	MR_np_tailcall_ent(require__unexpected_3_0);
MR_def_label(check_hlds__mode_ordering__mode_order_goal_2_6_0,129)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_r1 = (MR_Word) MR_string_const("check_hlds.mode_ordering", 24);
	MR_r2 = (MR_Word) MR_string_const("predicate \140check_hlds.mode_ordering.mode_order_goal_2\'/6", 56);
	MR_r3 = (MR_Word) MR_string_const("switch", 6);
	MR_np_tailcall_ent(require__unexpected_3_0);
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_thread_engine_base
#endif
MR_END_MODULE

MR_decl_entry(hlds__hlds_pred__pred_info_proc_info_3_0);
MR_decl_entry(fn__f_109_111_100_101_95_114_111_98_100_100_95_95_116_102_101_105_114_110_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_111_114_95_102_117_110_99_95_95_42_95_95_91_84_32_61_32_109_99_95_116_121_112_101_93_95_48_95_49_2_0);
MR_decl_entry(fn__hlds__inst_graph__implementation_inst_graph_1_0);
MR_decl_entry(fn__check_hlds__mode_constraint_robdd__atomic_prodvars_map_2_0);
MR_decl_entry(fn__stack__init_0_0);
MR_decl_entry(check_hlds__mode_constraint_robdd__get_forward_goal_path_map_for_pred_3_0);
MR_decl_entry(hlds__hlds_pred__proc_info_get_goal_2_0);
MR_decl_entry(hlds__hlds_pred__proc_info_set_goal_3_0);
MR_decl_entry(hlds__hlds_pred__pred_info_set_proc_info_4_0);

MR_BEGIN_MODULE(check_hlds__mode_ordering_module15)
	MR_init_entry1(check_hlds__mode_ordering__mode_ordering_check_proc_8_0);
	MR_INIT_PROC_LAYOUT_ADDR(mercury__check_hlds__mode_ordering__mode_ordering_check_proc_8_0);
	MR_init_label10(check_hlds__mode_ordering__mode_ordering_check_proc_8_0,2,3,4,5,6,7,8,9,11,12)
	MR_init_label1(check_hlds__mode_ordering__mode_ordering_check_proc_8_0,14)
MR_BEGIN_CODE

/*-------------------------------------------------------------------------*/
/* code for 'mode_ordering_check_proc'/8 mode 0 */
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_local_thread_engine_base
#endif
MR_def_static(check_hlds__mode_ordering__mode_ordering_check_proc_8_0)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_incr_sp(10);
	MR_sv(10) = (MR_Word) MR_succip;
	MR_sv(1) = MR_r1;
	MR_sv(2) = MR_r2;
	MR_sv(3) = MR_r3;
	MR_sv(4) = MR_r4;
	MR_sv(5) = MR_r5;
	{
	MR_Word MR_tempr1, MR_tempr2;
	MR_tempr1 = MR_r6;
	MR_sv(6) = MR_tempr1;
	MR_tempr2 = MR_r7;
	MR_sv(8) = MR_tempr2;
	MR_r1 = MR_tempr2;
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(hlds__hlds_pred__pred_info_proc_info_3_0,
		check_hlds__mode_ordering__mode_ordering_check_proc_8_0_i2);
MR_def_label(check_hlds__mode_ordering__mode_ordering_check_proc_8_0,2)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(7) = MR_r1;
	MR_np_call_localret_ent(hlds__hlds_pred__proc_info_head_modes_constraint_2_0,
		check_hlds__mode_ordering__mode_ordering_check_proc_8_0_i3);
MR_def_label(check_hlds__mode_ordering__mode_ordering_check_proc_8_0,3)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(5);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__f_109_111_100_101_95_114_111_98_100_100_95_95_116_102_101_105_114_110_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_111_114_95_102_117_110_99_95_95_42_95_95_91_84_32_61_32_109_99_95_116_121_112_101_93_95_48_95_49_2_0,
		check_hlds__mode_ordering__mode_ordering_check_proc_8_0_i4);
MR_def_label(check_hlds__mode_ordering__mode_ordering_check_proc_8_0,4)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(5) = MR_r1;
	MR_r1 = MR_sv(8);
	MR_np_call_localret_ent(hlds__hlds_pred__pred_info_get_inst_graph_info_2_0,
		check_hlds__mode_ordering__mode_ordering_check_proc_8_0_i5);
MR_def_label(check_hlds__mode_ordering__mode_ordering_check_proc_8_0,5)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_np_call_localret_ent(fn__hlds__inst_graph__implementation_inst_graph_1_0,
		check_hlds__mode_ordering__mode_ordering_check_proc_8_0_i6);
MR_def_label(check_hlds__mode_ordering__mode_ordering_check_proc_8_0,6)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_sv(5);
	MR_sv(5) = MR_r1;
	MR_r1 = MR_tempr1;
	MR_r2 = MR_sv(4);
	}
	MR_np_call_localret_ent(fn__check_hlds__mode_constraint_robdd__atomic_prodvars_map_2_0,
		check_hlds__mode_ordering__mode_ordering_check_proc_8_0_i7);
MR_def_label(check_hlds__mode_ordering__mode_ordering_check_proc_8_0,7)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(9) = MR_r1;
	MR_r1 = (MR_Word) MR_CTOR0_ADDR(mdbcomp__goal_path, goal_id);
	MR_np_call_localret_ent(fn__stack__init_0_0,
		check_hlds__mode_ordering__mode_ordering_check_proc_8_0_i8);
MR_def_label(check_hlds__mode_ordering__mode_ordering_check_proc_8_0,8)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_sv(3);
	MR_sv(3) = MR_r1;
	MR_r1 = MR_sv(4);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(check_hlds__mode_constraint_robdd__get_forward_goal_path_map_for_pred_3_0,
		check_hlds__mode_ordering__mode_ordering_check_proc_8_0_i9);
MR_def_label(check_hlds__mode_ordering__mode_ordering_check_proc_8_0,9)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_tag_alloc_heap(MR_r2, 0, (MR_Integer) 6);
	MR_tfield(0, MR_r2, 0) = MR_sv(5);
	MR_tfield(0, MR_r2, 1) = MR_sv(9);
	MR_tfield(0, MR_r2, 2) = MR_sv(3);
	MR_tfield(0, MR_r2, 3) = MR_sv(1);
	MR_tfield(0, MR_r2, 4) = MR_sv(2);
	MR_tfield(0, MR_r2, 5) = MR_r1;
	MR_sv(1) = MR_r2;
	MR_r1 = MR_sv(7);
	MR_np_call_localret_ent(hlds__hlds_pred__proc_info_get_goal_2_0,
		check_hlds__mode_ordering__mode_ordering_check_proc_8_0_i11);
MR_def_label(check_hlds__mode_ordering__mode_ordering_check_proc_8_0,11)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_tfield(0, MR_r1, 0);
	MR_r2 = MR_tfield(0, MR_tempr1, 1);
	MR_r3 = MR_sv(1);
	}
	MR_np_call_localret_ent(check_hlds__mode_ordering__mode_order_goal_2_6_0,
		check_hlds__mode_ordering__mode_ordering_check_proc_8_0_i12);
MR_def_label(check_hlds__mode_ordering__mode_ordering_check_proc_8_0,12)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tag_alloc_heap(MR_tempr1, 0, (MR_Integer) 2);
	MR_tfield(0, MR_tempr1, 0) = MR_r1;
	MR_tfield(0, MR_tempr1, 1) = MR_r2;
	MR_r1 = MR_tempr1;
	MR_r2 = MR_sv(7);
	}
	MR_np_call_localret_ent(hlds__hlds_pred__proc_info_set_goal_3_0,
		check_hlds__mode_ordering__mode_ordering_check_proc_8_0_i14);
MR_def_label(check_hlds__mode_ordering__mode_ordering_check_proc_8_0,14)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(6);
	MR_r2 = MR_tempr1;
	MR_r3 = MR_sv(8);
	MR_succip_word = MR_sv(10);
	MR_decr_sp(10);
	MR_np_tailcall_ent(hlds__hlds_pred__pred_info_set_proc_info_4_0);
	}
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_thread_engine_base
#endif
MR_END_MODULE


MR_BEGIN_MODULE(check_hlds__mode_ordering_module16)
	MR_init_entry1(check_hlds__mode_ordering__mode_ordering_infer_proc_8_0);
	MR_INIT_PROC_LAYOUT_ADDR(mercury__check_hlds__mode_ordering__mode_ordering_infer_proc_8_0);
MR_BEGIN_CODE

/*-------------------------------------------------------------------------*/
/* code for 'mode_ordering_infer_proc'/8 mode 0 */
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_local_thread_engine_base
#endif
MR_def_static(check_hlds__mode_ordering__mode_ordering_infer_proc_8_0)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_r6 = MR_r7;
	MR_np_tailcall_ent(f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_105_110_102_101_114_95_112_114_111_99_95_95_91_54_93_95_48_8_0);
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_thread_engine_base
#endif
MR_END_MODULE


MR_BEGIN_MODULE(check_hlds__mode_ordering_module17)
	MR_init_entry1(check_hlds__mode_ordering__mode_order_disj_2_3_0);
	MR_INIT_PROC_LAYOUT_ADDR(mercury__check_hlds__mode_ordering__mode_order_disj_2_3_0);
	MR_init_label10(check_hlds__mode_ordering__mode_order_disj_2_3_0,2,3,4,5,7,8,9,10,11,12)
	MR_init_label5(check_hlds__mode_ordering__mode_order_disj_2_3_0,13,14,15,16,17)
MR_BEGIN_CODE

/*-------------------------------------------------------------------------*/
/* code for 'mode_order_disj_2'/3 mode 0 */
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_local_thread_engine_base
#endif
MR_def_static(check_hlds__mode_ordering__mode_order_disj_2_3_0)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_incr_sp(9);
	MR_sv(9) = (MR_Word) MR_succip;
	MR_sv(1) = MR_tfield(0, MR_r1, 1);
	MR_sv(2) = MR_r2;
	MR_r1 = MR_r2;
	MR_np_call_localret_ent(fn__hlds__hlds_goal__producing_vars_1_0,
		check_hlds__mode_ordering__mode_order_disj_2_3_0_i2);
MR_def_label(check_hlds__mode_ordering__mode_order_disj_2_3_0,2)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(3) = MR_r1;
	MR_r1 = MR_sv(2);
	MR_np_call_localret_ent(fn__hlds__hlds_goal__consuming_vars_1_0,
		check_hlds__mode_ordering__mode_order_disj_2_3_0_i3);
MR_def_label(check_hlds__mode_ordering__mode_order_disj_2_3_0,3)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(4) = MR_r1;
	MR_r1 = MR_sv(2);
	MR_np_call_localret_ent(fn__hlds__hlds_goal__make_visible_vars_1_0,
		check_hlds__mode_ordering__mode_order_disj_2_3_0_i4);
MR_def_label(check_hlds__mode_ordering__mode_order_disj_2_3_0,4)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(5) = MR_r1;
	MR_r1 = MR_sv(2);
	MR_np_call_localret_ent(fn__hlds__hlds_goal__need_visible_vars_1_0,
		check_hlds__mode_ordering__mode_order_disj_2_3_0_i5);
MR_def_label(check_hlds__mode_ordering__mode_order_disj_2_3_0,5)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(6) = MR_r1;
	MR_sv(7) = (MR_Word) MR_CTOR0_ADDR(parse_tree__prog_data, prog_var_type);
	MR_sv(8) = (MR_Word) MR_TAG_COMMON(0,1,2);
	MR_r1 = MR_sv(1);
	MR_np_call_localret_ent(fn__hlds__hlds_goal__producing_vars_1_0,
		check_hlds__mode_ordering__mode_order_disj_2_3_0_i7);
MR_def_label(check_hlds__mode_ordering__mode_order_disj_2_3_0,7)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(7);
	MR_r2 = MR_sv(8);
	MR_r3 = MR_sv(3);
	MR_r4 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__f_115_101_116_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_111_114_95_102_117_110_99_95_95_105_110_116_101_114_115_101_99_116_95_95_91_84_32_61_32_118_97_114_40_86_95_50_41_93_95_48_95_49_2_0,
		check_hlds__mode_ordering__mode_order_disj_2_3_0_i8);
MR_def_label(check_hlds__mode_ordering__mode_order_disj_2_3_0,8)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(2);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__f_104_108_100_115_95_95_104_108_100_115_95_103_111_97_108_95_95_112_114_111_100_117_99_105_110_103_95_118_97_114_115_32_58_61_2_0,
		check_hlds__mode_ordering__mode_order_disj_2_3_0_i9);
MR_def_label(check_hlds__mode_ordering__mode_order_disj_2_3_0,9)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(2) = MR_r1;
	MR_r1 = MR_sv(1);
	MR_np_call_localret_ent(fn__hlds__hlds_goal__consuming_vars_1_0,
		check_hlds__mode_ordering__mode_order_disj_2_3_0_i10);
MR_def_label(check_hlds__mode_ordering__mode_order_disj_2_3_0,10)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(7);
	MR_r2 = MR_sv(8);
	MR_r3 = MR_sv(4);
	MR_r4 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__f_115_101_116_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_111_114_95_102_117_110_99_95_95_117_110_105_111_110_95_95_91_84_32_61_32_118_97_114_40_86_95_50_41_93_95_48_95_49_2_0,
		check_hlds__mode_ordering__mode_order_disj_2_3_0_i11);
MR_def_label(check_hlds__mode_ordering__mode_order_disj_2_3_0,11)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(2);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__f_104_108_100_115_95_95_104_108_100_115_95_103_111_97_108_95_95_99_111_110_115_117_109_105_110_103_95_118_97_114_115_32_58_61_2_0,
		check_hlds__mode_ordering__mode_order_disj_2_3_0_i12);
MR_def_label(check_hlds__mode_ordering__mode_order_disj_2_3_0,12)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(2) = MR_r1;
	MR_r1 = MR_sv(1);
	MR_np_call_localret_ent(fn__hlds__hlds_goal__make_visible_vars_1_0,
		check_hlds__mode_ordering__mode_order_disj_2_3_0_i13);
MR_def_label(check_hlds__mode_ordering__mode_order_disj_2_3_0,13)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(7);
	MR_r2 = MR_sv(8);
	MR_r3 = MR_sv(5);
	MR_r4 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__f_115_101_116_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_111_114_95_102_117_110_99_95_95_105_110_116_101_114_115_101_99_116_95_95_91_84_32_61_32_118_97_114_40_86_95_50_41_93_95_48_95_49_2_0,
		check_hlds__mode_ordering__mode_order_disj_2_3_0_i14);
MR_def_label(check_hlds__mode_ordering__mode_order_disj_2_3_0,14)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(2);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__f_104_108_100_115_95_95_104_108_100_115_95_103_111_97_108_95_95_109_97_107_101_95_118_105_115_105_98_108_101_95_118_97_114_115_32_58_61_2_0,
		check_hlds__mode_ordering__mode_order_disj_2_3_0_i15);
MR_def_label(check_hlds__mode_ordering__mode_order_disj_2_3_0,15)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_r2 = MR_sv(1);
	MR_sv(1) = MR_r1;
	MR_r1 = MR_r2;
	MR_np_call_localret_ent(fn__hlds__hlds_goal__need_visible_vars_1_0,
		check_hlds__mode_ordering__mode_order_disj_2_3_0_i16);
MR_def_label(check_hlds__mode_ordering__mode_order_disj_2_3_0,16)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(7);
	MR_r2 = MR_sv(8);
	MR_r3 = MR_sv(6);
	MR_r4 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__f_115_101_116_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_111_114_95_102_117_110_99_95_95_117_110_105_111_110_95_95_91_84_32_61_32_118_97_114_40_86_95_50_41_93_95_48_95_49_2_0,
		check_hlds__mode_ordering__mode_order_disj_2_3_0_i17);
MR_def_label(check_hlds__mode_ordering__mode_order_disj_2_3_0,17)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(1);
	MR_r2 = MR_tempr1;
	MR_succip_word = MR_sv(9);
	MR_decr_sp(9);
	MR_np_tailcall_ent(fn__f_104_108_100_115_95_95_104_108_100_115_95_103_111_97_108_95_95_110_101_101_100_95_118_105_115_105_98_108_101_95_118_97_114_115_32_58_61_2_0);
	}
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_thread_engine_base
#endif
MR_END_MODULE


MR_BEGIN_MODULE(check_hlds__mode_ordering_module18)
	MR_init_entry1(check_hlds__mode_ordering__union_mode_vars_set_3_0);
	MR_INIT_PROC_LAYOUT_ADDR(mercury__check_hlds__mode_ordering__union_mode_vars_set_3_0);
	MR_init_label10(check_hlds__mode_ordering__union_mode_vars_set_3_0,2,3,4,5,7,8,9,10,11,12)
	MR_init_label5(check_hlds__mode_ordering__union_mode_vars_set_3_0,13,14,15,16,17)
MR_BEGIN_CODE

/*-------------------------------------------------------------------------*/
/* code for 'union_mode_vars_set'/3 mode 0 */
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_local_thread_engine_base
#endif
MR_def_static(check_hlds__mode_ordering__union_mode_vars_set_3_0)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_incr_sp(9);
	MR_sv(9) = (MR_Word) MR_succip;
	MR_sv(1) = MR_r1;
	MR_sv(6) = MR_r2;
	MR_r1 = MR_r2;
	MR_np_call_localret_ent(fn__hlds__hlds_goal__producing_vars_1_0,
		check_hlds__mode_ordering__union_mode_vars_set_3_0_i2);
MR_def_label(check_hlds__mode_ordering__union_mode_vars_set_3_0,2)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(2) = MR_r1;
	MR_r1 = MR_sv(6);
	MR_np_call_localret_ent(fn__hlds__hlds_goal__consuming_vars_1_0,
		check_hlds__mode_ordering__union_mode_vars_set_3_0_i3);
MR_def_label(check_hlds__mode_ordering__union_mode_vars_set_3_0,3)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(3) = MR_r1;
	MR_r1 = MR_sv(6);
	MR_np_call_localret_ent(fn__hlds__hlds_goal__make_visible_vars_1_0,
		check_hlds__mode_ordering__union_mode_vars_set_3_0_i4);
MR_def_label(check_hlds__mode_ordering__union_mode_vars_set_3_0,4)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(4) = MR_r1;
	MR_r1 = MR_sv(6);
	MR_np_call_localret_ent(fn__hlds__hlds_goal__need_visible_vars_1_0,
		check_hlds__mode_ordering__union_mode_vars_set_3_0_i5);
MR_def_label(check_hlds__mode_ordering__union_mode_vars_set_3_0,5)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_r2 = MR_sv(1);
	MR_sv(1) = MR_r1;
	MR_sv(5) = MR_tfield(0, MR_r2, 1);
	MR_sv(7) = (MR_Word) MR_CTOR0_ADDR(parse_tree__prog_data, prog_var_type);
	MR_sv(8) = (MR_Word) MR_TAG_COMMON(0,1,2);
	MR_r1 = MR_sv(5);
	MR_np_call_localret_ent(fn__hlds__hlds_goal__producing_vars_1_0,
		check_hlds__mode_ordering__union_mode_vars_set_3_0_i7);
MR_def_label(check_hlds__mode_ordering__union_mode_vars_set_3_0,7)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(7);
	MR_r2 = MR_sv(8);
	MR_r3 = MR_sv(2);
	MR_r4 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__f_115_101_116_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_111_114_95_102_117_110_99_95_95_117_110_105_111_110_95_95_91_84_32_61_32_118_97_114_40_86_95_50_41_93_95_48_95_49_2_0,
		check_hlds__mode_ordering__union_mode_vars_set_3_0_i8);
MR_def_label(check_hlds__mode_ordering__union_mode_vars_set_3_0,8)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(6);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__f_104_108_100_115_95_95_104_108_100_115_95_103_111_97_108_95_95_112_114_111_100_117_99_105_110_103_95_118_97_114_115_32_58_61_2_0,
		check_hlds__mode_ordering__union_mode_vars_set_3_0_i9);
MR_def_label(check_hlds__mode_ordering__union_mode_vars_set_3_0,9)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(2) = MR_r1;
	MR_r1 = MR_sv(5);
	MR_np_call_localret_ent(fn__hlds__hlds_goal__consuming_vars_1_0,
		check_hlds__mode_ordering__union_mode_vars_set_3_0_i10);
MR_def_label(check_hlds__mode_ordering__union_mode_vars_set_3_0,10)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(7);
	MR_r2 = MR_sv(8);
	MR_r3 = MR_sv(3);
	MR_r4 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__f_115_101_116_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_111_114_95_102_117_110_99_95_95_117_110_105_111_110_95_95_91_84_32_61_32_118_97_114_40_86_95_50_41_93_95_48_95_49_2_0,
		check_hlds__mode_ordering__union_mode_vars_set_3_0_i11);
MR_def_label(check_hlds__mode_ordering__union_mode_vars_set_3_0,11)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(2);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__f_104_108_100_115_95_95_104_108_100_115_95_103_111_97_108_95_95_99_111_110_115_117_109_105_110_103_95_118_97_114_115_32_58_61_2_0,
		check_hlds__mode_ordering__union_mode_vars_set_3_0_i12);
MR_def_label(check_hlds__mode_ordering__union_mode_vars_set_3_0,12)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(2) = MR_r1;
	MR_r1 = MR_sv(5);
	MR_np_call_localret_ent(fn__hlds__hlds_goal__make_visible_vars_1_0,
		check_hlds__mode_ordering__union_mode_vars_set_3_0_i13);
MR_def_label(check_hlds__mode_ordering__union_mode_vars_set_3_0,13)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(7);
	MR_r2 = MR_sv(8);
	MR_r3 = MR_sv(4);
	MR_r4 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__f_115_101_116_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_111_114_95_102_117_110_99_95_95_117_110_105_111_110_95_95_91_84_32_61_32_118_97_114_40_86_95_50_41_93_95_48_95_49_2_0,
		check_hlds__mode_ordering__union_mode_vars_set_3_0_i14);
MR_def_label(check_hlds__mode_ordering__union_mode_vars_set_3_0,14)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(2);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__f_104_108_100_115_95_95_104_108_100_115_95_103_111_97_108_95_95_109_97_107_101_95_118_105_115_105_98_108_101_95_118_97_114_115_32_58_61_2_0,
		check_hlds__mode_ordering__union_mode_vars_set_3_0_i15);
MR_def_label(check_hlds__mode_ordering__union_mode_vars_set_3_0,15)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(2) = MR_r1;
	MR_r1 = MR_sv(5);
	MR_np_call_localret_ent(fn__hlds__hlds_goal__need_visible_vars_1_0,
		check_hlds__mode_ordering__union_mode_vars_set_3_0_i16);
MR_def_label(check_hlds__mode_ordering__union_mode_vars_set_3_0,16)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(7);
	MR_r2 = MR_sv(8);
	MR_r3 = MR_sv(1);
	MR_r4 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__f_115_101_116_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_111_114_95_102_117_110_99_95_95_117_110_105_111_110_95_95_91_84_32_61_32_118_97_114_40_86_95_50_41_93_95_48_95_49_2_0,
		check_hlds__mode_ordering__union_mode_vars_set_3_0_i17);
MR_def_label(check_hlds__mode_ordering__union_mode_vars_set_3_0,17)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(2);
	MR_r2 = MR_tempr1;
	MR_succip_word = MR_sv(9);
	MR_decr_sp(9);
	MR_np_tailcall_ent(fn__f_104_108_100_115_95_95_104_108_100_115_95_103_111_97_108_95_95_110_101_101_100_95_118_105_115_105_98_108_101_95_118_97_114_115_32_58_61_2_0);
	}
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_thread_engine_base
#endif
MR_END_MODULE

extern const MR_TypeCtorInfo_Struct mercury_data_hlds__inst_graph__type_ctor_info_node_0;
MR_decl_entry(__Unify___tree234__tree234_2_0);
MR_decl_entry(__Unify___stack__stack_1_0);
MR_decl_entry(__Unify___hlds__hlds_module__module_info_0_0);
extern const MR_TypeCtorInfo_Struct mercury_data_mdbcomp__goal_path__type_ctor_info_forward_goal_path_0;

MR_BEGIN_MODULE(check_hlds__mode_ordering_module19)
	MR_init_entry1(__Unify___check_hlds__mode_ordering__mode_ordering_info_0_0);
	MR_INIT_PROC_LAYOUT_ADDR(mercury____Unify___check_hlds__mode_ordering__mode_ordering_info_0_0);
	MR_init_label7(__Unify___check_hlds__mode_ordering__mode_ordering_info_0_0,5,9,11,13,15,19,1)
MR_BEGIN_CODE

/*-------------------------------------------------------------------------*/
/* code for '__Unify__'/2 mode 0 */
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_local_thread_engine_base
#endif
MR_def_static(__Unify___check_hlds__mode_ordering__mode_ordering_info_0_0)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if ((MR_r1 == MR_r2)) {
		MR_GOTO_LAB(__Unify___check_hlds__mode_ordering__mode_ordering_info_0_0_i19);
	}
	MR_incr_sp(12);
	MR_sv(12) = (MR_Word) MR_succip;
	MR_sv(1) = MR_r1;
	MR_sv(2) = MR_r2;
	{
	MR_Word MR_tempr1, MR_tempr2;
	MR_tempr1 = MR_sv(1);
	MR_sv(1) = MR_tfield(0, MR_tempr1, 1);
	MR_tempr2 = MR_sv(2);
	MR_sv(2) = MR_tfield(0, MR_tempr1, 2);
	MR_sv(3) = MR_tfield(0, MR_tempr1, 3);
	MR_sv(4) = MR_tfield(0, MR_tempr1, 4);
	MR_sv(5) = MR_tfield(0, MR_tempr1, 5);
	MR_sv(6) = MR_tfield(0, MR_tempr2, 1);
	MR_sv(7) = MR_tfield(0, MR_tempr2, 2);
	MR_sv(8) = MR_tfield(0, MR_tempr2, 3);
	MR_sv(9) = MR_tfield(0, MR_tempr2, 4);
	MR_sv(10) = MR_tfield(0, MR_tempr2, 5);
	MR_sv(11) = (MR_Word) MR_TAG_COMMON(0,1,2);
	MR_r1 = MR_sv(11);
	MR_r2 = (MR_Word) MR_CTOR0_ADDR(hlds__inst_graph, node);
	MR_r3 = MR_tfield(0, MR_tempr1, 0);
	MR_r4 = MR_tfield(0, MR_tempr2, 0);
	}
	MR_np_call_localret_ent(__Unify___tree234__tree234_2_0,
		__Unify___check_hlds__mode_ordering__mode_ordering_info_0_0_i5);
MR_def_label(__Unify___check_hlds__mode_ordering__mode_ordering_info_0_0,5)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (!(MR_r1)) {
		MR_GOTO_LAB(__Unify___check_hlds__mode_ordering__mode_ordering_info_0_0_i1);
	}
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_sv(1);
	MR_sv(1) = (MR_Word) MR_CTOR0_ADDR(mdbcomp__goal_path, goal_id);
	MR_r1 = (MR_Word) MR_TAG_COMMON(0,1,6);
	MR_r2 = (MR_Word) MR_TAG_COMMON(0,1,7);
	MR_r3 = MR_tempr1;
	MR_r4 = MR_sv(6);
	}
	MR_np_call_localret_ent(__Unify___tree234__tree234_2_0,
		__Unify___check_hlds__mode_ordering__mode_ordering_info_0_0_i9);
MR_def_label(__Unify___check_hlds__mode_ordering__mode_ordering_info_0_0,9)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (!(MR_r1)) {
		MR_GOTO_LAB(__Unify___check_hlds__mode_ordering__mode_ordering_info_0_0_i1);
	}
	MR_r1 = MR_sv(1);
	MR_r2 = MR_sv(2);
	MR_r3 = MR_sv(7);
	MR_np_call_localret_ent(__Unify___stack__stack_1_0,
		__Unify___check_hlds__mode_ordering__mode_ordering_info_0_0_i11);
MR_def_label(__Unify___check_hlds__mode_ordering__mode_ordering_info_0_0,11)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (!(MR_r1)) {
		MR_GOTO_LAB(__Unify___check_hlds__mode_ordering__mode_ordering_info_0_0_i1);
	}
	MR_r1 = MR_sv(3);
	MR_r2 = MR_sv(8);
	MR_np_call_localret_ent(__Unify___hlds__hlds_module__module_info_0_0,
		__Unify___check_hlds__mode_ordering__mode_ordering_info_0_0_i13);
MR_def_label(__Unify___check_hlds__mode_ordering__mode_ordering_info_0_0,13)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (!(MR_r1)) {
		MR_GOTO_LAB(__Unify___check_hlds__mode_ordering__mode_ordering_info_0_0_i1);
	}
	MR_r1 = (MR_Word) MR_CTOR0_ADDR(hlds__hlds_pred, pred_id);
	MR_r2 = (MR_Word) MR_CTOR0_ADDR(check_hlds__mode_ordering, pred_constraint_info);
	MR_r3 = MR_sv(4);
	MR_r4 = MR_sv(9);
	MR_np_call_localret_ent(__Unify___tree234__tree234_2_0,
		__Unify___check_hlds__mode_ordering__mode_ordering_info_0_0_i15);
MR_def_label(__Unify___check_hlds__mode_ordering__mode_ordering_info_0_0,15)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (!(MR_r1)) {
		MR_GOTO_LAB(__Unify___check_hlds__mode_ordering__mode_ordering_info_0_0_i1);
	}
	MR_r1 = (MR_Word) MR_CTOR0_ADDR(mdbcomp__goal_path, goal_id);
	MR_r2 = (MR_Word) MR_CTOR0_ADDR(mdbcomp__goal_path, forward_goal_path);
	MR_r3 = MR_sv(5);
	MR_r4 = MR_sv(10);
	MR_succip_word = MR_sv(12);
	MR_decr_sp(12);
	MR_np_tailcall_ent(__Unify___tree234__tree234_2_0);
MR_def_label(__Unify___check_hlds__mode_ordering__mode_ordering_info_0_0,19)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_r1 = MR_TRUE;
	MR_proceed();
MR_def_label(__Unify___check_hlds__mode_ordering__mode_ordering_info_0_0,1)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_r1 = MR_FALSE;
	MR_decr_sp_and_return(12);
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_thread_engine_base
#endif
MR_END_MODULE

MR_decl_entry(__Compare___tree234__tree234_2_0);
MR_decl_entry(__Compare___stack__stack_1_0);
MR_decl_entry(__Compare___hlds__hlds_module__module_info_0_0);

MR_BEGIN_MODULE(check_hlds__mode_ordering_module20)
	MR_init_entry1(__Compare___check_hlds__mode_ordering__mode_ordering_info_0_0);
	MR_INIT_PROC_LAYOUT_ADDR(mercury____Compare___check_hlds__mode_ordering__mode_ordering_info_0_0);
	MR_init_label8(__Compare___check_hlds__mode_ordering__mode_ordering_info_0_0,3,2,6,13,17,21,25,75)
MR_BEGIN_CODE

/*-------------------------------------------------------------------------*/
/* code for '__Compare__'/3 mode 0 */
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_local_thread_engine_base
#endif
MR_def_static(__Compare___check_hlds__mode_ordering__mode_ordering_info_0_0)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if ((MR_r1 == MR_r2)) {
		MR_GOTO_LAB(__Compare___check_hlds__mode_ordering__mode_ordering_info_0_0_i3);
	}
	MR_incr_sp(11);
	MR_sv(11) = (MR_Word) MR_succip;
	MR_sv(1) = MR_r1;
	MR_sv(2) = MR_r2;
	MR_GOTO_LAB(__Compare___check_hlds__mode_ordering__mode_ordering_info_0_0_i2);
MR_def_label(__Compare___check_hlds__mode_ordering__mode_ordering_info_0_0,3)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_r1 = (MR_Integer) 0;
	MR_proceed();
MR_def_label(__Compare___check_hlds__mode_ordering__mode_ordering_info_0_0,2)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1, MR_tempr2, MR_tempr3, MR_tempr4, MR_tempr5, MR_tempr6;
	MR_tempr5 = MR_sv(2);
	MR_sv(10) = MR_tfield(0, MR_tempr5, 5);
	MR_sv(9) = MR_tfield(0, MR_tempr5, 4);
	MR_sv(8) = MR_tfield(0, MR_tempr5, 3);
	MR_sv(7) = MR_tfield(0, MR_tempr5, 2);
	MR_sv(6) = MR_tfield(0, MR_tempr5, 1);
	MR_tempr6 = MR_sv(1);
	MR_sv(5) = MR_tfield(0, MR_tempr6, 5);
	MR_sv(4) = MR_tfield(0, MR_tempr6, 4);
	MR_sv(3) = MR_tfield(0, MR_tempr6, 3);
	MR_tempr1 = MR_tfield(0, MR_tempr6, 2);
	MR_tempr2 = MR_tfield(0, MR_tempr6, 1);
	MR_tempr3 = MR_tempr6;
	MR_sv(1) = MR_tempr2;
	MR_tempr4 = MR_tempr5;
	MR_sv(2) = MR_tempr1;
	MR_r1 = (MR_Word) MR_TAG_COMMON(0,1,2);
	MR_r2 = (MR_Word) MR_CTOR0_ADDR(hlds__inst_graph, node);
	MR_r3 = MR_tfield(0, MR_tempr3, 0);
	MR_r4 = MR_tfield(0, MR_tempr4, 0);
	}
	MR_np_call_localret_ent(__Compare___tree234__tree234_2_0,
		__Compare___check_hlds__mode_ordering__mode_ordering_info_0_0_i6);
MR_def_label(__Compare___check_hlds__mode_ordering__mode_ordering_info_0_0,6)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (MR_INT_NE(MR_r1,0)) {
		MR_GOTO_LAB(__Compare___check_hlds__mode_ordering__mode_ordering_info_0_0_i75);
	}
	MR_r1 = (MR_Word) MR_TAG_COMMON(0,1,6);
	MR_r2 = (MR_Word) MR_TAG_COMMON(0,1,7);
	MR_r3 = MR_sv(1);
	MR_r4 = MR_sv(6);
	MR_np_call_localret_ent(__Compare___tree234__tree234_2_0,
		__Compare___check_hlds__mode_ordering__mode_ordering_info_0_0_i13);
MR_def_label(__Compare___check_hlds__mode_ordering__mode_ordering_info_0_0,13)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (MR_INT_NE(MR_r1,0)) {
		MR_GOTO_LAB(__Compare___check_hlds__mode_ordering__mode_ordering_info_0_0_i75);
	}
	MR_r1 = (MR_Word) MR_CTOR0_ADDR(mdbcomp__goal_path, goal_id);
	MR_r2 = MR_sv(2);
	MR_r3 = MR_sv(7);
	MR_np_call_localret_ent(__Compare___stack__stack_1_0,
		__Compare___check_hlds__mode_ordering__mode_ordering_info_0_0_i17);
MR_def_label(__Compare___check_hlds__mode_ordering__mode_ordering_info_0_0,17)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (MR_INT_NE(MR_r1,0)) {
		MR_GOTO_LAB(__Compare___check_hlds__mode_ordering__mode_ordering_info_0_0_i75);
	}
	MR_r1 = MR_sv(3);
	MR_r2 = MR_sv(8);
	MR_np_call_localret_ent(__Compare___hlds__hlds_module__module_info_0_0,
		__Compare___check_hlds__mode_ordering__mode_ordering_info_0_0_i21);
MR_def_label(__Compare___check_hlds__mode_ordering__mode_ordering_info_0_0,21)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (MR_INT_NE(MR_r1,0)) {
		MR_GOTO_LAB(__Compare___check_hlds__mode_ordering__mode_ordering_info_0_0_i75);
	}
	MR_r1 = (MR_Word) MR_CTOR0_ADDR(hlds__hlds_pred, pred_id);
	MR_r2 = (MR_Word) MR_CTOR0_ADDR(check_hlds__mode_ordering, pred_constraint_info);
	MR_r3 = MR_sv(4);
	MR_r4 = MR_sv(9);
	MR_np_call_localret_ent(__Compare___tree234__tree234_2_0,
		__Compare___check_hlds__mode_ordering__mode_ordering_info_0_0_i25);
MR_def_label(__Compare___check_hlds__mode_ordering__mode_ordering_info_0_0,25)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (MR_INT_NE(MR_r1,0)) {
		MR_GOTO_LAB(__Compare___check_hlds__mode_ordering__mode_ordering_info_0_0_i75);
	}
	MR_r1 = (MR_Word) MR_CTOR0_ADDR(mdbcomp__goal_path, goal_id);
	MR_r2 = (MR_Word) MR_CTOR0_ADDR(mdbcomp__goal_path, forward_goal_path);
	MR_r3 = MR_sv(5);
	MR_r4 = MR_sv(10);
	MR_succip_word = MR_sv(11);
	MR_decr_sp(11);
	MR_np_tailcall_ent(__Compare___tree234__tree234_2_0);
MR_def_label(__Compare___check_hlds__mode_ordering__mode_ordering_info_0_0,75)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_decr_sp_and_return(11);
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_thread_engine_base
#endif
MR_END_MODULE

MR_decl_entry(__Unify___mode_robdd__tfeirn__tfeirn_1_0);
MR_decl_entry(__Unify___check_hlds__mode_constraint_robdd__mode_constraint_info_0_0);

MR_BEGIN_MODULE(check_hlds__mode_ordering_module21)
	MR_init_entry1(__Unify___check_hlds__mode_ordering__pred_constraint_info_0_0);
	MR_INIT_PROC_LAYOUT_ADDR(mercury____Unify___check_hlds__mode_ordering__pred_constraint_info_0_0);
	MR_init_label3(__Unify___check_hlds__mode_ordering__pred_constraint_info_0_0,4,8,1)
MR_BEGIN_CODE

/*-------------------------------------------------------------------------*/
/* code for '__Unify__'/2 mode 0 */
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_local_thread_engine_base
#endif
MR_define_entry(mercury____Unify___check_hlds__mode_ordering__pred_constraint_info_0_0);
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if ((MR_r1 == MR_r2)) {
		MR_GOTO_LAB(__Unify___check_hlds__mode_ordering__pred_constraint_info_0_0_i8);
	}
	MR_incr_sp(3);
	MR_sv(3) = (MR_Word) MR_succip;
	MR_sv(1) = MR_r1;
	MR_sv(2) = MR_r2;
	{
	MR_Word MR_tempr1, MR_tempr2;
	MR_tempr1 = MR_sv(1);
	MR_sv(1) = MR_tfield(0, MR_tempr1, 1);
	MR_tempr2 = MR_sv(2);
	MR_sv(2) = MR_tfield(0, MR_tempr2, 1);
	MR_r1 = (MR_Word) MR_CTOR0_ADDR(check_hlds__mode_constraint_robdd, mc_type);
	MR_r2 = MR_tfield(0, MR_tempr1, 0);
	MR_r3 = MR_tfield(0, MR_tempr2, 0);
	}
	MR_np_call_localret_ent(__Unify___mode_robdd__tfeirn__tfeirn_1_0,
		__Unify___check_hlds__mode_ordering__pred_constraint_info_0_0_i4);
MR_def_label(__Unify___check_hlds__mode_ordering__pred_constraint_info_0_0,4)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (!(MR_r1)) {
		MR_GOTO_LAB(__Unify___check_hlds__mode_ordering__pred_constraint_info_0_0_i1);
	}
	MR_r1 = MR_sv(1);
	MR_r2 = MR_sv(2);
	MR_succip_word = MR_sv(3);
	MR_decr_sp(3);
	MR_np_tailcall_ent(__Unify___check_hlds__mode_constraint_robdd__mode_constraint_info_0_0);
MR_def_label(__Unify___check_hlds__mode_ordering__pred_constraint_info_0_0,8)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_r1 = MR_TRUE;
	MR_proceed();
MR_def_label(__Unify___check_hlds__mode_ordering__pred_constraint_info_0_0,1)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_r1 = MR_FALSE;
	MR_decr_sp_and_return(3);
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_thread_engine_base
#endif
MR_END_MODULE

MR_decl_entry(__Compare___mode_robdd__tfeirn__tfeirn_1_0);
MR_decl_entry(__Compare___check_hlds__mode_constraint_robdd__mode_constraint_info_0_0);

MR_BEGIN_MODULE(check_hlds__mode_ordering_module22)
	MR_init_entry1(__Compare___check_hlds__mode_ordering__pred_constraint_info_0_0);
	MR_INIT_PROC_LAYOUT_ADDR(mercury____Compare___check_hlds__mode_ordering__pred_constraint_info_0_0);
	MR_init_label4(__Compare___check_hlds__mode_ordering__pred_constraint_info_0_0,3,2,5,21)
MR_BEGIN_CODE

/*-------------------------------------------------------------------------*/
/* code for '__Compare__'/3 mode 0 */
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_local_thread_engine_base
#endif
MR_define_entry(mercury____Compare___check_hlds__mode_ordering__pred_constraint_info_0_0);
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if ((MR_r1 == MR_r2)) {
		MR_GOTO_LAB(__Compare___check_hlds__mode_ordering__pred_constraint_info_0_0_i3);
	}
	MR_incr_sp(3);
	MR_sv(3) = (MR_Word) MR_succip;
	MR_sv(1) = MR_r1;
	MR_sv(2) = MR_r2;
	MR_GOTO_LAB(__Compare___check_hlds__mode_ordering__pred_constraint_info_0_0_i2);
MR_def_label(__Compare___check_hlds__mode_ordering__pred_constraint_info_0_0,3)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_r1 = (MR_Integer) 0;
	MR_proceed();
MR_def_label(__Compare___check_hlds__mode_ordering__pred_constraint_info_0_0,2)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1, MR_tempr2, MR_tempr3, MR_tempr4, MR_tempr5, MR_tempr6;
	MR_tempr5 = MR_sv(2);
	MR_tempr1 = MR_tfield(0, MR_tempr5, 1);
	MR_tempr6 = MR_sv(1);
	MR_tempr2 = MR_tfield(0, MR_tempr6, 1);
	MR_tempr3 = MR_tempr6;
	MR_sv(1) = MR_tempr2;
	MR_tempr4 = MR_tempr5;
	MR_sv(2) = MR_tempr1;
	MR_r1 = (MR_Word) MR_CTOR0_ADDR(check_hlds__mode_constraint_robdd, mc_type);
	MR_r2 = MR_tfield(0, MR_tempr3, 0);
	MR_r3 = MR_tfield(0, MR_tempr4, 0);
	}
	MR_np_call_localret_ent(__Compare___mode_robdd__tfeirn__tfeirn_1_0,
		__Compare___check_hlds__mode_ordering__pred_constraint_info_0_0_i5);
MR_def_label(__Compare___check_hlds__mode_ordering__pred_constraint_info_0_0,5)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (MR_INT_NE(MR_r1,0)) {
		MR_GOTO_LAB(__Compare___check_hlds__mode_ordering__pred_constraint_info_0_0_i21);
	}
	MR_r1 = MR_sv(1);
	MR_r2 = MR_sv(2);
	MR_succip_word = MR_sv(3);
	MR_decr_sp(3);
	MR_np_tailcall_ent(__Compare___check_hlds__mode_constraint_robdd__mode_constraint_info_0_0);
MR_def_label(__Compare___check_hlds__mode_ordering__pred_constraint_info_0_0,21)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_decr_sp_and_return(3);
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_thread_engine_base
#endif
MR_END_MODULE


MR_BEGIN_MODULE(check_hlds__mode_ordering_module23)
	MR_init_entry1(__Unify___check_hlds__mode_ordering__pred_constraint_map_0_0);
	MR_INIT_PROC_LAYOUT_ADDR(mercury____Unify___check_hlds__mode_ordering__pred_constraint_map_0_0);
MR_BEGIN_CODE

/*-------------------------------------------------------------------------*/
/* code for '__Unify__'/2 mode 0 */
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_local_thread_engine_base
#endif
MR_define_entry(mercury____Unify___check_hlds__mode_ordering__pred_constraint_map_0_0);
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1, MR_tempr2;
	MR_tempr1 = MR_r1;
	MR_r1 = (MR_Word) MR_CTOR0_ADDR(hlds__hlds_pred, pred_id);
	MR_tempr2 = MR_r2;
	MR_r2 = (MR_Word) MR_CTOR0_ADDR(check_hlds__mode_ordering, pred_constraint_info);
	MR_r3 = MR_tempr1;
	MR_r4 = MR_tempr2;
	MR_np_tailcall_ent(__Unify___tree234__tree234_2_0);
	}
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_thread_engine_base
#endif
MR_END_MODULE


MR_BEGIN_MODULE(check_hlds__mode_ordering_module24)
	MR_init_entry1(__Compare___check_hlds__mode_ordering__pred_constraint_map_0_0);
	MR_INIT_PROC_LAYOUT_ADDR(mercury____Compare___check_hlds__mode_ordering__pred_constraint_map_0_0);
MR_BEGIN_CODE

/*-------------------------------------------------------------------------*/
/* code for '__Compare__'/3 mode 0 */
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_local_thread_engine_base
#endif
MR_define_entry(mercury____Compare___check_hlds__mode_ordering__pred_constraint_map_0_0);
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1, MR_tempr2;
	MR_tempr1 = MR_r1;
	MR_r1 = (MR_Word) MR_CTOR0_ADDR(hlds__hlds_pred, pred_id);
	MR_tempr2 = MR_r2;
	MR_r2 = (MR_Word) MR_CTOR0_ADDR(check_hlds__mode_ordering, pred_constraint_info);
	MR_r3 = MR_tempr1;
	MR_r4 = MR_tempr2;
	MR_np_tailcall_ent(__Compare___tree234__tree234_2_0);
	}
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_thread_engine_base
#endif
MR_END_MODULE

MR_declare_entry(MR_do_fail);
MR_decl_entry(hlds__inst_graph__same_graph_corresponding_nodes_5_0);

MR_BEGIN_MODULE(check_hlds__mode_ordering_module25)
	MR_init_entry1(check_hlds__mode_ordering__IntroducedFrom__pred__mode_order_goal_2__251__1_5_0);
	MR_INIT_PROC_LAYOUT_ADDR(mercury__check_hlds__mode_ordering__IntroducedFrom__pred__mode_order_goal_2__251__1_5_0);
	MR_init_label4(check_hlds__mode_ordering__IntroducedFrom__pred__mode_order_goal_2__251__1_5_0,1,4,2,7)
MR_BEGIN_CODE

/*-------------------------------------------------------------------------*/
/* code for 'IntroducedFrom__pred__mode_order_goal_2__251__1'/5 mode 0 */
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_local_thread_engine_base
#endif
MR_def_static(check_hlds__mode_ordering__IntroducedFrom__pred__mode_order_goal_2__251__1_5_0)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_mkframe("pred check_hlds.mode_ordering.IntroducedFrom__pred__mode_order_goal_2__251__1/5-0", 3,
		MR_ENTRY(MR_do_fail));
	MR_fv(3) = MR_r4;
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_r2;
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(hlds__inst_graph__same_graph_corresponding_nodes_5_0,
		check_hlds__mode_ordering__IntroducedFrom__pred__mode_order_goal_2__251__1_5_0_i1);
MR_def_label(check_hlds__mode_ordering__IntroducedFrom__pred__mode_order_goal_2__251__1_5_0,1)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_fv(1) = MR_r1;
	MR_fv(2) = MR_r2;
	MR_r1 = (MR_Word) MR_CTOR0_ADDR(parse_tree__prog_data, prog_var_type);
	MR_r2 = (MR_Word) MR_TAG_COMMON(0,1,2);
	MR_r3 = MR_fv(3);
	MR_r4 = MR_fv(1);
	MR_np_call_localret_ent(f_115_101_116_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_95_99_111_110_116_97_105_110_115_95_95_91_84_32_61_32_118_97_114_40_86_95_50_41_93_95_48_95_49_2_0,
		check_hlds__mode_ordering__IntroducedFrom__pred__mode_order_goal_2__251__1_5_0_i4);
MR_def_label(check_hlds__mode_ordering__IntroducedFrom__pred__mode_order_goal_2__251__1_5_0,4)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (!(MR_r1)) {
		MR_GOTO_LAB(check_hlds__mode_ordering__IntroducedFrom__pred__mode_order_goal_2__251__1_5_0_i2);
	}
	MR_r1 = MR_fv(2);
	MR_succeed();
MR_def_label(check_hlds__mode_ordering__IntroducedFrom__pred__mode_order_goal_2__251__1_5_0,2)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_r1 = (MR_Word) MR_CTOR0_ADDR(parse_tree__prog_data, prog_var_type);
	MR_r2 = (MR_Word) MR_TAG_COMMON(0,1,2);
	MR_r3 = MR_fv(3);
	MR_r4 = MR_fv(2);
	MR_np_call_localret_ent(f_115_101_116_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_95_99_111_110_116_97_105_110_115_95_95_91_84_32_61_32_118_97_114_40_86_95_50_41_93_95_48_95_49_2_0,
		check_hlds__mode_ordering__IntroducedFrom__pred__mode_order_goal_2__251__1_5_0_i7);
MR_def_label(check_hlds__mode_ordering__IntroducedFrom__pred__mode_order_goal_2__251__1_5_0,7)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (!(MR_r1)) {
		MR_GOTO(MR_ENTRY(MR_do_redo));
	}
	MR_r1 = MR_fv(1);
	MR_succeed();
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_thread_engine_base
#endif
MR_END_MODULE

extern const MR_TypeCtorInfo_Struct mercury_data_mdbcomp__goal_path__type_ctor_info_goal_path_step_0;
MR_decl_entry(list__last_2_0);
MR_decl_entry(fn__map__det_insert_3_0);

MR_BEGIN_MODULE(check_hlds__mode_ordering_module26)
	MR_init_entry1(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__423__1_3_0);
	MR_INIT_PROC_LAYOUT_ADDR(mercury__fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__423__1_3_0);
	MR_init_label5(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__423__1_3_0,2,3,5,4,9)
MR_BEGIN_CODE

/*-------------------------------------------------------------------------*/
/* code for 'IntroducedFrom__func__mode_order_conj__423__1'/4 mode 0 */
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_local_thread_engine_base
#endif
MR_def_static(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__423__1_3_0)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_incr_sp(4);
	MR_sv(4) = (MR_Word) MR_succip;
	MR_sv(1) = MR_r1;
	MR_sv(2) = MR_r2;
	MR_sv(3) = MR_r3;
	MR_r1 = MR_tfield(0, MR_r2, 1);
	MR_np_call_localret_ent(fn__hlds__hlds_goal__goal_info_get_goal_id_1_0,
		fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__423__1_3_0_i2);
MR_def_label(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__423__1_3_0,2)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = (MR_Word) MR_CTOR0_ADDR(mdbcomp__goal_path, goal_id);
	MR_r2 = (MR_Word) MR_CTOR0_ADDR(mdbcomp__goal_path, forward_goal_path);
	MR_r3 = MR_sv(1);
	MR_r4 = MR_tempr1;
	}
	MR_np_call_localret_ent(map__lookup_3_0,
		fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__423__1_3_0_i3);
MR_def_label(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__423__1_3_0,3)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = (MR_Word) MR_CTOR0_ADDR(mdbcomp__goal_path, goal_path_step);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(list__last_2_0,
		fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__423__1_3_0_i5);
MR_def_label(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__423__1_3_0,5)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (!(MR_r1)) {
		MR_GOTO_LAB(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__423__1_3_0_i4);
	}
	if (MR_PTAG_TESTR(MR_r2,1)) {
		MR_GOTO_LAB(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__423__1_3_0_i4);
	}
	MR_r4 = MR_tfield(1, MR_r2, 0);
	MR_r5 = MR_sv(2);
	MR_r3 = MR_sv(3);
	MR_r1 = (MR_Word) MR_INT_CTOR_ADDR;
	MR_r2 = (MR_Word) MR_CTOR0_ADDR(hlds__hlds_goal, hlds_goal);
	MR_succip_word = MR_sv(4);
	MR_decr_sp(4);
	MR_np_tailcall_ent(fn__map__det_insert_3_0);
MR_def_label(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__423__1_3_0,4)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_r1 = (MR_Word) MR_string_const("check_hlds.mode_ordering", 24);
	MR_r2 = (MR_Word) MR_string_const("predicate \140check_hlds.mode_ordering.mode_order_conj\'/3", 54);
	MR_r3 = (MR_Word) MR_string_const("goal_path error", 15);
	MR_np_call_localret_ent(require__unexpected_3_0,
		fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__423__1_3_0_i9);
MR_def_label(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__423__1_3_0,9)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_succip_word = MR_sv(4);
	MR_decr_sp(4);
	MR_np_tailcall_ent(fn__map__det_insert_3_0);
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_thread_engine_base
#endif
MR_END_MODULE


MR_BEGIN_MODULE(check_hlds__mode_ordering_module27)
	MR_init_entry1(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__439__1_3_0);
	MR_INIT_PROC_LAYOUT_ADDR(mercury__fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__439__1_3_0);
MR_BEGIN_CODE

/*-------------------------------------------------------------------------*/
/* code for 'IntroducedFrom__func__mode_order_conj__439__1'/4 mode 0 */
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_local_thread_engine_base
#endif
MR_def_static(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__439__1_3_0)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1, MR_tempr2;
	MR_tempr1 = MR_r1;
	MR_r1 = (MR_Word) MR_TAG_COMMON(0,1,2);
	MR_tempr2 = MR_r2;
	MR_r2 = (MR_Word) MR_INT_CTOR_ADDR;
	MR_r4 = MR_tempr2;
	MR_r5 = MR_tempr1;
	MR_np_tailcall_ent(fn__map__det_insert_3_0);
	}
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_thread_engine_base
#endif
MR_END_MODULE

MR_decl_entry(fn__set__to_sorted_list_1_0);

MR_BEGIN_MODULE(check_hlds__mode_ordering_module28)
	MR_init_entry1(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__438__1_3_0);
	MR_INIT_PROC_LAYOUT_ADDR(mercury__fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__438__1_3_0);
	MR_init_label2(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__438__1_3_0,5,6)
MR_BEGIN_CODE

/*-------------------------------------------------------------------------*/
/* code for 'IntroducedFrom__func__mode_order_conj__438__1'/4 mode 0 */
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_local_thread_engine_base
#endif
MR_def_static(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__438__1_3_0)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_incr_sp(5);
	MR_sv(5) = (MR_Word) MR_succip;
	{
	MR_Word MR_tempr1;
	MR_tag_alloc_heap(MR_tempr1, 0, (MR_Integer) 4);
	MR_sv(2) = MR_tempr1;
	MR_tfield(0, MR_tempr1, 0) = (MR_Word) MR_COMMON(2,7);
	MR_tfield(0, MR_tempr1, 1) = (MR_Word) MR_ENTRY_AP(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__439__1_3_0);
	MR_tfield(0, MR_tempr1, 2) = (MR_Integer) 1;
	MR_tfield(0, MR_tempr1, 3) = MR_r1;
	MR_sv(1) = MR_r3;
	MR_sv(3) = (MR_Word) MR_TAG_COMMON(0,1,2);
	MR_sv(4) = (MR_Word) MR_TAG_COMMON(0,0,4);
	MR_r1 = MR_tfield(0, MR_r2, 1);
	}
	MR_np_call_localret_ent(fn__hlds__hlds_goal__producing_vars_1_0,
		fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__438__1_3_0_i5);
MR_def_label(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__438__1_3_0,5)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(3);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__set__to_sorted_list_1_0,
		fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__438__1_3_0_i6);
MR_def_label(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__438__1_3_0,6)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(3);
	MR_r2 = MR_sv(4);
	MR_r3 = MR_sv(2);
	MR_r4 = MR_tempr1;
	MR_r5 = MR_sv(1);
	MR_succip_word = MR_sv(5);
	MR_decr_sp(5);
	MR_np_tailcall_ent(fn__list__foldl_3_0);
	}
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_thread_engine_base
#endif
MR_END_MODULE

MR_decl_entry(fn__map__set_3_0);

MR_BEGIN_MODULE(check_hlds__mode_ordering_module29)
	MR_init_entry1(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__445__1_3_0);
	MR_INIT_PROC_LAYOUT_ADDR(mercury__fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__445__1_3_0);
MR_BEGIN_CODE

/*-------------------------------------------------------------------------*/
/* code for 'IntroducedFrom__func__mode_order_conj__445__1'/4 mode 0 */
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_local_thread_engine_base
#endif
MR_def_static(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__445__1_3_0)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1, MR_tempr2;
	MR_tempr1 = MR_r1;
	MR_r1 = (MR_Word) MR_TAG_COMMON(0,1,2);
	MR_tempr2 = MR_r2;
	MR_r2 = (MR_Word) MR_INT_CTOR_ADDR;
	MR_r4 = MR_tempr2;
	MR_r5 = MR_tempr1;
	MR_np_tailcall_ent(fn__map__set_3_0);
	}
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_thread_engine_base
#endif
MR_END_MODULE


MR_BEGIN_MODULE(check_hlds__mode_ordering_module30)
	MR_init_entry1(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__444__1_3_0);
	MR_INIT_PROC_LAYOUT_ADDR(mercury__fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__444__1_3_0);
	MR_init_label2(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__444__1_3_0,5,6)
MR_BEGIN_CODE

/*-------------------------------------------------------------------------*/
/* code for 'IntroducedFrom__func__mode_order_conj__444__1'/4 mode 0 */
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_local_thread_engine_base
#endif
MR_def_static(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__444__1_3_0)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_incr_sp(5);
	MR_sv(5) = (MR_Word) MR_succip;
	{
	MR_Word MR_tempr1;
	MR_tag_alloc_heap(MR_tempr1, 0, (MR_Integer) 4);
	MR_sv(2) = MR_tempr1;
	MR_tfield(0, MR_tempr1, 0) = (MR_Word) MR_COMMON(2,8);
	MR_tfield(0, MR_tempr1, 1) = (MR_Word) MR_ENTRY_AP(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__445__1_3_0);
	MR_tfield(0, MR_tempr1, 2) = (MR_Integer) 1;
	MR_tfield(0, MR_tempr1, 3) = MR_r1;
	MR_sv(1) = MR_r3;
	MR_sv(3) = (MR_Word) MR_TAG_COMMON(0,1,2);
	MR_sv(4) = (MR_Word) MR_TAG_COMMON(0,0,4);
	MR_r1 = MR_tfield(0, MR_r2, 1);
	}
	MR_np_call_localret_ent(fn__hlds__hlds_goal__make_visible_vars_1_0,
		fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__444__1_3_0_i5);
MR_def_label(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__444__1_3_0,5)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(3);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__set__to_sorted_list_1_0,
		fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__444__1_3_0_i6);
MR_def_label(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__444__1_3_0,6)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(3);
	MR_r2 = MR_sv(4);
	MR_r3 = MR_sv(2);
	MR_r4 = MR_tempr1;
	MR_r5 = MR_sv(1);
	MR_succip_word = MR_sv(5);
	MR_decr_sp(5);
	MR_np_tailcall_ent(fn__list__foldl_3_0);
	}
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_thread_engine_base
#endif
MR_END_MODULE

MR_decl_entry(fn__f_109_97_112_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_111_114_95_102_117_110_99_95_95_115_101_97_114_99_104_95_95_91_75_32_61_32_118_97_114_40_86_95_50_41_93_95_48_95_49_2_0);
MR_decl_entry(digraph__add_vertex_4_0);
MR_decl_entry(digraph__add_edge_4_0);

MR_BEGIN_MODULE(check_hlds__mode_ordering_module31)
	MR_init_entry1(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__457__1_4_0);
	MR_INIT_PROC_LAYOUT_ADDR(mercury__fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__457__1_4_0);
	MR_init_label3(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__457__1_4_0,4,6,2)
MR_BEGIN_CODE

/*-------------------------------------------------------------------------*/
/* code for 'IntroducedFrom__func__mode_order_conj__457__1'/5 mode 0 */
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_local_thread_engine_base
#endif
MR_def_static(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__457__1_4_0)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_incr_sp(3);
	MR_sv(3) = (MR_Word) MR_succip;
	MR_sv(1) = MR_r2;
	MR_sv(2) = MR_r4;
	{
	MR_Word MR_tempr1, MR_tempr2;
	MR_tempr1 = MR_r1;
	MR_r1 = (MR_Word) MR_CTOR0_ADDR(parse_tree__prog_data, prog_var_type);
	MR_r2 = (MR_Word) MR_TAG_COMMON(0,1,2);
	MR_tempr2 = MR_r3;
	MR_r3 = (MR_Word) MR_INT_CTOR_ADDR;
	MR_r4 = MR_tempr1;
	MR_r5 = MR_tempr2;
	}
	MR_np_call_localret_ent(fn__f_109_97_112_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_111_114_95_102_117_110_99_95_95_115_101_97_114_99_104_95_95_91_75_32_61_32_118_97_114_40_86_95_50_41_93_95_48_95_49_2_0,
		fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__457__1_4_0_i4);
MR_def_label(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__457__1_4_0,4)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (!(MR_r1)) {
		MR_GOTO_LAB(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__457__1_4_0_i2);
	}
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_sv(2);
	MR_sv(2) = (MR_Word) MR_INT_CTOR_ADDR;
	MR_r1 = MR_sv(2);
	MR_r3 = MR_tempr1;
	}
	MR_np_call_localret_ent(digraph__add_vertex_4_0,
		fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__457__1_4_0_i6);
MR_def_label(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__457__1_4_0,6)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1, MR_tempr2;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(2);
	MR_tempr2 = MR_r2;
	MR_r2 = MR_tempr1;
	MR_r3 = MR_sv(1);
	MR_r4 = MR_tempr2;
	MR_succip_word = MR_sv(3);
	MR_decr_sp(3);
	MR_np_tailcall_ent(digraph__add_edge_4_0);
	}
MR_def_label(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__457__1_4_0,2)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_r1 = MR_sv(2);
	MR_decr_sp_and_return(3);
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_thread_engine_base
#endif
MR_END_MODULE


MR_BEGIN_MODULE(check_hlds__mode_ordering_module32)
	MR_init_entry1(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__465__1_4_0);
	MR_INIT_PROC_LAYOUT_ADDR(mercury__fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__465__1_4_0);
	MR_init_label3(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__465__1_4_0,4,6,2)
MR_BEGIN_CODE

/*-------------------------------------------------------------------------*/
/* code for 'IntroducedFrom__func__mode_order_conj__465__1'/5 mode 0 */
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_local_thread_engine_base
#endif
MR_def_static(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__465__1_4_0)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_incr_sp(3);
	MR_sv(3) = (MR_Word) MR_succip;
	MR_sv(1) = MR_r2;
	MR_sv(2) = MR_r4;
	{
	MR_Word MR_tempr1, MR_tempr2;
	MR_tempr1 = MR_r1;
	MR_r1 = (MR_Word) MR_CTOR0_ADDR(parse_tree__prog_data, prog_var_type);
	MR_r2 = (MR_Word) MR_TAG_COMMON(0,1,2);
	MR_tempr2 = MR_r3;
	MR_r3 = (MR_Word) MR_INT_CTOR_ADDR;
	MR_r4 = MR_tempr1;
	MR_r5 = MR_tempr2;
	}
	MR_np_call_localret_ent(fn__f_109_97_112_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_111_114_95_102_117_110_99_95_95_115_101_97_114_99_104_95_95_91_75_32_61_32_118_97_114_40_86_95_50_41_93_95_48_95_49_2_0,
		fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__465__1_4_0_i4);
MR_def_label(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__465__1_4_0,4)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (!(MR_r1)) {
		MR_GOTO_LAB(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__465__1_4_0_i2);
	}
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_sv(2);
	MR_sv(2) = (MR_Word) MR_INT_CTOR_ADDR;
	MR_r1 = MR_sv(2);
	MR_r3 = MR_tempr1;
	}
	MR_np_call_localret_ent(digraph__add_vertex_4_0,
		fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__465__1_4_0_i6);
MR_def_label(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__465__1_4_0,6)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1, MR_tempr2;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(2);
	MR_tempr2 = MR_r2;
	MR_r2 = MR_tempr1;
	MR_r3 = MR_sv(1);
	MR_r4 = MR_tempr2;
	MR_succip_word = MR_sv(3);
	MR_decr_sp(3);
	MR_np_tailcall_ent(digraph__add_edge_4_0);
	}
MR_def_label(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__465__1_4_0,2)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_r1 = MR_sv(2);
	MR_decr_sp_and_return(3);
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_thread_engine_base
#endif
MR_END_MODULE


MR_BEGIN_MODULE(check_hlds__mode_ordering_module33)
	MR_init_entry1(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__450__1_5_0);
	MR_INIT_PROC_LAYOUT_ADDR(mercury__fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__450__1_5_0);
	MR_init_label6(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__450__1_5_0,2,6,7,8,10,11)
MR_BEGIN_CODE

/*-------------------------------------------------------------------------*/
/* code for 'IntroducedFrom__func__mode_order_conj__450__1'/6 mode 0 */
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_local_thread_engine_base
#endif
MR_def_static(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__450__1_5_0)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_incr_sp(8);
	MR_sv(8) = (MR_Word) MR_succip;
	MR_sv(1) = MR_r1;
	MR_sv(2) = MR_r2;
	MR_sv(5) = MR_tfield(0, MR_r4, 1);
	MR_sv(3) = (MR_Word) MR_INT_CTOR_ADDR;
	MR_r1 = MR_sv(3);
	MR_r2 = MR_r3;
	MR_r3 = MR_r5;
	MR_np_call_localret_ent(digraph__add_vertex_4_0,
		fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__450__1_5_0_i2);
MR_def_label(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__450__1_5_0,2)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tag_alloc_heap(MR_tempr1, 0, (MR_Integer) 5);
	MR_sv(4) = MR_tempr1;
	MR_tfield(0, MR_tempr1, 0) = (MR_Word) MR_COMMON(3,4);
	MR_tfield(0, MR_tempr1, 1) = (MR_Word) MR_ENTRY_AP(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__457__1_4_0);
	MR_tfield(0, MR_tempr1, 2) = (MR_Integer) 2;
	MR_tfield(0, MR_tempr1, 3) = MR_sv(1);
	MR_tfield(0, MR_tempr1, 4) = MR_r1;
	MR_sv(1) = MR_r1;
	MR_sv(3) = MR_r2;
	MR_sv(6) = (MR_Word) MR_TAG_COMMON(0,1,2);
	MR_sv(7) = (MR_Word) MR_TAG_COMMON(0,1,4);
	MR_r1 = MR_sv(5);
	}
	MR_np_call_localret_ent(fn__hlds__hlds_goal__consuming_vars_1_0,
		fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__450__1_5_0_i6);
MR_def_label(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__450__1_5_0,6)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(6);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__set__to_sorted_list_1_0,
		fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__450__1_5_0_i7);
MR_def_label(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__450__1_5_0,7)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(6);
	MR_r2 = MR_sv(7);
	MR_r3 = MR_sv(4);
	MR_r4 = MR_tempr1;
	MR_r5 = MR_sv(3);
	}
	MR_np_call_localret_ent(fn__list__foldl_3_0,
		fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__450__1_5_0_i8);
MR_def_label(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__450__1_5_0,8)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_tag_alloc_heap(MR_r2, 0, (MR_Integer) 5);
	MR_tfield(0, MR_r2, 0) = (MR_Word) MR_COMMON(3,5);
	MR_tfield(0, MR_r2, 1) = (MR_Word) MR_ENTRY_AP(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__465__1_4_0);
	MR_tfield(0, MR_r2, 2) = (MR_Integer) 2;
	MR_tfield(0, MR_r2, 3) = MR_sv(2);
	MR_tfield(0, MR_r2, 4) = MR_sv(1);
	MR_sv(1) = MR_r1;
	MR_sv(2) = MR_r2;
	MR_r1 = MR_sv(5);
	MR_np_call_localret_ent(fn__hlds__hlds_goal__need_visible_vars_1_0,
		fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__450__1_5_0_i10);
MR_def_label(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__450__1_5_0,10)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(6);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__set__to_sorted_list_1_0,
		fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__450__1_5_0_i11);
MR_def_label(fn__check_hlds__mode_ordering__IntroducedFrom__func__mode_order_conj__450__1_5_0,11)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(6);
	MR_r2 = MR_sv(7);
	MR_r3 = MR_sv(2);
	MR_r4 = MR_tempr1;
	MR_r5 = MR_sv(1);
	MR_succip_word = MR_sv(8);
	MR_decr_sp(8);
	MR_np_tailcall_ent(fn__list__foldl_3_0);
	}
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_thread_engine_base
#endif
MR_END_MODULE


MR_BEGIN_MODULE(check_hlds__mode_ordering_module34)
	MR_init_entry1(check_hlds__mode_ordering__IntroducedFrom__pred__find_matching_proc_2__569__1_7_0);
	MR_INIT_PROC_LAYOUT_ADDR(mercury__check_hlds__mode_ordering__IntroducedFrom__pred__find_matching_proc_2__569__1_7_0);
	MR_init_label3(check_hlds__mode_ordering__IntroducedFrom__pred__find_matching_proc_2__569__1_7_0,1,3,4)
MR_BEGIN_CODE

/*-------------------------------------------------------------------------*/
/* code for 'IntroducedFrom__pred__find_matching_proc_2__569__1'/7 mode 0 */
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_local_thread_engine_base
#endif
MR_def_static(check_hlds__mode_ordering__IntroducedFrom__pred__find_matching_proc_2__569__1_7_0)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_mkframe("pred check_hlds.mode_ordering.IntroducedFrom__pred__find_matching_proc_2__569__1/7-0", 3,
		MR_ENTRY(MR_do_fail));
	MR_fv(1) = MR_r4;
	MR_fv(2) = MR_r6;
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_r2;
	MR_r2 = MR_r3;
	MR_r3 = MR_tempr1;
	MR_r4 = MR_r5;
	}
	MR_np_call_localret_ent(hlds__inst_graph__corresponding_nodes_from_lists_6_0,
		check_hlds__mode_ordering__IntroducedFrom__pred__find_matching_proc_2__569__1_7_0_i1);
MR_def_label(check_hlds__mode_ordering__IntroducedFrom__pred__find_matching_proc_2__569__1_7_0,1)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tag_alloc_heap(MR_tempr1, 0, (MR_Integer) 1);
	MR_tfield(0, MR_tempr1, 0) = MR_r2;
	MR_fv(3) = MR_r1;
	MR_r1 = MR_fv(1);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(fn__check_hlds__mode_constraint_robdd__mode_constraint_var_2_0,
		check_hlds__mode_ordering__IntroducedFrom__pred__find_matching_proc_2__569__1_7_0_i3);
MR_def_label(check_hlds__mode_ordering__IntroducedFrom__pred__find_matching_proc_2__569__1_7_0,3)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_fv(2);
	MR_r2 = MR_tempr1;
	}
	MR_np_call_localret_ent(f_109_111_100_101_95_114_111_98_100_100_95_95_116_102_101_105_114_110_95_95_84_121_112_101_83_112_101_99_79_102_95_95_112_114_101_100_95_111_114_95_102_117_110_99_95_95_118_97_114_95_101_110_116_97_105_108_101_100_95_95_91_84_32_61_32_109_99_95_116_121_112_101_93_95_48_95_49_2_0,
		check_hlds__mode_ordering__IntroducedFrom__pred__find_matching_proc_2__569__1_7_0_i4);
MR_def_label(check_hlds__mode_ordering__IntroducedFrom__pred__find_matching_proc_2__569__1_7_0,4)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (!(MR_r1)) {
		MR_GOTO(MR_ENTRY(MR_do_redo));
	}
	MR_r1 = MR_fv(3);
	MR_succeed();
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_thread_engine_base
#endif
MR_END_MODULE

extern const MR_TypeCtorInfo_Struct mercury_data_hlds__hlds_pred__type_ctor_info_pred_info_0;
MR_decl_entry(hlds__hlds_module__module_info_set_pred_info_4_0);
MR_decl_entry(fn__hlds__hlds_pred__pred_info_non_imported_procids_1_0);

MR_BEGIN_MODULE(check_hlds__mode_ordering_module35)
	MR_init_entry1(f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_112_114_101_100_95_95_91_50_93_95_48_5_0);
	MR_INIT_PROC_LAYOUT_ADDR(mercury__f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_112_114_101_100_95_95_91_50_93_95_48_5_0);
	MR_init_label9(f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_112_114_101_100_95_95_91_50_93_95_48_5_0,4,5,6,9,12,11,8,17,19)
MR_BEGIN_CODE

/*-------------------------------------------------------------------------*/
/* code for 'UnusedArgs__pred__mode_ordering_pred__[2]_0'/5 mode 0 */
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_local_thread_engine_base
#endif
MR_def_static(f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_112_114_101_100_95_95_91_50_93_95_48_5_0)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_incr_sp(10);
	MR_sv(10) = (MR_Word) MR_succip;
	MR_sv(1) = MR_r1;
	MR_sv(2) = MR_r2;
	MR_sv(7) = MR_r3;
	MR_sv(8) = (MR_Word) MR_TAG_COMMON(0,1,13);
	MR_sv(9) = (MR_Word) MR_TAG_COMMON(0,1,14);
	MR_r1 = (MR_Word) MR_CTOR0_ADDR(hlds__hlds_pred, pred_id);
	MR_r2 = MR_sv(9);
	MR_np_call_localret_ent(fn__map__init_0_0,
		f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_112_114_101_100_95_95_91_50_93_95_48_5_0_i4);
MR_def_label(f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_112_114_101_100_95_95_91_50_93_95_48_5_0,4)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(3) = MR_r1;
	MR_r1 = MR_sv(7);
	MR_r2 = MR_sv(2);
	MR_np_call_localret_ent(hlds__hlds_module__module_info_pred_info_3_0,
		f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_112_114_101_100_95_95_91_50_93_95_48_5_0_i5);
MR_def_label(f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_112_114_101_100_95_95_91_50_93_95_48_5_0,5)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(4) = MR_r1;
	MR_r1 = (MR_Word) MR_CTOR0_ADDR(hlds__hlds_pred, pred_id);
	MR_r2 = (MR_Word) MR_CTOR0_ADDR(check_hlds__mode_ordering, pred_constraint_info);
	MR_r3 = MR_sv(1);
	MR_r4 = MR_sv(2);
	MR_np_call_localret_ent(map__lookup_3_0,
		f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_112_114_101_100_95_95_91_50_93_95_48_5_0_i6);
MR_def_label(f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_112_114_101_100_95_95_91_50_93_95_48_5_0,6)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_sv(6) = MR_tfield(0, MR_r1, 1);
	MR_sv(5) = MR_tfield(0, MR_r1, 0);
	MR_r1 = MR_sv(4);
	MR_np_call_localret_ent(hlds__hlds_pred__pred_info_infer_modes_1_0,
		f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_112_114_101_100_95_95_91_50_93_95_48_5_0_i9);
MR_def_label(f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_112_114_101_100_95_95_91_50_93_95_48_5_0,9)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (!(MR_r1)) {
		MR_GOTO_LAB(f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_112_114_101_100_95_95_91_50_93_95_48_5_0_i8);
	}
	MR_r1 = (MR_Word) MR_CTOR0_ADDR(hlds__hlds_pred, pred_id);
	MR_r2 = MR_sv(9);
	MR_r3 = MR_sv(3);
	MR_r4 = MR_sv(2);
	MR_np_call_localret_ent(map__search_3_0,
		f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_112_114_101_100_95_95_91_50_93_95_48_5_0_i12);
MR_def_label(f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_112_114_101_100_95_95_91_50_93_95_48_5_0,12)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (!(MR_r1)) {
		MR_GOTO_LAB(f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_112_114_101_100_95_95_91_50_93_95_48_5_0_i11);
	}
	{
	MR_Word MR_tempr1, MR_tempr2;
	MR_tag_alloc_heap(MR_tempr1, 0, (MR_Integer) 8);
	MR_r3 = MR_tempr1;
	MR_tfield(0, MR_tempr1, 0) = (MR_Word) MR_COMMON(8,0);
	MR_tfield(0, MR_tempr1, 1) = (MR_Word) MR_ENTRY_AP(check_hlds__mode_ordering__mode_ordering_infer_proc_8_0);
	MR_tfield(0, MR_tempr1, 2) = (MR_Integer) 5;
	MR_tfield(0, MR_tempr1, 3) = MR_sv(7);
	MR_tfield(0, MR_tempr1, 4) = MR_sv(1);
	MR_tfield(0, MR_tempr1, 5) = MR_sv(2);
	MR_tfield(0, MR_tempr1, 6) = MR_sv(6);
	MR_tfield(0, MR_tempr1, 7) = MR_sv(5);
	MR_r1 = MR_sv(8);
	MR_tempr2 = MR_r2;
	MR_r2 = (MR_Word) MR_CTOR0_ADDR(hlds__hlds_pred, pred_info);
	MR_r4 = MR_tempr2;
	MR_r5 = MR_sv(4);
	}
	MR_np_call_localret_ent(list__foldl_4_0,
		f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_112_114_101_100_95_95_91_50_93_95_48_5_0_i19);
MR_def_label(f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_112_114_101_100_95_95_91_50_93_95_48_5_0,11)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_r1 = MR_sv(2);
	MR_r2 = MR_sv(4);
	MR_r3 = MR_sv(7);
	MR_succip_word = MR_sv(10);
	MR_decr_sp(10);
	MR_np_tailcall_ent(hlds__hlds_module__module_info_set_pred_info_4_0);
MR_def_label(f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_112_114_101_100_95_95_91_50_93_95_48_5_0,8)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_r1 = MR_sv(4);
	MR_np_call_localret_ent(fn__hlds__hlds_pred__pred_info_non_imported_procids_1_0,
		f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_112_114_101_100_95_95_91_50_93_95_48_5_0_i17);
MR_def_label(f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_112_114_101_100_95_95_91_50_93_95_48_5_0,17)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1, MR_tempr2;
	MR_tag_alloc_heap(MR_tempr1, 0, (MR_Integer) 8);
	MR_r3 = MR_tempr1;
	MR_tfield(0, MR_tempr1, 0) = (MR_Word) MR_COMMON(8,1);
	MR_tfield(0, MR_tempr1, 1) = (MR_Word) MR_ENTRY_AP(check_hlds__mode_ordering__mode_ordering_check_proc_8_0);
	MR_tfield(0, MR_tempr1, 2) = (MR_Integer) 5;
	MR_tfield(0, MR_tempr1, 3) = MR_sv(7);
	MR_tfield(0, MR_tempr1, 4) = MR_sv(1);
	MR_tfield(0, MR_tempr1, 5) = MR_sv(2);
	MR_tfield(0, MR_tempr1, 6) = MR_sv(6);
	MR_tfield(0, MR_tempr1, 7) = MR_sv(5);
	MR_tempr2 = MR_r1;
	MR_r1 = (MR_Word) MR_CTOR0_ADDR(hlds__hlds_pred, proc_id);
	MR_r2 = (MR_Word) MR_CTOR0_ADDR(hlds__hlds_pred, pred_info);
	MR_r4 = MR_tempr2;
	MR_r5 = MR_sv(4);
	}
	MR_np_call_localret_ent(list__foldl_4_0,
		f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_112_114_101_100_95_95_91_50_93_95_48_5_0_i19);
MR_def_label(f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_112_114_101_100_95_95_91_50_93_95_48_5_0,19)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(2);
	MR_r2 = MR_tempr1;
	MR_r3 = MR_sv(7);
	MR_succip_word = MR_sv(10);
	MR_decr_sp(10);
	MR_np_tailcall_ent(hlds__hlds_module__module_info_set_pred_info_4_0);
	}
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_thread_engine_base
#endif
MR_END_MODULE

MR_decl_entry(builtin__semidet_succeed_0_0);
MR_decl_entry(require__sorry_3_0);
MR_decl_entry(fn__hlds__hlds_pred__initial_proc_id_0_0);

MR_BEGIN_MODULE(check_hlds__mode_ordering_module36)
	MR_init_entry1(f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_105_110_102_101_114_95_112_114_111_99_95_95_91_54_93_95_48_8_0);
	MR_INIT_PROC_LAYOUT_ADDR(mercury__f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_105_110_102_101_114_95_112_114_111_99_95_95_91_54_93_95_48_8_0);
	MR_init_label4(f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_105_110_102_101_114_95_112_114_111_99_95_95_91_54_93_95_48_8_0,4,2,7,8)
MR_BEGIN_CODE

/*-------------------------------------------------------------------------*/
/* code for 'UnusedArgs__pred__mode_ordering_infer_proc__[6]_0'/8 mode 0 */
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_local_thread_engine_base
#endif
MR_def_static(f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_105_110_102_101_114_95_112_114_111_99_95_95_91_54_93_95_48_8_0)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_incr_sp(7);
	MR_sv(7) = (MR_Word) MR_succip;
	MR_sv(1) = MR_r1;
	MR_sv(2) = MR_r2;
	MR_sv(3) = MR_r3;
	MR_sv(4) = MR_r4;
	MR_sv(5) = MR_r5;
	MR_sv(6) = MR_r6;
	MR_np_call_localret_ent(builtin__semidet_succeed_0_0,
		f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_105_110_102_101_114_95_112_114_111_99_95_95_91_54_93_95_48_8_0_i4);
MR_def_label(f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_105_110_102_101_114_95_112_114_111_99_95_95_91_54_93_95_48_8_0,4)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	if (!(MR_r1)) {
		MR_GOTO_LAB(f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_105_110_102_101_114_95_112_114_111_99_95_95_91_54_93_95_48_8_0_i2);
	}
	MR_r1 = (MR_Word) MR_string_const("check_hlds.mode_ordering", 24);
	MR_r2 = (MR_Word) MR_string_const("predicate \140check_hlds.mode_ordering.pred_info_create_proc_info_for_mode_decl_constraint\'/4", 90);
	MR_r3 = (MR_Word) MR_string_const("NYI", 3);
	MR_np_call_localret_ent(require__sorry_3_0,
		f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_105_110_102_101_114_95_112_114_111_99_95_95_91_54_93_95_48_8_0_i8);
MR_def_label(f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_105_110_102_101_114_95_112_114_111_99_95_95_91_54_93_95_48_8_0,2)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_r1 = MR_sv(1);
	MR_r2 = MR_sv(2);
	{
	MR_Word MR_tempr1, MR_tempr2, MR_tempr3, MR_tempr4;
	MR_tempr1 = MR_sv(3);
	MR_tempr2 = MR_sv(4);
	MR_tempr3 = MR_sv(5);
	MR_tempr4 = MR_sv(6);
	}
	MR_np_call_localret_ent(fn__hlds__hlds_pred__initial_proc_id_0_0,
		f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_105_110_102_101_114_95_112_114_111_99_95_95_91_54_93_95_48_8_0_i7);
MR_def_label(f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_105_110_102_101_114_95_112_114_111_99_95_95_91_54_93_95_48_8_0,7)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	{
	MR_Word MR_tempr1;
	MR_tempr1 = MR_r1;
	MR_r1 = MR_sv(1);
	MR_r2 = MR_sv(2);
	MR_r3 = MR_sv(3);
	MR_r4 = MR_sv(4);
	MR_r5 = MR_sv(5);
	MR_r6 = MR_tempr1;
	MR_r7 = MR_sv(6);
	}
MR_def_label(f_99_104_101_99_107_95_104_108_100_115_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_95_85_110_117_115_101_100_65_114_103_115_95_95_112_114_101_100_95_95_109_111_100_101_95_111_114_100_101_114_105_110_103_95_105_110_102_101_114_95_112_114_111_99_95_95_91_54_93_95_48_8_0,8)
	MR_MAYBE_INIT_LOCAL_THREAD_ENGINE_BASE
	MR_succip_word = MR_sv(7);
	MR_decr_sp(7);
	MR_np_tailcall_ent(check_hlds__mode_ordering__mode_ordering_check_proc_8_0);
#ifdef MR_maybe_local_thread_engine_base
	#undef MR_maybe_local_thread_engine_base
	#define MR_maybe_local_thread_engine_base MR_thread_engine_base
#endif
MR_END_MODULE

static void mercury__check_hlds__mode_ordering_maybe_bunch_0(void)
{
	check_hlds__mode_ordering_module0();
	check_hlds__mode_ordering_module1();
	check_hlds__mode_ordering_module2();
	check_hlds__mode_ordering_module3();
	check_hlds__mode_ordering_module4();
	check_hlds__mode_ordering_module5();
	check_hlds__mode_ordering_module6();
	check_hlds__mode_ordering_module7();
	check_hlds__mode_ordering_module8();
	check_hlds__mode_ordering_module9();
	check_hlds__mode_ordering_module10();
	check_hlds__mode_ordering_module11();
	check_hlds__mode_ordering_module12();
	check_hlds__mode_ordering_module13();
	check_hlds__mode_ordering_module14();
	check_hlds__mode_ordering_module15();
	check_hlds__mode_ordering_module16();
	check_hlds__mode_ordering_module17();
	check_hlds__mode_ordering_module18();
	check_hlds__mode_ordering_module19();
	check_hlds__mode_ordering_module20();
	check_hlds__mode_ordering_module21();
	check_hlds__mode_ordering_module22();
	check_hlds__mode_ordering_module23();
	check_hlds__mode_ordering_module24();
	check_hlds__mode_ordering_module25();
	check_hlds__mode_ordering_module26();
	check_hlds__mode_ordering_module27();
	check_hlds__mode_ordering_module28();
	check_hlds__mode_ordering_module29();
	check_hlds__mode_ordering_module30();
	check_hlds__mode_ordering_module31();
	check_hlds__mode_ordering_module32();
	check_hlds__mode_ordering_module33();
	check_hlds__mode_ordering_module34();
	check_hlds__mode_ordering_module35();
	check_hlds__mode_ordering_module36();
}

/* suppress gcc -Wmissing-decls warnings */
void mercury__check_hlds__mode_ordering__init(void);
void mercury__check_hlds__mode_ordering__init_type_tables(void);
void mercury__check_hlds__mode_ordering__init_debugger(void);
#ifdef MR_DEEP_PROFILING
void mercury__check_hlds__mode_ordering__write_out_proc_statics(FILE *deep_fp, FILE *procrep_fp);
#endif
#ifdef MR_RECORD_TERM_SIZES
void mercury__check_hlds__mode_ordering__init_complexity_procs(void);
#endif
#ifdef MR_THREADSCOPE
void mercury__check_hlds__mode_ordering__init_threadscope_string_table(void);
#endif

void mercury__check_hlds__mode_ordering__init(void)
{
	static MR_bool done = MR_FALSE;
	if (done) {
		return;
	}
	done = MR_TRUE;
	mercury__check_hlds__mode_ordering_maybe_bunch_0();
	MR_INIT_TYPE_CTOR_INFO(
		mercury_data_check_hlds__mode_ordering__type_ctor_info_mode_ordering_info_0,
		check_hlds__mode_ordering__mode_ordering_info_0_0);
	MR_INIT_TYPE_CTOR_INFO(
		mercury_data_check_hlds__mode_ordering__type_ctor_info_pred_constraint_info_0,
		check_hlds__mode_ordering__pred_constraint_info_0_0);
	MR_INIT_TYPE_CTOR_INFO(
		mercury_data_check_hlds__mode_ordering__type_ctor_info_pred_constraint_map_0,
		check_hlds__mode_ordering__pred_constraint_map_0_0);
	mercury__check_hlds__mode_ordering__init_debugger();
}

void mercury__check_hlds__mode_ordering__init_type_tables(void)
{
	static MR_bool done = MR_FALSE;
	if (done) {
		return;
	}
	done = MR_TRUE;
	{
		MR_register_type_ctor_info(
		&mercury_data_check_hlds__mode_ordering__type_ctor_info_mode_ordering_info_0);
	}
	{
		MR_register_type_ctor_info(
		&mercury_data_check_hlds__mode_ordering__type_ctor_info_pred_constraint_info_0);
	}
	{
		MR_register_type_ctor_info(
		&mercury_data_check_hlds__mode_ordering__type_ctor_info_pred_constraint_map_0);
	}
}


void mercury__check_hlds__mode_ordering__init_debugger(void)
{
	static MR_bool done = MR_FALSE;
	if (done) {
		return;
	}
	done = MR_TRUE;
}

#ifdef MR_DEEP_PROFILING

void mercury__check_hlds__mode_ordering__write_out_proc_statics(FILE *deep_fp, FILE *procrep_fp)
{
	MR_write_out_module_proc_reps_start(procrep_fp, &mercury_data__module_common_layout__check_hlds__mode_ordering);
	MR_write_out_module_proc_reps_end(procrep_fp);
}

#endif

#ifdef MR_RECORD_TERM_SIZES

void mercury__check_hlds__mode_ordering__init_complexity_procs(void)
{
}

#endif

#ifdef MR_THREADSCOPE

void mercury__check_hlds__mode_ordering__init_threadscope_string_table(void)
{
}

#endif

/* ensure everything is compiled with the same grade */
static const void *const MR_grade = &MR_GRADE_VAR;
